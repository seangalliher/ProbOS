...
### AD-855 - BLOCKED -> request -> approve -> resume work-item gap driver (Wave 212)

Third unit of the Crew Self-Unblock epic (built on AD-853's CapabilityRequest queue + AD-854's triage router; AD-856/857 still deferred - AgenticLoop dispatch bridge, HXI/chat decision surface). Closes the loop on the work-item kanban board so a capability gap encountered mid-task no longer strands the card: it drives the card into BLOCKED, files the request via triage, and on resolution either resumes-and-re-dispatches (approved+fulfilled) or cancels (denied). New module src/probos/cognitive/capability_gap_driver.py with class CapabilityGapDriver(*, runtime, work_item_store, capability_request_store). async on_capability_gap(*, work_item_id, gap_target, agent_id) -> CapabilityRequest | None does the forward path in strict order: (1) triage_and_file(...) FIRST to obtain req.id, passing rationale="work item {id} blocked on capability: {gap_target}", work_item_id, and the live registries via getattr (tool_registry, tool_permission_store, extension_registry, ontology, trust_network, self_mod_pipeline) + config from runtime.config.capability_triage; (2) transition_work_item(id, "blocked", source="capability_gap_driver") - the in_progress->blocked transition is legal for the task state machine (AD-498); (3) read-merge-write metadata (get_work_item -> dict(item.metadata) -> set blocked_reason=gap_target + capability_request_id=req.id -> update_work_item) because update_work_item REPLACES metadata wholesale, so a naive write would clobber pre-existing keys like dispatchable. async on_capability_event(event) is the resolution listener registered on runtime.add_event_listener for ["capability_request_fulfilled","capability_request_decided"]; it reads the AD-1118 envelope shape (event["data"]), recovers the work_item_id via request_store.get(request_id) because DECIDED/FULFILLED payloads do NOT carry work_item_id (only FILED does), and is idempotent: if the item is no longer "blocked" it returns early (debug). On CAPABILITY_REQUEST_FULFILLED -> _resume: transition blocked->in_progress + re-dispatch by calling runtime.work_item_router.on_work_item_created({"type":"work_item_created","data":{"work_item":item.to_dict()},...}). On CAPABILITY_REQUEST_DECIDED with data status=="denied" -> _cancel: transition blocked->cancelled (NOT failed - blocked->failed is illegal for tasks) + record denial_reason from req.decision_reason into merged metadata. DECIDED(approved) is a deliberate no-op: the grant fast-path fires DECIDED+FULFILLED together, so resuming on approval AND fulfillment would double-dispatch; resume keys on FULFILLED only. Every public method is Tier-2 log-and-degrade (logger.warning + return None/early-return) when a store, router, or item is absent - never raises. Wiring: src/probos/startup/finalize.py gains _wire_capability_gap_driver(*, runtime, config) -> bool placed immediately after _wire_hybrid_dispatch (it needs its own function because _wire_hybrid_dispatch early-returns); it skips cleanly (logger.info + return False) when either store is absent, otherwise constructs runtime.capability_gap_driver and registers the event listener via the AD-581a getattr+try/except pattern. runtime.py gains the TYPE_CHECKING import + class-body annotation self.capability_gap_driver: "CapabilityGapDriver | None" = None (forward-ref string to avoid the cognitive->runtime import cycle). Acceptance: tests/test_ad855_block_resume.py - 8 tests against REAL WorkItemStore + REAL CapabilityRequestStore (NO MagicMock at the storage boundary per BF-287) with a small _RecordingRouter stub: gap->blocked+merged-metadata (asserts pre-existing dispatchable survives)+request filed carrying work_item_id, FULFILLED->in_progress+re-dispatch, DECIDED(denied)->cancelled+denial_reason, DECIDED(approved)-on-blocked->no-op, work_item_id recovered via store.get() when the envelope omits it, absent-store degrade-no-raise, missing-request-id ignored. Focused gate 38 passed (8 new + AD-853 8 + AD-854 22); broader gate test_workforce.py 114 + finalize suites 9 unchanged. One commit, no push.

### AD-856 - AgenticLoop executor for dispatchable work items (bridge) + capability-gap denial capture (Wave 213)

Fourth unit of the Crew Self-Unblock epic (built on AD-855's gap driver; AD-857 still deferred - HXI/chat decision surface). Bridges the agent dispatch seam to the multi-iteration AgenticLoop (AD-545) so a dispatched work item runs as a real tool-using agentic session instead of a single-shot intent, and any tool denied mid-loop is surfaced as a capability gap through AD-855's driver. New config class AgenticDispatchConfig(BaseModel) with a single transitional field enabled: bool = False (convention #14 - default OFF, flip in a later grandchild AD) wired onto SystemConfig as agentic_dispatch via Field(default_factory). New module src/probos/cognitive/agentic_dispatch.py with three pieces: (1) class DispatchToolExecutor(ToolExecutor) overrides async invoke to catch ToolPermissionDenied, append the denied tool_id to self.denied_tools, log INFO, and RE-RAISE (AgenticLoop translates the re-raise into an is_error ToolCallResult and continues, so denied_tools is the post-loop record of every gap hit during the session); (2) class _MeshIntentTool structurally implements the Tool Protocol (tool_type=ToolType.UTILITY_AGENT) and on invoke broadcasts an IntentMessage(intent=...) onto the mesh IntentBus, wrapping web_search/read_page/http_fetch as first-class tools the LLM can call; (3) def register_mesh_intent_tools(registry, intent_bus) -> list[str] idempotently registers the three mesh tools (skips any already-present tool_id) with empty default_permissions (Layer-3 READ-for-all) and provider="AD-856". Wiring in cognitive_agent.py: _handle_work_item_dispatch (AD-839) now calls the new async _run_agentic_dispatch(*, work_item_id, task_text, runtime) -> str | None FIRST; on None (gate off OR any of llm_client / tool_permission_store / capability_gap_driver / tool_registry unwired) it falls back to the original single-shot await self.handle_intent(dm) seam. BOTH paths preserve the Captain task-message thread-log (before), the agent-reply thread-log (after, Tier-2 try/except), and await store.transition_work_item(id, "in_progress", source=self.id) (after, Tier-2 try/except). _run_agentic_dispatch gathers deps off runtime, builds a DispatchToolExecutor over the registry, registers the mesh tools, scopes the LLM tool list to the agent's active grants (perm_store.get_active_grants_sync, restrictions filtered out) UNION the available mesh tool ids (dedup via dict.fromkeys, each resolved through tool_registration_to_llm_definition), runs loop = AgenticLoop(llm_client, tool_executor, event_emit_fn=runtime.emit_event).run(system_prompt=instructions, user_message=task_text, tools=..., context={agent_id, department, rank}), then for every denied_tool in executor.denied_tools awaits gap_driver.on_capability_gap(work_item_id, gap_target=denied_tool, agent_id=self.id) and returns agentic_result.final_text or "". Four Architect corrections honored: (1) denial-capture seam via DispatchToolExecutor.denied_tools + post-loop on_capability_gap (not a pre-loop permission pre-check); (2) three mesh-intent Tool wrappers registered idempotently with empty default_permissions; (3) feature gate enabled=False + full fallback to handle_intent(dm) preserving thread-logging AND transition_work_item on both paths; (4) the real single-shot seam is await self.handle_intent(dm), NOT act(). Tests: tests/test_ad856_agentic_dispatch.py 7 passed using REAL ToolRegistry + ToolPermissionStore fixtures (BF-287 - MagicMock at the substrate boundary auto-creates phantom attributes and false-greens); a denial is forced by registering a tool with default_permissions={"captain":"full"} so an ensign resolves NONE and trips ToolPermissionDenied on the default READ requirement. Regression: the pre-existing test_ad839 _make_runtime MagicMock had a truthy config.agentic_dispatch.enabled leaf that wrongly activated the new path; pinned it to False (production default) - test_ad839 16 + test_ad855 + test_config all green. One commit, no push.

### AD-857 - Capability-request HXI/chat decision surface (thin API + dual-surface Captain-DM listener + HXI card) (Wave 214)

Fifth and final unit of the Crew Self-Unblock epic (built on AD-853's CapabilityRequest queue + AD-855's gap driver). Surfaces a pending CapabilityRequest on TWO surfaces per HXI Principle #9 (alert-driven layout reconfiguration) so the Captain can approve/deny from either the HXI card or the relevant 1:1 chat. Four Architect verify-first corrections honored: (1) the cited "Yeo completion DM (AD-846)" is UNBUILT - no reusable completion-DM helper exists; the chat half instead REUSES the AD-485 proactive Captain-DM primitive (find-or-create dm-captain-{agent_id[:8]} channel via ward_room.create_channel, post via ward_room.create_thread) rather than authoring a second DM mechanism; (2) CapabilityRequestStore.decide(request_id, approve, reason="", decided_by="captain") returns None on unknown id (no raise) and has NO already-decided guard, so the endpoint owns both - unknown id -> 404, req.status != "pending" -> 400; (3) the CAPABILITY_REQUEST_FILED/DECIDED events omit work_item_id/rationale, so the HXI card reads the full CapabilityRequest objects (work_item_id/rationale/target) from the GET list, not the event payload; (4) no existing HXI approval-card or department->color constant existed - the Builder created both. Thin API: new src/probos/routers/capability_requests.py with APIRouter(prefix="/api/capability-requests", tags=["capability-requests"]), both endpoints runtime: Any = Depends(get_runtime) from routers/deps.py, registered in BOTH the import tuple AND the include loop in api.py. GET ""?status=pending -> 503 if no runtime.capability_request_store else (non-pending status -> empty list; pending -> await store.list_pending()) serialized via _serialize(req) -> dict (id, agent_id, kind, target, rationale, work_item_id, status, created_at, decided_at, decided_by, decision_reason). POST "/{request_id}/decide" (body CapabilityRequestDecideRequest: approve: bool, reason: str = "") -> 503 guard; existing = await store.get(request_id); None -> 404; existing.status != "pending" -> 400; then await store.decide(request_id, req.approve, reason=req.reason, decided_by="captain"). New api_models.py model CapabilityRequestDecideRequest with a Pydantic v2 @model_validator(mode="after") _require_reason_on_deny raising ValueError ("a reason is required when denying a capability request") when not approve and not reason.strip() -> 422 at the API boundary. Thin chat-delivery listener: new src/probos/capability_request_notifier.py module-level async notify_captain_of_capability_request(runtime, event) -> None; honest-degrade (Tier-2 log-and-return) when runtime.ward_room is None or the event carries no agent_id; best-effort callsign via SYNC registry.get(agent_id) + callsign_registry.get_callsign(agent.agent_type); reuses an existing dm channel matched by name+channel_type=="dm" via list_channels() else create_channel(channel_type="dm"); posts a one-line notice via create_thread. Wired in startup/finalize.py via _wire_capability_request_notifier(*, runtime, config) -> bool immediately after _wire_capability_gap_driver, subscribing an inner async _on_filed via getattr(runtime, "add_event_listener", None) with event_types=["capability_request_filed"] (string value, matching the neighboring work_item_created/gap-driver listeners), Tier-2 log-and-degrade if the listener API is absent or registration raises; the chat notice is informational, the authoritative approve/deny goes through the API. HXI card: new ui/src/components/capability/CapabilityRequestPanel.tsx (default export) - WorkBoard.tsx fetch/structure copy-pattern; fetches /api/capability-requests?status=pending on mount (Tier-1 swallow), renders a department-colored (Builder-defined DEPARTMENT_COLORS map) pending card per request with rationale + linked work item + Approve/Deny buttons, inline stroke-SVG glyphs only (strokeWidth 1.5, strokeLinecap round, amber #f0b060 active / #666680 dim - NO emoji per Principle #3), POSTs {approve, reason} JSON to /api/capability-requests/{id}/decide and removes the decided card; returns null when loaded and empty (recedes when no pending work). Tests (substrate-boundary rule - REAL CapabilityRequestStore over tmp-sqlite, NOT MagicMock): 9 API tests (test_ad857_capability_request_api.py - list happy/empty/503, decide approve/deny-records-reason/deny-without-reason-422/unknown-404/already-decided-400/no-store-503), 4 chat-delivery tests (test_ad857_capability_request_notifier.py - creates dm channel+thread, reuses existing channel, degrades on no ward_room, degrades on no agent_id), 1+ vitest component test (ui/src/__tests__/CapabilityRequestPanel.test.tsx - renders pending card with rationale+buttons+linked-work-item; approve click POSTs approve=true to the decide endpoint; 2 tests). All additive (api.py +2, api_models.py +19/-1 import, finalize.py +44, two new src modules, two new test files, one new UI component + test). Focused: 13 API+notifier passed; vitest 2 passed; epic regression test_ad853/854/855 38 passed; import smoke OK. Scope honored: no new request kinds, no triage/resume changes, no second DM mechanism, AD-846 helper not built. One commit, no push.

### AD-859a - Extract the AD-856 dispatch loop into a reusable result-returning WorkItemAgenticExecutor + persisted tool trace (prerequisite refactor) (Wave 215)

Prerequisite refactor for the AD-859 crew fan-out (the original AD-859 plan to "reuse the WorkItemRouter seam" was a verify-first NO-GO: WorkItemRouter.on_work_item_created(event) -> None is fire-and-forget and the AD-856 inline path returned a bare str | None, discarding the AgenticResult/tool_calls/stopped_reason and persisting no trace - so no caller could collect a result with provenance). Pure extraction (zero behavior change on the AD-839/AD-856 path) of the loop-wiring out of CognitiveAgent._run_agentic_dispatch into two new pieces at the END of src/probos/cognitive/agentic_dispatch.py: (1) @dataclass WorkItemAgenticOutcome(final_text: str = "", stopped_reason: str = "", denied_tools: list[str] = field(default_factory=list), tool_trace_ref: str | None = None) - the structured replacement for the old str | None so BOTH the AD-839 handler AND the AD-859 crew executor can collect a result with provenance; (2) class WorkItemAgenticExecutor(*, llm_client) with async run(*, agent_id, instructions, task_text, runtime, department="", rank="ensign") -> WorkItemAgenticOutcome that mirrors the AD-856 inline body EXACTLY - reads tool_registry/tool_permission_store/intent_bus off the runtime via getattr, builds DispatchToolExecutor(registry=registry), honest-degrade register_mesh_intent_tools (guarded if intent_bus is not None and registry is not None), gathers grants via perm_store.get_active_grants_sync(agent_id) filtered on not is_restriction, dedupes [*granted_ids, *mesh_ids] via dict.fromkeys, builds the tool defs via tool_registration_to_llm_definition (skipping registry.get(tid) is None), constructs AgenticLoop(llm_client, tool_executor, event_emit_fn=getattr(runtime,"emit_event",None)) and awaits loop.run(system_prompt=instructions or "", user_message=task_text, tools=tools, context={agent_id, department, rank}) - then ADDS the one new AD-859a behavior: async _persist_tool_trace(agentic_result, runtime, agent_id) -> str | None serializes [dataclasses.asdict(tc) for tc in agentic_result.tool_calls] to json.dumps(sort_keys=True, default=str).encode("utf-8"), computes content_hash = hashlib.sha256(blob).hexdigest(), and awaits runtime.attachment_store.write(content_hash=..., blob=..., mime="application/json", origin="crew_trace") returning the SHA ref (AD-731: refs on the bus, bytes in the store), honest-degrading to None (Tier-2 log-and-degrade) when the store is unwired or the write raises - a missing trace is provenance loss, not a dispatch failure. Caller refactor: CognitiveAgent._run_agentic_dispatch keeps its signature (*, work_item_id, task_text, runtime) -> str | None and ALL its gate checks (runtime None; config/ad_cfg disabled; any of llm/perm_store/gap_driver/registry None -> return None) UNCHANGED, then delegates the loop body to WorkItemAgenticExecutor(llm_client=llm).run(...) and KEEPS the capability-gap surfacing loop (for denied_tool in outcome.denied_tools: await gap_driver.on_capability_gap(work_item_id, gap_target=denied_tool, agent_id=self.id), same Tier-2 try/except) IN THE CALLER - the executor only records denials, it does not drive the gap driver. Returns outcome.final_text or "". The cognitive_agent.py diff is a net ~60-line deletion from the pure line-move (expected, additive elsewhere). Tests (substrate-boundary rule - REAL ToolRegistry/ToolPermissionStore + a hand-written recording AttachmentStore, NOT MagicMock per BF-287; the AgenticLoop is driven end-to-end by a scripted LLM exactly as the AD-856 suite does): new test_ad859a_agentic_executor.py with 7 tests - executor returns final_text+stopped_reason ("complete") from the real loop, executor captures denied_tools when an ensign hits a captain-only tool, tool_trace_ref is a 64-char hex SHA + the recording store got exactly one write tagged mime="application/json"/origin="crew_trace" with content_hash==tool_trace_ref and bytes blob, tool_trace_ref honest-degrades to None when runtime.attachment_store is None, and three caller-regression tests proving _run_agentic_dispatch still returns the loop str / returns None when gated off / still surfaces gaps for denied tools. Focused 7 passed; AD-856 regression 7 passed. Scope honored: ONLY AD-859a - AD-859 (crew fan-out), AD-860, AD-861, AD-862 NOT built. One commit, no push.

### AD-859 - CrewTaskExecutor: dependency-gated crew fan-out collecting SubtaskResults via the AD-859a executor (Wave 216)

The crew fan-out unit of the Crew Collaboration epic, built directly on the AD-859a WorkItemAgenticExecutor (committed 608c7ce0) - it does NOT re-create the executor. Architect verify-first correction honored: there is NO dispatch-and-collect seam on WorkItemRouter (on_work_item_created is fire-and-forget -> None) and the AD-856 inline path only transitions to in_progress and returns str | None with no persisted trace, so AD-859 OWNS its own depends_on-gated scheduling and awaits WorkItemAgenticExecutor.run(...) DIRECTLY rather than reusing a router seam. New module src/probos/cognitive/crew_executor.py (Cognitive layer): (1) @dataclass SubtaskResult(work_item_id, spec_id, agent_id, output, status, tool_trace_ref: str | None = None, started_at: float = 0.0, finished_at: float = 0.0) - the per-subtask provenance record (tool_trace_ref is the AD-731 content-addressable SHA ref, never inline bytes); (2) class CrewTaskExecutor(*, work_item_store, agent_registry, agentic_executor, runtime, max_parallel_subtasks=3, emit_fn=None) storing _max_parallel = max(1, int(...)) and a _tasks: set held-reference set for every create_task (async-discipline). async run(parent_id) -> list[SubtaskResult] lists children via list_work_items(parent_id=parent_id, limit=1000), emits CREW_TASK_STARTED once with {parent_id, child_count}, returns [] on no children, then runs a depends_on-gated scheduler: an asyncio.Semaphore(self._max_parallel) bounds concurrency (Safety Budget - a wide parent cannot exhaust the LLM tier), runnable children (deps in done_ids, not yet started) are dispatched via create_task into _tasks, awaited with asyncio.wait(..., return_when=asyncio.FIRST_COMPLETED), finished tasks discarded from _tasks/pending and added to done_ids only on status=="done", and SUBTASK_COMPLETED emitted per finished child; if pending children remain but nothing is runnable (blocked by a failed dependency) it logs a Tier-2 warning and breaks rather than hanging. _deps_met(child, done_ids) = all(dep in done_ids for dep in child.depends_on or []). async _run_child(child) -> SubtaskResult resolves the agent via registry.get(child.assigned_to) (None -> status="failed", executor NEVER called), transitions the task child open->in_progress (transition_work_item(child.id, "in_progress", source="crew_executor"), honoring the task state machine that forbids a direct open->done), awaits executor.run(agent_id, instructions, task_text, runtime, department, rank) in a Tier-2 try/except (Exception -> log-and-degrade status="failed"), and on outcome.stopped_reason == "complete" transitions in_progress->done with status="done" else keeps status = outcome.stopped_reason or "failed" (no done transition - a failed child surfaces its status without being marked done, so its dependents stay blocked). _emit(event_type, data) honest-degrades when emit_fn is None and wraps the call in Tier-2 try/except. Two new EventType members added in events.py (CREW_TASK_STARTED, SUBTASK_COMPLETED) and one new config field AgenticDispatchConfig.max_parallel_subtasks: int = 3 (conservative default per Safety Budget). Tests (substrate-boundary rule - REAL WorkItemStore fixture, NOT MagicMock per BF-287; _Fake agent/registry/executor stubs and an _EventSink callable): new test_ad859_crew_executor.py with 8 tests - independent children all run and complete; a child waits for its depends_on; SubtaskResult carries the persistent agent_id and a str (not bytes) tool_trace_ref; a failed child surfaces its status without unblocking dependents and is NOT marked done in the store; the concurrency cap is respected (max_active <= 2 and reaches 2); lifecycle events emitted (1 CREW_TASK_STARTED, 2 SUBTASK_COMPLETED); no children returns [] but still emits the start event; an unresolvable agent marks the child failed without calling the executor. Focused 8 passed (+ AD-859a 7 = 15); workforce + AD-856 regression 121 passed. Scope honored: ONLY AD-859 - AD-859a reused (not rebuilt), AD-860/861/862 NOT built. One commit, no push.

### AD-860 - SubtaskVerifier: adversarial LLM-judge convergence gate anchored to expected_output, trust-recorded, Vote-mapped (Wave 217)

The verification/convergence unit of the Crew Collaboration epic - the semantic sibling of the deterministic RedTeamAgent (RedTeamAgent stays the tool re-execution verifier; this is new, for semantic task results). Three Architect verify-first blockers fixed before build: (A) the expected_output anchor was dead - AD-858 added WorkItemSpec.expected_output (consultation/dispatch.py:64) but ParallelDispatcher.dispatch() never copied it into the persisted WorkItem metadata, so the "anchored to the declared contract" path was unreachable and every subtask hit the free-text fallback. Fix (additive, one key, persistence-not-logic so the don't-fork-the-dispatcher boundary holds): metadata gains "expected_output": spec.expected_output at dispatch.py:~452; the verifier resolves the contract via wi = await store.get_work_item(result.work_item_id); expected = (wi.metadata or {}).get("expected_output"). (B) the convergence loop cannot go "back through AD-859" - CrewTaskExecutor exposes no single-child-with-critique seam and the duty state machine has NO done->in_progress transition (done is terminal, workforce.py:86), so a completed child cannot be re-opened. Fix: re-dispatch points at the PUBLIC AD-859a engine - await WorkItemAgenticExecutor.run(agent_id=<producer>, instructions=<orig>, task_text=<orig_task + "\n\nCRITIQUE:\n" + verdict.critique>, runtime=runtime) (async, public, state-machine-independent); the executor is an AD-860 constructor dependency. (C) record_outcome is SYNCHRONOUS and keyword-shaped (consensus/trust.py:217: def record_outcome(self, agent_id, success, weight=1.0, intent_type="", episode_id="", verifier_id="", source="verification") -> float) - episode_id/verifier_id sit between intent_type and source, so a positional 5th arg would land in episode_id; called with keywords only, no await. New module src/probos/cognitive/crew_verifier.py (Cognitive layer): (1) @dataclass VerificationVerdict(accepted: bool, confidence: float, critique: str, verifier_agent_id: str); (2) @dataclass ConvergenceOutcome(result, verdict, status, rounds=0) with status constants converged/unverified; (3) class SubtaskVerifier(*, llm_client, work_item_store, agent_registry, trust_network, agentic_executor, runtime, max_convergence_rounds=2) storing _max_rounds = max(1, int(...)). async verify(result) -> VerificationVerdict picks an INDEPENDENT verifier via _pick_independent_verifier (registry.all(), first agent .id != producer; None -> honest-degrade VerificationVerdict(accepted=False, confidence=0.0, critique="No independent verifier available...", verifier_agent_id="") with NO trust write - never lets an agent verify itself), resolves expected_output, builds LLMRequest(prompt=_build_judge_prompt(result, expected), system_prompt=_JUDGE_SYSTEM_PROMPT, tier="standard"), awaits llm_client.complete (LLM failure -> honest-degrade refuted verdict), parses via robust _parse_verdict (non-JSON -> refuted), then records the verdict against the trust ledger via the SYNCHRONOUS keyword call self._trust.record_outcome(verdict.verifier_agent_id, success=verdict.accepted, intent_type="crew_verification", verifier_id=result.agent_id, source="crew_verification"). async converge(result, *, instructions, task_text) -> ConvergenceOutcome returns converged at rounds=0 if first verdict accepts, else loops up to _max_rounds re-running the producer through WorkItemAgenticExecutor.run with the critique appended to task_text (result.output updated from outcome.final_text), re-verifying each round; converged on accept, else status=unverified (max rounds reached - escalates, does NOT silently accept a refuted result - Safety Budget). @staticmethod verdict_to_vote(verdict) -> Vote builds Vote(agent_id=verifier_agent_id, approved=accepted, confidence=confidence, reason=critique) so AD-861 can call compute_shapley_values - this AD does NOT call it and does NOT invent a parallel attribution scheme. Config: AgenticDispatchConfig.max_convergence_rounds: int = 2 (config.py, Safety-Budget bound). tests/test_ad860_crew_verifier.py: 11 (fake LLM judge + fake WorkItemStore + fake registry/executor + REAL TrustNetwork per BF-287) - accepted folds through with positive trust; refuted records negative trust; prompt anchored to expected_output when present; free-text fallback when absent (a genuine branch now persistence is fixed); no-independent-agent -> unverified degrade; verifier != producer enforced; non-JSON -> refuted degrade; converge accepts first-pass with no re-run; refuted-then-accepted re-runs via the AD-859a executor with the critique in task_text; max-rounds -> unverified not silent-accept; verdict_to_vote maps the real Vote shape. Gate: focused 11 passed; regression (AD-858/859/859a/860) 40 passed; config + trust 21 passed; import smoke confirms max_convergence_rounds=2. Scope honored: ONLY AD-860 - RedTeamAgent NOT modified, compute_shapley_values NOT called (AD-861), no done->in_progress duty transition, no new EventType, synthesis/UI NOT built. One commit, no push.

### AD-861 - CrewSynthesizer: fold verified ConvergenceOutcomes into parent completion with Shapley attribution + collaboration episode (Wave 218)

The synthesis/completion unit of the Crew Collaboration epic - the terminal stage after AD-859's fan-out and AD-860's verification convergence. New module src/probos/cognitive/crew_synth.py (Cognitive layer) with @dataclass SynthesisResult(parent_id, final_output, completed, shapley_values=field(default_factory=dict), provenance_ref=None, accepted_count=0, total_count=0) and class CrewSynthesizer(*, llm_client, work_item_store, trust_network, episodic_memory, attachment_store, runtime, emit_fn=None, quorum_policy=None) (constructor injection mirroring SubtaskVerifier; quorum_policy defaults to QuorumPolicy()). async synthesize(parent_id, outcomes: list[ConvergenceOutcome]) -> SynthesisResult filters accepted = [oc for oc in outcomes if oc.verdict.accepted] then runs: (1) _synthesize_output - LLMRequest(prompt, system_prompt=_SYSTEM_PROMPT, tier="standard"), await self._llm.complete(request), reads (getattr(response,"content","") or "").strip(), and on empty/error/no-accepted honest-degrades to _concat_fallback (join of accepted outputs; "" when nothing accepted, and the LLM is NOT called in the no-accepted path); (2) _store_provenance - None when attachment_store unwired, else builds a provenance dict, sha256-keys it, and await self._attachments.write(content_hash=..., blob=..., mime="application/json", origin="crew_synth_provenance") returning the content_hash (AD-731 content-addressable ref, never inline bytes; try/except -> None); (3) _complete_parent - await self._store.transition_work_item(parent_id, "done", source="crew_synth") (validated AD-498 transition; None -> completed=False honest-degrade, no raise), then a read-merge-write of metadata["crew_synth"] = {provenance_ref, completed, accepted_count, total_count, caveat} (caveat names a partial collaboration when accepted_count < total_count); (4) _build_votes - a producer Vote per accepted outcome plus a verifier Vote via SubtaskVerifier.verdict_to_vote(oc.verdict) when oc.verdict.verifier_agent_id is non-empty; (5) _attribute - compute_shapley_values(votes, approval_threshold=self._policy.approval_threshold, use_confidence_weights=self._policy.use_confidence_weights) keyed by agent_id so a dual-role (producer+verifier) agent's two Votes merge into one share ({} on empty votes); (6) _record_trust - per agent_id self._trust.record_outcome(agent_id, success=True, intent_type="crew_collaboration", source="crew_synth") - SYNCHRONOUS, keyword-only, Tier-2 try/except; (7) _store_episode - early return when episodic unwired, else Episode(timestamp, user_input, dag_summary, outcomes, reflection=final_output, agent_ids=sorted(shapley.keys()), shapley_values=dict(shapley), source="crew_collaboration") and await self._episodic.store(episode). Emits CREW_TASK_COMPLETED (new EventType, events.py, "crew_task_completed") with the summary payload. Tests: tests/test_ad861_crew_synth.py (13 tests, REAL WorkItemStore + REAL TrustNetwork per BF-287 - no MagicMock at substrate/trust boundaries; parent is a "task" work_type driven open->in_progress->done) covering validated-transition completion, invalid-transition honest-degrade, Shapley spanning producers+verifiers, dual-role merge, trust ledger success, episode store/guard, provenance-as-ref, partial collaboration, no-accepted empty degrade, LLM-failure concat fallback, CREW_TASK_COMPLETED emission, and attachment-store-unwired -> None. AD-862 (runtime wiring + crew dispatch trigger) deferred.

### AD-862 - HXI crew-collaboration surface: thin GET /api/crew-tasks/{parent_id} (persisted-state-only) + pulse/settle panel (Wave 219)

The HXI surface unit and terminal stage of the Crew Collaboration epic - the read-only window onto an AD-859 fan-out / AD-860 verification / AD-861 synthesis tree. Architect verify-first corrections honored: (blocker) the spec's cited `_row_to_work_item` serializer does not exist - the reusable serializer is WorkItem.to_dict() (workforce.py:590, returns id/title/status/parent_id/metadata/etc), so the router reuses it rather than authoring a parallel serializer; (Fix A re-scope) the endpoint reads ONLY persisted state - there is NO live verifier-verdict feed and NO hook added to AD-860 verdict persistence (forward marker only) - the live persisted `status` drives the HXI motion, and per-subtask verdict/rounds are dereferenced from the AD-861 provenance blob ONLY once the parent is done. New module src/probos/routers/crew_tasks.py (thin router, Depends(get_runtime) from routers/deps.py, registered in BOTH the api.py import tuple AND the include loop): APIRouter(prefix="/api/crew-tasks", tags=["crew-tasks"]) with GET /{parent_id} -> 503 if no runtime.work_item_store (detail "workforce engine not enabled"); parent = await store.get_work_item(parent_id), None -> 404 (detail "crew task not found"); children = await store.list_work_items(parent_id=parent_id, limit=1000); returns {parent: parent.to_dict(), children: [child.to_dict() + verdict + rounds], count}. Provenance deref helper _provenance_by_work_item(runtime, parent) returns {} unless parent.status == "done" AND (parent.metadata or {}).get("crew_synth",{}).get("provenance_ref") is set AND runtime.attachment_store is wired; reads blob = await store.read(ref) (AD-731 content-addressable read, NOT inline bytes), json.loads, indexes prov["subtasks"] by "work_item_id"; honest-degrades to {} on missing ref / unwired store / FileNotFoundError / parse error (so verdict/rounds stay null and the HXI shows "awaiting verification", never a fabricated verdict). Per child: _verdict_from_subtask(sub) -> {accepted, confidence, critique, verifier_agent_id} mapped from the AD-861 subtask record, node["rounds"] = sub.get("rounds"); both null when no provenance entry. HXI panel ui/src/components/crew/CrewCollaborationPanel.tsx (AD-857 fetch+useState pattern, NOT Zustand): fetches GET /api/crew-tasks/{parentId}, renders parent + children; each child PULSES while status=="in_progress" (crew-subtask-pulse keyframe) and SETTLES (static, dim border) once "done" (HXI Principle #4 - motion encodes state); verdict shows accept-green/reject-red with confidence+rounds+critique when present else a neutral "awaiting verification" line; inline SVG glyphs only (SpinnerGlyph/CheckGlyph/CrossGlyph, strokeWidth 1.5, amber #f0b060 active / dim #666680, accept #60c070 / reject #d05050) - NO emoji (HXI Principle #3). Tests: tests/test_ad862_crew_tasks_api.py (6, real WorkItemStore tmp_path db per BF-287 + fake content-addressable attachment store: happy path with live status, 404 missing parent, 503 no store, in_progress parent -> null verdict/rounds even with a ref, completed parent -> verdict/rounds populated from canned provenance blob, completed-but-missing-blob -> honest-degrade null); ui/src/components/crew/CrewCollaborationPanel.test.tsx (3 vitest: pulse vs settle by status, verdict-present vs pending-null, empty payload renders null). No change to AD-860/AD-861 persistence - this AD is purely additive read surface.
