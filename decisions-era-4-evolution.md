# ProbOS — Architectural Decisions: Era IV — Evolution (Phase 30)

Archived decisions from the Evolution era. AD-330 through AD-641, BF-071 through BF-201.
...
## AD-641: Ship's Computer / Crew Integration — Brain Enhancement Phase (2026-04-17)

**Context:** ProbOS has two execution paths that evolved independently: the Intent Bus path (brain, Phases 1-21) and the Ward Room path (crew, Phases 33+). These run in parallel with almost zero integration. The brain hasn't evolved alongside the crew. Survey shows VitalsMonitor, Hebbian Router, Attention Manager, Workflow Cache, Consensus/Quorum, and Gossip Protocol have zero crew-facing exposure.

**Decision:** Research and decomposition AD. Four integration categories: (A) Shared Systems — already correct, no work; (B) Brain→Crew Observability — read-only Ward Room feeds; (C) Parallel Systems — same concept, separate implementations; (D) Brain-Only — no crew integration. Enterprise coupling model: crew loosely coupled to brain, ship runs without crew, crew function without ship. Six sub-ADs: 641a (Observability Bridge), 641b (WR Hebbian), 641c (WR Thread Priority), 641d (Crew Deliberation), 641e (JIT↔Cache Abstraction), 641f (Engineering Chief Observability).

| Design Decision | Choice | Rationale |
|---|---|---|
| DD-1: Coupling model | Enterprise (loose) | Crew observe and use brain services, never become components. Ship runs without crew. Crew transfer between ships. |
| DD-2: Shared vs parallel | Four categories (A/B/C/D) | Prevents the mistake of either over-coupling (merging everything) or over-isolating (duplicating shared infrastructure). |
| DD-3: Observability approach | Ward Room system channels | Brain publishes to channels, crew subscribe. No direct imports. Same pattern as sensor displays on the bridge. |
