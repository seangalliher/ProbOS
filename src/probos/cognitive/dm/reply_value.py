...
A DM reply has always been an untyped ``str`` doing double duty as both the
payload and the metadata channel. Any fact *about* the reply had nowhere to
live, so it was either smuggled inside the text and parsed back out, or routed
around the text through a side channel only some delivery paths carried. BF-773
("a failed tool call must reach the text the Captain reads") was built and
rejected four times against that shape.

This module carries the shape that removes the problem. The load-bearing
property is that **no component ever re-parses display text to recover a fact**
-- which deletes the ownership question rather than answering it.
