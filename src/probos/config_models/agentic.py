...
class AgenticToolsConfig(BaseModel):  # AD-1072
    ...
    * ``browser_tool.destructive_url_patterns`` is **not** a ``BrowserTool``
    ...
    AD-1190 adds the four ``delegation_tree_max_*`` ceilings. The depth and
    iteration fields above bound one delegation chain and one child; these
    bound one delegation TREE in aggregate -- a root agentic run plus every
    ``delegate_task`` descendant, siblings and nested levels alike. The root
    run's own spend is excluded (its own caps govern it), and each AD-1164
    continuation pass or AD-1155 outer iteration starts a new tree. All four
    default to ``None``, today's behaviour: no budget object exists and a
    delegated run's arguments are unchanged. An exhausted ceiling returns a
    typed ``delegation_tree_limit_reached`` result to the delegating agent
    instead of starting a child. The iteration ceiling is exact; the token
    ceiling bounds admission and each child's grant, but a grant is checked
    only after an LLM call returns, so a tree can exceed it by up to one call
    per delegated run in flight. A run that stops uncleanly is charged at
    least its whole token grant. Charged spend can also trail real spend by up
    to one call per delegated run: a run that reported usage and then ended on
    an unmeasured empty completion is charged nothing for that final call, and
    a failed call can cost more than an unclean charge covers. Tokens are those
    each delegated run's own loop counts; model calls made inside a tool's
    implementation are outside the tree budget. No validator relates these to
    each other or to the depth and iteration fields."""
    ...
    # AD-1190: aggregate ceilings for one delegation tree; None means no ceiling.
    delegation_tree_max_tokens: int | None = Field(
        default=None,
        ge=1024,
        description=(
            "AD-1190: aggregate token ceiling for one delegation tree -- a root "
            "agentic run plus every delegate_task descendant, excluding the "
            "root run's own spend; each AD-1164 continuation pass or AD-1155 "
            "outer iteration starts a new tree. None (the default) means no "
            "ceiling, which is today's behaviour. Each delegated child is "
            "admitted with a token grant carved from what remains and runs "
            "with that grant as its token_budget; when fewer than 1024 tokens "
            "remain, the delegation returns a typed delegation_tree_limit_reached "
            "result and no child starts. The loop checks a grant only after an "
            "LLM call returns, so a child can overshoot its grant by one call's "
            "usage, and tree spend can exceed this ceiling by up to one LLM "
            "call per delegated run in flight when it runs out. Counts are the "
            "tokens each delegated run's own loop counts: provider-reported "
            "non-negative figures, or BF-680 estimates when the provider "
            "reports none or a negative one; model calls made inside a tool's "
            "implementation are outside this ceiling. A child whose total is 0 "
            "is charged its whole grant, because that zero went unmeasured, "
            "and a child whose run stops uncleanly -- any stop other than "
            "complete, max_iterations or token_budget, including the loop's "
            "error stop and a run that raised or was cancelled -- is charged "
            "the larger of its counted total and its whole grant, because a "
            "failed call may have been billed without being counted. Real "
            "spend can still exceed charged spend by up to one call per "
            "delegated run, in addition to that overshoot: an unmeasured empty "
            "completion (no text or tool call, reported as 0 or negative) that "
            "ends a run which already reported usage is charged nothing, and a "
            "failed call can cost more than an unclean charge covers. At "
            "delegation_max_depth >= 2 a child at depth d (1 for the "
            "root's own delegate) is granted 1/(delegation_max_depth - d + 1) "
            "of the remainder, never less than 1024 tokens, which leaves room "
            "for its own descendants; a child at the deepest level is granted "
            "all of it."
        ),
    )
    delegation_tree_max_iterations: int | None = Field(
        default=None,
        ge=1,
        le=2500,
        description=(
            "AD-1190: aggregate agentic-loop iteration ceiling for one "
            "delegation tree -- a root agentic run plus every delegate_task "
            "descendant, excluding the root run's own iterations; each AD-1164 "
            "continuation pass or AD-1155 outer iteration starts a new tree. "
            "None (the default) means no ceiling, which is today's behaviour. "
            "Each child runs with max_iterations set to the smaller of "
            "delegation_max_iterations and its grant from what remains, so "
            "this bound is exact: the tree's delegated runs never exceed it. "
            "With nothing left, the delegation returns a typed "
            "delegation_tree_limit_reached result and no child starts. At "
            "delegation_max_depth >= 2 a child at depth d (1 for the root's "
            "own delegate) is granted 1/(delegation_max_depth - d + 1) of the "
            "remainder, at least one iteration, which leaves room for its own "
            "descendants; a child at the deepest level is granted all of it, "
            "still capped by delegation_max_iterations."
        ),
    )
    delegation_tree_max_concurrent: int | None = Field(
        default=None,
        ge=1,
        le=16,
        description=(
            "AD-1190: ceiling on delegated runs in flight at once within one "
            "delegation tree -- a root agentic run plus every delegate_task "
            "descendant; each AD-1164 continuation pass or AD-1155 outer "
            "iteration starts a new tree. A delegated run that is awaiting its "
            "own delegate still counts, so a nested chain counts every "
            "delegated level. "
            "None (the default) means no ceiling, which is today's behaviour. "
            "A delegation that would exceed it returns a typed "
            "delegation_tree_limit_reached result and no child starts; it is "
            "never queued. delegate_task calls within one loop run one at a "
            "time, so today runs are in flight together only through nesting, "
            "at most delegation_max_depth of them: a value >= "
            "delegation_max_depth is inert."
        ),
    )
    delegation_tree_max_children: int | None = Field(
        default=None,
        ge=1,
        le=100,
        description=(
            "AD-1190: ceiling on delegated children admitted across one "
            "delegation tree -- siblings, later iterations of the same root "
            "run and nested descendants all count, the root run itself does "
            "not; each AD-1164 continuation pass or AD-1155 outer iteration "
            "starts a new tree. None (the default) means no ceiling, which is "
            "today's behaviour. Once it is reached, a further delegation "
            "returns a typed delegation_tree_limit_reached result and no "
            "child starts."
        ),
    )
