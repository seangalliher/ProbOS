...
class _FakeThreadStore:
    ...
    # AD-1274: the promoted-report path posts through ``append_message_once``,
    # so a retry replays one minted id rather than inserting a second copy.
    # Mirrored here because a double offering only the older method is LESS
    # capable than production: the report silently stops reaching the thread
    # while this test still reads as a clean artifact check.
    def append_message_once(
        self, thread_id: str, *, message_id: str, author_id: str, role: str,
        body: str, created_at: float | None = None, metadata: dict | None = None,
    ) -> object:
