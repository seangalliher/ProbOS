...
def test_record_verification_outcome_ignores_an_empty_verifier_id():
    verifier, trust = _make([])

    verifier.record_verification_outcome("", "producer", refusal_was_upheld=True)

    assert trust.get_record("") is None


def test_record_verification_outcome_writes_the_full_causal_envelope():
    """A positive outcome must reach the ledger with the right causal fields.

    Every other test here asserts ABSENCE, so a mutation to the target agent,
    the causal ``verifier_id``, or the ``intent_type`` would survive all of
    them. This is the one test that pins what a real write looks like.
    """
    recorded: list[tuple[Any, dict[str, Any]]] = []
    trust = TrustNetwork()
    real = trust.record_outcome
    trust.record_outcome = lambda *a, **kw: (recorded.append((a, kw)), real(*a, **kw))[1]

    verifier, _t = _make([], trust=trust)
    verifier.record_verification_outcome(
        "the-verifier", "the-producer", refusal_was_upheld=True
    )

    assert len(recorded) == 1
    args, kwargs = recorded[0]
    # The SUBJECT is the verifier; the producer is only causal context.
    assert args[0] == "the-verifier"
    assert kwargs["success"] is True
    assert kwargs["verifier_id"] == "the-producer"
    assert kwargs["intent_type"] == "crew_verification"
    # And it actually moved the ledger in the positive direction.
    record = trust.get_record("the-verifier")
    assert record is not None
    assert record.alpha > 2
    assert trust.get_score("the-verifier") > 0.5


def test_record_verification_outcome_honours_a_false_outcome():
    """The parameter must actually drive the signal, not be decorative.

    Nothing calls this method yet, so no convergence test would catch it
    hardcoding a direction. A public API that takes a bool and ignores it is the
    shape BF-778 was made of.
    """
    verifier, trust = _make([])

    verifier.record_verification_outcome(
        "verifier", "producer", refusal_was_upheld=False
    )
...
def test_record_verification_outcome_validates_the_bool_before_the_id_no_op():
    """The type guard must not be bypassable by an unrelated argument.

    It originally sat AFTER the empty-verifier-id early return, so
    ``("", "producer", refusal_was_upheld="false")`` returned cleanly and the
    guard's advertised unconditional behaviour was false.
    """
    verifier, trust = _make([])

    with pytest.raises(TypeError, match="must be a bool"):
        verifier.record_verification_outcome(
            "", "producer", refusal_was_upheld="false"
        )
...
def test_record_verification_outcome_rejects_a_non_bool():
    """The new public API must not repeat the defect it was written to fix.

    ``TrustNetwork`` branches on truthiness, so ``"false"`` here would record a
    SUCCESS -- BF-777 one layer down.
    """
    verifier, trust = _make([])
