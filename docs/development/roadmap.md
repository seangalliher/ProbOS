...
### LLM Rate Governance (AD-617) *(scoped, OSS)*

**AD-617: LLM Rate Governance — Token Bucket Rate Limiter, HTTP 429 Backoff, Per-Agent Budget, Cache Eviction** *(scoped, OSS, depends: AD-576 LLM Unavailability Awareness)* — No system-wide LLM call rate limiter exists. During the 8,448-DM flood, the LLM proxy received up to 101K requests in 90 minutes — HTTP 500 errors were the only backpressure mechanism. Four fixes: (1) **Token bucket rate limiter**: Per-tier RPM limits (fast/standard/deep) on `OpenAICompatibleClient`. Default: fast=60, standard=30, deep=15 RPM. Requests exceeding budget queue with configurable max wait. (2) **HTTP 429 backoff**: Currently 429 is caught by generic `HTTPStatusError` — logged but no backoff. Specific 429 handling with exponential retry (1s/2s/4s/8s) + `Retry-After` header respect. (3) **Per-agent token budget**: Configurable hourly cap with log-and-degrade semantics — budget-exhausted agents skip proactive thinks but still respond to Captain DMs. Tracked via existing cognitive journal token accounting. (4) **LLM cache eviction**: `_cache` dict grows unbounded — add LRU eviction at 500 entries. Config: `LLMRateConfig` with `rpm_fast`, `rpm_standard`, `rpm_deep`, `per_agent_hourly_token_cap`, `cache_max_entries`. 2 source files (llm_client.py, config.py), ~15 tests.
