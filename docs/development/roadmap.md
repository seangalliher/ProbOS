...
*Connects to: AD-551 (dream consolidation identifies convergence retrospectively, AD-554 identifies it in real-time), AD-434 (Ship's Records — report storage), Ward Room (Bridge notification), AD-524 (Archive — convergence reports are high-value archive material), AD-562 (Knowledge Browser — convergence clusters visualized in graph view). Triggered by: iatrogenic trust detection — three agents independently reaching the same conclusion, discovered only by manual review of 419 files.*

*Future enhancement: Divergence detection — the inverse of convergence. When agents reach opposing conclusions on the same topic through different analytical lenses, this is potentially more actionable than agreement. Disagreement surfaces where the crew's understanding is incomplete or domain-specific nuances haven't been reconciled. Convergence validates; divergence identifies knowledge frontiers. (Insight absorbed from Karpathy LLM Knowledge Base architecture analysis, 2026-04-03.)*
...
*Connects to: AD-550 (dedup generates novel content signals), AD-551 (consolidation generates consolidation metrics), AD-552 (self-repetition alerts feed quality metrics), AD-523c (Ship's Records Dashboard — notebook quality shown alongside records browser), AD-562 (Knowledge Browser — quality metrics as graph node overlays), VitalsMonitor (notebook health as part of ship health). Triggered by: the 16% signal-to-noise ratio discovered in the fleet-wide notebook analysis.*

*Future enhancement: Knowledge linting beyond noise reduction. Current pipeline (AD-550–555) focuses on dedup, consolidation, quality scoring — eliminating redundancy. A broader "knowledge linting" pass should also: (1) detect knowledge inconsistencies (agents with contradicting conclusions on the same topic — distinct from convergence/divergence, this is factual disagreement within the knowledge base), (2) identify coverage gaps (topics the crew should be monitoring but aren't — knowledge-level gap detection, complementing Step 8's procedure-level gap detection), (3) suggest missing cross-references (entries that discuss related topics but don't link to each other). This transforms the quality pipeline from noise reduction to active knowledge enrichment. (Insight absorbed from Karpathy LLM Knowledge Base "linting" function, 2026-04-03.)*
...
**Status:** AD-550 COMPLETE, AD-551 COMPLETE, AD-552 COMPLETE, AD-553 COMPLETE. AD-554–555 PLANNED.
...
AD-541 (Memory Integrity)     ←  verification + social provenance + hierarchy (COMPLETE — MVP)
    ...
    ├── AD-541b: Reconsolidation guard  ←  depends: dreaming.py LLM processing (future)
    ├── AD-541c: Active retrieval (SRT) ←  depends: AD-503 (Counselor Activation)
    └── AD-541d: Counselor reminiscence ←  depends: AD-503, AD-505
```

**AD-541b: Reconsolidation Protection — Read-Only Memory Framing** *(planned, OSS, depends: dream LLM processing)* — When episodes are sent through an LLM during dream cycles (strategy extraction, Cognitive JIT procedure extraction), wrap in `=== READ-ONLY EPISODE ===` boundary. Episode in ChromaDB never modified; derived insights stored as new artifacts referencing episode IDs. Biological analog: synaptic tagging — important memories get consolidation that resists modification.

**AD-541c: Spaced Retrieval Therapy — Active Recall Practice** *(planned, OSS, depends: AD-503)* — During dream cycles, prompt agents to actively RECALL episode details (not just passively replay). Spaced repetition schedule strengthens genuine memory traces. Based on Camp (1989) spaced retrieval therapy. Requires Counselor data gathering (AD-503) for scheduling and assessment.

**AD-541d: Counselor Guided Reminiscence — Therapeutic Memory Sessions** *(planned, OSS, depends: AD-503, AD-505)* — Counselor conducts structured 1:1 reminiscence sessions with agents showing memory integrity concerns. Guided recall + validation therapy (Feil, 1993). Requires Counselor therapeutic intervention capability (AD-505) and data gathering (AD-503). Feeds `memory_integrity_score` and `confabulation_rate` into CognitiveProfile.

**Minimum viable slice:** (1) Episode verification + (3) Social provenance + (6) Memory hierarchy standing order — **COMPLETE (AD-541 MVP).** AD-541b is cheap (prompt framing, no dependency). AD-541c/d are enrichments requiring AD-503/505.
