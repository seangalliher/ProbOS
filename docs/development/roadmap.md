...
| AD-927 | Outputs folder — mount the existing `ArtifactDrawer` (AD-797, thread-keyed + versioned) on the task room as the **Output** pane; agents write artifacts via a `[ARTIFACT name="…"]…[/ARTIFACT]` proactive action tag (the AD-924 `[GROUP_CHAT]` extractor pattern), so produced documents/images/slides land in the room. | Task workspace | 2 |
| AD-928 | Show-your-work status protocol *(deferred — Captain review)* — a `federation.md` standing order + a structured progress/final-result message convention tied to the work-item lifecycle (`in_progress → done`), so the crew narrate their work in the room and post a clear final result. | Task workspace | 3 |
| AD-929 | Unified workspace HXI view *(deferred — Captain review)* — the Cowork-style three-pane surface for a task room: transcript (center) + Inputs (left) + Outputs/Artifacts (right), reusing the AD-926/927 panes + the AD-917 chat. | Task workspace | 3 |
