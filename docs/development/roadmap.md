...
**AD-514: Spatial Knowledge Explorer — Digital Twin & 3D Ontology Visualization** *(planned, OSS + Commercial, depends: AD-429, AD-513)* — The ship ontology defines a topology — departments, posts, chain of command, trust networks, records — but it's only visible as text and 2D tables. The Spatial Knowledge Explorer renders this topology as an interactive 3D environment in the HXI, progressing through three visualization tiers toward a full digital twin of the ship.
