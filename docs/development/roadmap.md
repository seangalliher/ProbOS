...
Build order: Standing Orders ✅ → AD-566a ✅ → AD-566b ✅ → AD-566c ✅ → AD-566d ✅ → AD-566e ✅ → AD-566f (/qualify command) ✅ → AD-567a ✅ → AD-567b ✅ (absorbs AD-462a) → 567c ✅ → 567d ✅ → 567f ✅ (absorbs AD-462d) → AD-567g ✅ → AD-566 re-run (measure impact of memory anchoring wave) → AD-569 ✅ (behavioral metrics) → AD-462c/d/e ✅ (memory architecture) → AD-568a/b/c ✅ (adaptive source governance). Independent: AD-566g (Qualification → Skill Bridge, needs AD-423), AD-566h/i/j. Deferred: AD-568d/e (need runtime data from 568a/b/c).

### Adaptive Source Governance (AD-568) *(568a/b/c complete, 568d/e deferred)*
...
> - **AD-568d: Source Monitoring Skill** — Agent metacognitive capability to distinguish "I know this from experience" vs "I know this from training." Reframed as **cognitive proprioception** (Sacks, 1985 — "The Man Who Mistook His Wife for a Hat"): proprioception is pre-conscious body-position awareness; patients who lose it must visually check where their limbs are — functional but exhausting. AD-568d's source monitoring should be an always-on sense, not an explicit check the agent invokes. The distinction: metacognitive *skill* (conscious, invoked) vs metacognitive *sense* (ambient, automatic). Implementation should aim for the latter — source attribution embedded in the retrieval pipeline itself, not a post-hoc verification step. Connects to cognitive skill training lineage.
...
**AD-577: Temporal Sequence Precision — Intra-Cycle Event Ordering for High-Activity Periods** *(planned, OSS, depends: AD-567a, AD-573)* — During high-activity Ward Room periods, the proactive cycle batches multiple posts into a single processing pass. All posts consumed in one cycle share the same episode timestamp, losing their relative ordering. Echo (Counselor) identified the clinical impact: "The temporal anchoring during high-activity periods sometimes gets a bit fuzzy... it makes my clinical assessments slightly less precise."

**Root cause:** The proactive cycle's `_gather_context()` fetches Ward Room activity as a batch. When the cycle processes multiple posts (e.g., 5 agents posting in rapid succession), the episode created from that cycle captures all as simultaneous context. The AnchorFrame (AD-567a) has a `timestamp` field but no sub-timestamp ordering within a batch. Working memory entries share the source pathway timestamp of the cycle, not the individual post timestamps.

**Two capabilities:**

**(1) Intra-cycle sequence numbering:** Add a `sequence_index` field to AnchorFrame metadata — a monotonically increasing counter within a single proactive cycle. When multiple Ward Room posts are processed in one batch, each gets a distinct sequence number preserving their original posting order. Ward Room posts already have creation timestamps in the database; this surfaces them into the episode anchor metadata so recall can reconstruct the exact sequence.

**(2) Source timestamp preservation:** When creating episodes from Ward Room activity, use the individual post's `created_at` timestamp rather than the cycle's processing timestamp. The proactive loop creates episodes with `time.time()` at processing time; Ward Room posts have their own creation timestamps that may be minutes or hours earlier. Preserving the source timestamp in the AnchorFrame gives recall access to the actual event time, not just when the agent processed it.

**Scope:** Small — AnchorFrame metadata enrichment + episode creation timestamp sourcing. No new files; modifications to proactive.py (Ward Room context processing) and possibly types.py (AnchorFrame field). Connects to: AD-567a (anchor infrastructure), AD-570 (anchor-indexed recall — benefits from richer temporal metadata), AD-568b (adaptive budget — temporal precision improves confidence scoring).
...
| BF-124 | **Emergence/cooperation cluster detectors fire false positives after stasis recovery.** Crew-identified (Lynx, Science): a 23-second stasis event triggered a 5x cooperation cluster cascade (1,667% spike, validated by Kira). Detectors interpret agent re-entry as abnormal cooperation clustering — conflating operational state changes (network topology shifts, trust recalibration, communication resumption) with genuine behavioral emergence. Produces false positive `emergent_cooperation_cluster` and `consolidation_anomaly` events. Also triggers repetitive cross-department divergence alerts (7 identical Forge×Lynx alerts in 3 hours on same topic). **Two fixes needed:** (1) Post-stasis cooldown window in emergence detection to filter lifecycle-driven topology changes from genuine cooperation signals. (2) Divergence alert dedup/suppression — same agent pair + same topic should not re-alert until content materially changes. | Medium | Open |
