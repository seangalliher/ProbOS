...
|----|-----------|---------|----------|
| BF-079 | #17 | Mock discipline audit — Phase 1 done (18 factories spec'd). Phase 2/3 blocked on ProbOSRuntime annotations (→BF-085) | Medium | **Phase 1 Closed** |
| BF-085 | #11 | Type safety audit — replace ~70 remaining `Any` annotations + add ProbOSRuntime class-level annotations (unblocks BF-079 Phase 2/3) | Medium |
...
| BF-079 | **Test mock factories use unspec'd MagicMock — systematic refactoring blind spot.** Phase 1 COMPLETE: 18 factory functions spec'd across 14 test files (BaseAgent, WardRoomChannel, CodebaseIndex, TrustNetwork, HebbianRouter, EpisodicMemory, EventLog, AgentSpawner, ResourcePool). Found hidden bug: missing `.confidence` attribute on agent mocks in test_proactive.py. Compliance: 3.5% → 11.8%. **Phase 2/3 blocked:** `spec=ProbOSRuntime` doesn't work — all service attributes (registry, trust_network, ward_room, etc.) are instance-only, not class-level. `MagicMock(spec=ProbOSRuntime)` blocks access to every attribute. **Fix path:** Add class-level type annotations to ProbOSRuntime (folded into BF-085). | Medium | **Phase 1 Closed** |
