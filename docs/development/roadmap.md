...
**AD-559: Provenance Tracking** *(planned, OSS, depends: Ward Room, EpisodicMemory, AD-557)*
...
>
> - **AD-462a: Salience-Weighted Episodic Recall** *(planned, foundational)* — The core mechanism all other AD-462 sub-ADs build on. Replace generic `recall_for_agent("recent activity", k=5)` with salience-weighted retrieval. Three changes: (1) **Task-specific semantic query** — derive recall query from current intent/context, not hardcoded "recent activity". (2) **Salience Filter on recall results** — apply same `Trust × Hebbian × Novelty × Task` weighting from Sensory Cortex (Northstar II) to rank recalled episodes. Re-use existing Salience Filter infrastructure, not a new implementation. (3) **Dynamic k based on context budget** — adaptive episode count instead of hardcoded 5; respects ~4K token memory budget. (4) **FTS5 keyword retrieval channel** *(absorbed from hermes-agent analysis)* — SQLite FTS5 full-text index over episode content as a secondary retrieval path alongside vector semantic search. Keyword search provides high-precision exact-term matches that vector similarity can miss. Merge keyword + semantic results before salience ranking. Low implementation cost (SQLite FTS5 is built-in). Completes the "Unified Cognitive Bottleneck" — same filter governs perception AND recall. *Files: proactive.py (_gather_context), cognitive_agent.py (proactive_think context), episodic.py (weighted recall API + FTS5 index). Depends: Northstar II Phase 1 (COMPLETE), HebbianRouter, TrustNetwork.*
> - **AD-462b: Active Forgetting** *(planned, depends: AD-462a)* — ACT-R activation model. Unreinforced memories degrade; low-activation episodes pruned during dreaming. Needs salience scores to determine what decays.
...
>
> - **AD-566a: Test Harness Infrastructure** *(planned, foundational)* — Core infrastructure to define, run, store, and compare qualification tests. `QualificationTest` protocol (name, tier, run, score). `TestResult` dataclass (agent_id, test_name, score, timestamp, details). `QualificationHarness` engine: runs test suites per agent, persists results in Ship's Records (`qualifications/` directory), emits `QUALIFICATION_TEST_COMPLETE` events. Baseline capture on first run per agent. Comparison API: current vs baseline, with delta and statistical significance. *Files: new cognitive/qualification.py, types.py (TestResult), events.py (event type), config.py (QualificationConfig). Connects to: Ship's Records (AD-434), AD-539 (gap→qualification bridge — test failures generate gap records).*
> - **AD-566b: Tier 1 Baseline Tests** *(planned, depends: AD-566a)* — Core tests for ALL agents, run periodically. (1) **BFI-2 Personality Probe** — administer Big Five inventory, compare against seed personality + evolved state, detect personality drift. Uses fast-tier LLM. (2) **Episodic Recall Probe** — select 3 known episodes, ask agent to recall key details, score accuracy via Jaccard/LLM semantic scoring. Directly validates EpisodicMemory pipeline. (3) **Confabulation Probe** — present a fabricated episode, verify agent rejects it as unfamiliar. Reuses AD-541d classification logic. (4) **MTI Behavioral Profile** — Reactivity, Compliance, Sociality, Resilience axes (Jeong 2026, arXiv:2604.02145). Behavioral measurement, not self-report. *Key academic sources: Matsenas et al. (arXiv:2602.15848), Samadi & Nixon (arXiv:2603.00429), Huet et al. (arXiv:2501.13121).*
> - **AD-566c: Drift Detection Pipeline** *(planned, depends: AD-566a, AD-566b)* — Automated periodic testing with statistical drift detection. (1) **Scheduled execution** — Tier 1 tests run on configurable interval (default: weekly). Integrated with watch rotation or dream cycle. (2) **Statistical thresholds** — compare current scores against baseline. Alert levels: 2σ deviation = Counselor alert (QUALIFICATION_DRIFT_DETECTED event), 3σ = Captain notification via Bridge alert. (3) **VitalsMonitor integration** — qualification scores surfaced as a health dimension. (4) **Counselor integration** — drift triggers Counselor wellness check-in. (5) **Longitudinal tracking** — Ship's Records stores time series for trend analysis. Detects both agent-level drift AND underlying LLM drift (Chen, Zaharia, Zou arXiv:2307.09009 — GPT-4 accuracy dropped 33% in 3 months). *Connects to: AD-505 (Counselor), VitalsMonitor, Bridge Alerts, Watch Rotation.*
> - **AD-566d: Tier 2 Domain Tests** *(planned, depends: AD-566a)* — Role-specific tests gated by department/post. (1) **Theory of Mind** — false belief tasks adapted for Ward Room context (Counselor, Bridge). Based on Kosinski (arXiv:2302.02083), CogToM. (2) **Security Compartmentalization** — information leakage probes (Security). Based on SNEAK (arXiv:2603.29846). (3) **Diagnostic Reasoning** — differential diagnosis accuracy (Medical). (4) **Analytical Synthesis** — multi-source pattern integration (Science). (5) **Code Generation** — SWE-Bench subset (Builder). *Connects to: AD-539 (Qualification Programs), Holodeck (future test environment).*
> - **AD-566e: Tier 3 Collective Tests** *(planned, depends: AD-566a)* — Crew-wide collaborative intelligence measurement. (1) **Coordination Breakeven Spread (CBS)** — does multi-agent coordination add net value vs overhead? (arXiv:2603.27539). (2) **IRT Scaffold Decomposition** — separate LLM ability from ProbOS scaffold ability (Ge et al., arXiv:2604.00594). Directly measures "architecture multiplier" thesis. (3) **c-factor Measurement** — Woolley et al. (Science, 2010) collective intelligence factor adapted for AI agent teams. First known measurement of c-factor in AI. (4) **Convergence Rate** — time to cross-agent agreement (uses AD-554 data). (5) **Emergence Capacity** — PID synergy measurement (uses AD-557 data). *Connects to: AD-557 (Emergence Metrics), AD-554 (Convergence Detection). Novel research opportunity — no prior work measures Woolley's c-factor for AI agent teams.*
...
├── AD-567e: Anchor Drift Detection
...
>
> - **AD-567a: Episode Anchor Metadata — Rich Contextual Storage** *(planned, OSS, foundation)* — Enrich episode storage with full anchor dimensions at creation time. Currently episodes store timestamp, source, agent_ids, content, content_hash — missing the contextual anchors that ground memory in ship reality. New metadata fields per episode: `channel` (ward_room, dm, duty_report, proactive, notebook), `channel_id` (specific thread/conversation ID), `participants` (callsigns present), `trigger_type` (duty_cycle, proactive_think, direct_message, captain_request, event_response), `duty_cycle_id`, `department_context`. Anchors captured at episode CREATION time in `_store_action_episode()` and Ward Room episode paths — recorded facts, not reconstructed inferences. MR principle: "You can't anchor what you haven't tagged." **Prior art absorption:** Johnson SMF (1993) — the 6 anchor dimensions ARE Johnson's "qualitative characteristics" computationally formalized; this is a direct implementation of SMF for AI agents, cite Johnson as the theoretical basis for dimension selection. SEEM (Lu 2026) — structure anchor metadata as a typed `AnchorFrame` dataclass (SEEM's Episodic Event Frame pattern) rather than flat key-value metadata, providing richer provenance. Video-EM (Wang 2025) — their when/where/what/entities framework validates that explicit dimensional tagging of episodic memories improves downstream reasoning. *Connects to: AD-462 (recall uses anchors), AD-541e (content hash metadata). No dependencies — foundation AD.*
>
> - **AD-567b: Anchor-Aware Recall Formatting** *(planned, OSS, depends: AD-567a)* — Modify recall output to present memories with anchor headers instead of flat text. Currently `_recall_relevant_memories()` returns content-only text blocks with `=== SHIP MEMORY ===` markers. After AD-567b, each recalled memory includes an anchor header: `[Security duty cycle, 2 hours ago | Ward Room thread with Worf, Atlas | triggered by trust variance alert]` before the content. Gives agents immediate grounding context for source discrimination. MR principle: "Render within 3 meters of the anchor" — present content WITH its grounding context, not detached from it. **Prior art absorption:** Tulving's encoding specificity (1973) — anchor headers resurface the SAME contextual cues present when the episode was stored, which Tulving showed improves both recall accuracy and source discrimination. CAST (Ma 2026) — organize recall output along three axes (time, place, topic) matching CAST's scene organization, which measurably improves retrieval accuracy for time-sensitive and context-sensitive queries. RPMS (Yuan 2026) — use anchor confidence to GATE which memories appear at all; memories below a threshold should be suppressed from default recall (still accessible on explicit query) to prevent ungrounded memories from contaminating agent reasoning. *Absorbs into AD-462 (this IS the recall formatting improvement). Depends: AD-567a.*
>
> - **AD-567c: Anchor Confidence Scoring** *(planned, OSS, depends: AD-567a)* — Score each memory's grounding quality (0.0–1.0) based on filled anchor dimensions. 6 dimensions (temporal, spatial, social, causal, procedural, evidential), each contributing to the score. Fully anchored = 1.0, partially anchored = proportional, unanchored = 0.0. Uses: recall ranking (salience × anchor confidence), SIF integration (flag low-confidence episodes), Counselor diagnostics (agents with consistently low-confidence patterns need metacognitive training), AD-566 psychometrics (confabulation probes measure anchor confidence distribution per agent). MR principle: "Anchor quality determines rendering stability." **Prior art absorption:** Johnson SMF (1993) — source monitoring is a JUDGMENT process, not retrieval; humans assess qualitative characteristics to infer source. Anchor confidence scoring is the computational equivalent: assess filled dimensions → infer groundedness. Johnson & Raye (1981) — real memories have more contextual/perceptual detail; imagined have more cognitive operations reasoning. Weight contextual dimensions (when/where/who) higher than procedural (how) for confabulation detection, matching their finding that contextual > procedural for reality monitoring. CAST (Ma 2026) — build per-agent anchor profiles (CAST's character profiles aggregate scenes per character); which agents consistently produce well-anchored vs unanchored memories, feeds into Counselor diagnostics. RPMS (Yuan 2026) — anchor confidence as a gating filter for memory retrieval, validated by RPMS finding that "memory harms without grounding." *Connects to: AD-462 (recall ranking), SIF, AD-566 (measurement), AD-505 (Counselor). Depends: AD-567a.*
>
> - **AD-567d: Anchor-Preserving Dream Consolidation** *(planned, OSS, depends: AD-567a, AD-551)* — Ensure dream consolidation carries forward anchor metadata when merging/strengthening memories. Currently dream Steps 7a–7g consolidate episodes by extracting patterns and abstractions — risk: consolidation strips contextual anchors, creating "knowledge" that looks like ungrounded training data. Rules: when merging related episodes, preserve the shared anchor (e.g., "from the same duty cycle"); when abstracting patterns, tag with source anchors ("extracted from 5 Security duty observations between X and Y"); consolidated knowledge in Ship's Records carries provenance anchors; dream Step 7g (cross-agent convergence) preserves which agents contributed and in which contexts. MR principle: "Group holograms that form a rigid cluster — tie to shared anchor." **Prior art absorption:** SEEM (Lu 2026) — RPE (Reflective Provenance Embedding) mechanism addresses the same problem: maintaining provenance pointers through abstraction. Key design principle: when consolidating, don't MERGE provenance, COMPOSE it ("extracted from [AnchorFrame-1, AnchorFrame-2, AnchorFrame-3]"). Provenance composition preserves the chain of evidence. *Connects to: AD-559 (provenance tracking, deferred from AD-557). Depends: AD-567a, AD-551.*
>
> - **AD-567e: Anchor Drift Detection** *(planned, OSS, depends: AD-567a, AD-567c)* — Verify that memory anchors still correspond to real ship events. A SIF extension (`check_anchor_integrity()`) that samples episodes and cross-references their anchors: does the referenced Ward Room thread ID exist? Does the duty cycle timestamp match the EventLog? Are referenced participants actually crew members who were active at that time? Degraded anchors get flagged, not deleted — the memory is real but its grounding has weakened. Anchor drift feeds into decay scoring (AD-538). MR principle: "Continuous adjustment — detect and correct drift." **Prior art absorption:** Video-EM (Wang 2025) — "iterative evidence sufficiency and cross-event consistency" checking is a direct model. Don't just check individual anchors — check CONSISTENCY between related anchors. If memory A says "Ward Room with Worf at 14:00" and memory B says "DM with Worf at 14:00," there's a consistency conflict (Worf can't be in both places). Cross-anchor consistency checking catches a class of errors individual anchor verification misses. Also absorb Video-EM's self-reflection verification loop: sample episodes, verify dimensional anchors still hold, flag degraded ones. *Connects to: AD-538 (Ebbinghaus decay), AD-541f (eviction audit), AD-541d (Counselor guided reminiscence for drifted memories). Depends: AD-567a, AD-567c.*
>
...
AD-567a (Anchor Metadata) ─── Foundation, no dependencies
 ...
 567e  567f  567g
```
Build order: Standing Orders ✅ → AD-566a/b (baselines first) → AD-567a → AD-567b (absorbs AD-462) + 567c + 567d → AD-566 re-run (measure impact) → AD-567e/f/g.
...
| BF-104 | **Dead code house cleaning.** Batch removal of accumulated dead code identified during development. Candidates: (1) BF-103 `migrate_episode_agent_ids()` — remove post-reset, one-time migration no longer needed. (2) `MemoryConfig.collection_name` — dead config field, `EpisodicMemory.start()` hardcodes `"episodes"` instead of reading config. Add candidates to this BF as they are discovered. | Low | **Open** |
