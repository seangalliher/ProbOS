...
### Metacognitive Architecture Awareness (AD-587 / AD-588 / AD-589) *(planned, OSS)*
...
> **AD-587: Cognitive Architecture Manifest** *(ready, OSS, extends: AD-567g Orientation, Westworld Principle)* — `CognitiveArchitectureManifest` frozen dataclass in `orientation.py` (not separate module) containing verifiable mechanistic facts about the agent's cognitive architecture. Five domains: memory (ChromaDB, cosine similarity, no offline processing), trust (Bayesian beta, 0.05–0.95, outcome-based), stasis (all processing stops, dreams run at startup not during), cognition (discrete LLM inference, no emotional subsystem), self-regulation (graduated zones, metric-based not emotional). Integrated into `OrientationService`: full manifest in cold_start, abbreviated reminder in warm_boot, one-line note in proactive supplement. ~23 tests. Build prompt: `prompts/ad-587-cognitive-architecture-manifest.md`. Issue #151.
>
...
### Confabulation Scaling Mitigation (AD-590 / AD-591 / AD-592 / AD-593) *(planned, OSS)*
...
> - **AD-593: Pruning Acceleration + Similarity Floor Tightening** *(planned, OSS)* — (A) Raise `max_prune_fraction` from 0.10 to 0.20 for episodes >48h, add aggressive tier (>7d, score <0.0, 30% fraction), add episode-count pressure scaling above 5000. (B) Raise `agent_recall_threshold` from 0.15 to 0.25. Location: `dreaming.py:955-969`, `episodic.py:467`, `config.py`. ~15-20 tests.
