...
### Importance Scoring at Encoding (AD-598) *(planned, OSS)*

**AD-598: Importance Scoring at Encoding** *(planned, OSS, depends: AD-538 Episode Lifecycle, AD-567a Anchor System)* — Assign a 1–10 importance score to episodes at creation time. High-importance episodes resist Ebbinghaus activation decay (AD-538) and pruning (AD-593 two-tier dream Step 12). Inspired by Park et al. (2023) Generative Agents architecture where importance scoring at encoding enables selective retention.

**Problem statement:** All episodes are born equal in ProbOS — a routine status update and a critical trust violation get the same initial activation weight. AD-538's Ebbinghaus decay and AD-593's pruning treat all episodes identically by age and activation frequency. This means high-signal moments (first contact, trust breaches, key discoveries) decay at the same rate as routine observations.

**Design:**
- `importance: int` field (1–10) on `Episode` dataclass, default 5 (neutral).
- **Rule-based scoring** (no LLM call at encoding): event type mapping (TRUST_UPDATE→7, CIRCUIT_BREAKER_TRIP→9, DREAM_COMPLETE→6, GAME_COMPLETED→4, routine observation→3, DM from Captain→8). Configurable via `ImportanceScoringConfig`.
- **Decay resistance:** AD-538 `compute_activation()` multiplied by `importance / 5.0` — importance 10 decays at 2x slower rate, importance 1 at 0.2x.
- **Pruning resistance:** AD-593 dream Step 12 threshold adjusted by `importance * 0.1` — high-importance episodes need lower activation to survive pruning.
- ChromaDB metadata field for importance-aware recall filtering.
