...
**AD-413: Fine-Grained Reset Scope** — Roadmap. `probos reset` currently wipes KnowledgeStore subdirs (episodes, agents, skills, trust, routing, workflows, qa), ChromaDB, and Hebbian weights DB — but does not touch Ward Room (`ward_room.db`), PersistentTaskStore (`scheduled_tasks.db`), Assignments (`assignments.db`), or DAG checkpoints. This creates a **cognitive coherence problem**: after a reset, agents have no episodic memory but the Ward Room still contains posts they authored pre-reset. An agent encountering its own forgotten posts is system-level cognitive dissonance. **Fix:** (1) Record a `last_reset_at` epoch timestamp (persisted in `data_dir/reset_epoch.txt` or similar). (2) Ward Room queries for agents filter to post-epoch by default — pre-epoch posts are "ship's log" (Captain can still see everything). (3) Add fine-grained reset flags: `--include-wardroom` (wipe Ward Room), `--include-tasks` (wipe scheduled tasks), `--include-assignments` (wipe assignments), `--include-checkpoints` (wipe DAG checkpoints), `--keep-trust` (existing), `--all` (wipe everything). (4) DAG checkpoints should be wiped by default (stale checkpoints from pre-reset agents are meaningless). (5) Default reset = current behavior + DAG checkpoints + reset epoch marker. Naval metaphor: new watch rotation — the ship's log from the previous watch is still in the binder, but the new watch doesn't claim those entries as their own.
...
**AD-419: Agent Duty Schedule & Justification** — Roadmap. The proactive cognitive loop (Phase 28b) gives agents freedom to think independently. But freedom without structure is chaos — observed when Wesley (Scout) ran scout reports repeatedly within minutes instead of once daily. Agents need a **duty schedule** that defines their expected recurring responsibilities, and proactive activity outside that schedule should require justification. **Design:** (1) Each crew agent type has a `duty_schedule` definition: a list of recurring responsibilities with cron-like timing (e.g., Scout: `scout_report` once daily at 0600, Security: `security_audit` every 4 hours, Engineering: `systems_check` every 2 hours). (2) The proactive loop checks the duty schedule first — if a scheduled duty is due, that takes priority over free-form thinking. (3) If an agent wants to perform an action outside its schedule (e.g., Wesley running an extra scout report), the proactive think must include a `justification` field explaining why (e.g., "detected anomalous Hebbian weight changes since last report"). (4) Justification quality gates by Earned Agency tier: Ensigns need stronger justification than Commanders. (5) Duty schedules are defined per agent type in config or Standing Orders, but individual agents can propose schedule modifications through AD-412 (Crew Improvement Proposals). Naval metaphor: the Plan of the Day (POD). Every sailor knows their watch schedule, maintenance duties, and drill times. They can act on initiative, but they'd better have a reason if the XO asks why they skipped a scheduled evolution. *Connects to: Earned Agency (AD-357), Standing Orders (AD-339), PersistentTaskStore (Phase 25a — duty schedules could be backed by persistent tasks), Qualification Programs (duty completion feeds qualification tracking).*
