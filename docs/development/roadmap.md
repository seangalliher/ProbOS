...
| BF-069 | **LLM proxy failure is silent — no alerts, no visibility.** When the Copilot proxy (127.0.0.1:8080) goes down or returns empty responses, the entire crew stops thinking proactively with zero indication to the Captain. Proactive loop swallows errors silently. Shell chat returns "(Empty response)". No bridge alert fires. **Need:** (1) LLM health check — periodic ping, track consecutive failures. (2) Bridge alert when N consecutive LLM calls fail ("Communications array offline"). (3) System panel LLM proxy status indicator (green/red). (4) Proactive loop should log warnings and increment failure counter instead of swallowing empty responses. | High | **Open** |
...
| BF-101 | **Agent uses seed callsign instead of chosen callsign in Ward Room.** Post-AD-560 observation: Kira (Data Analyst) identified as "Rahda" (seed callsign) despite naming ceremony completing. Possible warm boot restoration failure or stale `_build_personality_block` lru_cache entry. BF-083 pipeline works but defensive gap remains when `self.callsign` is empty string (passes `None` to personality block, falls back to YAML seed). | High | **Open** |
| BF-102 | **Newly commissioned agents don't know they're new.** All three AD-560 agents welcomed "Kira, Lynx, and Atlas" without recognizing those names as their own. Root cause: no per-agent commissioning awareness. Temporal context shows "age: 0s" but LLM doesn't infer "I am the new crew." BF-034 cold-start system note only in `proactive_think`, not `ward_room_notification`. Fix: commissioning awareness line in temporal context (age < 300s) + cold-start note in Ward Room context. | High | **Open** |
| BF-103 | **`thread_mode="announce"` does not suppress agent notifications.** Ward Room router only suppresses `"inform"` mode. `"announce"` mode (used by onboarding welcome, cold-start announcement, startup announcement) falls through to normal routing, triggering unlimited crew responses. `max_responders=0` means unlimited (not zero), and cap only applies to `"discuss"` mode. Fix: add `"announce"` to suppressed modes in router. | Medium | **Open** |
