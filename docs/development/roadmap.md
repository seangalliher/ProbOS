...
| BF-125 | **Working memory not resynced after stasis recovery.** Crew-identified (Horizon, Science): working memory preserves pre-stasis state snapshots (including game board positions) but doesn't validate against live system state on recovery. Creates "false continuity" — agent believes stale game states are current, leading to incorrect move attempts and inflated self-assessments. Broader implication: any external observation tracked in working memory (not just games) could suffer identical temporal desync across stasis boundaries. **Fix needed:** Post-stasis sync validation in working memory restore — compare preserved entries against current Ward Room / system state, flag or discard entries where external state has diverged. | Medium | Open |
