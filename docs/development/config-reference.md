...
| `hold_degraded_turn_max_threads` | `int` | `16` | ≥ 1, ≤ 256 | AD-1230: how many distinct threads may hold a turn at once. One turn is held per thread (a newer message supersedes the one held for that thread), so this bounds the ship, not the conversation. At the ceiling a further thread is told to resend rather than being promised an answer that would queue behind fifteen others. |
