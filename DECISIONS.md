...
### AD-926a: Task-level multi-file input upload — `POST /api/work-items/{work_item_id}/inputs` populates the AD-926 `WorkItem.metadata["input_attachments"]` convention via the SHARED chat uploader and a single read-merge-write (the WRITE half of the Task Workspace Rooms Inputs folder; the AD-926 read endpoint + the AD-929 Files rail already work)
