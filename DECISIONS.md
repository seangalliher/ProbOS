...
| AD-338 | Builder Commit Gate & Fix Loop (PLANNED) — Gate commits on test passage. Pass llm_client from api.py to enable 2-retry test-fix loop. Fix "Files: 0" reporting bug. |
| AD-339 | Standing Orders Architecture (PLANNED) — ProbOS constitution system. 4-tier hierarchy: Federation Constitution (universal, immutable across all instances) → Ship Standing Orders (per-instance project conventions) → Department Protocols (per-department standards) → Agent Standing Orders (per-agent, evolvable via self-mod pipeline). `config/standing_orders/` directory with `federation.md`, `ship.md`, `engineering.md`, `science.md`, `medical.md`, `security.md`, `bridge.md`. `compose_instructions()` assembles complete system prompt at call time. Integrated into `CognitiveAgent.decide()`. Like Claude Code's CLAUDE.md and OpenClaw's soul.md but hierarchical, composable, and evolvable. No IDE dependency. |
| AD-340 | Builder Instructions Enhancement (PLANNED) — Add concrete test-writing rules to Builder's hardcoded instructions: read __init__ signatures before instantiating, use full `probos.*` import paths, trace mock coverage through method bodies, match actual output format in assertions. |
| AD-341 | Code Review Agent (PLANNED) — `CodeReviewAgent` reviews Builder output against Standing Orders before commit gate. Engineering department, standard LLM tier. Starts as soft gate (advisory, logs issues but doesn't block). Earns hard gate authority through ProbOS trust model: Beta(1,3) = 0.25 probationary, promoted after demonstrated accuracy across builds. |

**Status:** AD-337 complete (2213 Python + 30 Vitest). AD-338–341 planned — build prompt at `prompts/builder-quality-gates.md`.
