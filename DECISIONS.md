...
### AD-933b: Group-chat image generation — add `step_4c_image_gen_parse` (AD-730-3 `[GEN_IMAGE]`) to the AD-933 channel-agnostic escalation subset so a crew agent can generate an image in a room, and surface the generated SHA refs onto the persisted group message metadata — **no `build_response` / 1:1-path / `DmReplyContext` / `IntentMessage` / facilitator / AD-933a-episodic-write / Ward Room / return-shape change**

**The verified invisible-image gap.** `reply_pipeline.step_4c_image_gen_parse` is already channel-agnostic and 1:1-safe: it reads `ctx.sanity_gate` + `ctx.runtime.config.avatars.image_gen_*`, calls `dispatch_image_gen(runtime, agent_id=…, prompt=…)`, appends the returned SHA to `ctx.generated_attachment_ids`, and honest-degrades (marker stripped, operator message appended) when the tier is disabled/unconfigured — nothing in it is 1:1- or avatar-scoped. But AD-933's `_escalation_steps()` deliberately omitted 4c, and the group fan-out (`thread_fanout.group_chat_fanout._send_one`) returns `{agent_id, callsign, text}` and never calls `build_response()` — the 1:1 method that exposes `generated_attachment_ids` on the response. Net: a group-generated image would be created and persisted to `AttachmentStore` (AD-731 SHA refs) yet **invisible** — no path carried the ref to anywhere the room could render it.

**The two additive changes.** (1) `_escalation_steps()` adds `self.step_4c_image_gen_parse` in **run()-order** (4c precedes 4e in `_full_steps`), so the subset tuple becomes `(4c, 4e, 4i, 4h, 4f, 4g)` and `run_escalation_only()` runs **6** steps. `run()` and `_full_steps()` are **byte-identical** — 4c was already in `_full_steps`, so the 1:1 path is untouched; only the docstring's Included/Excluded lists and the relative-order parenthetical were corrected (4c moved from Excluded → Included; the stale `Forward markers: AD-933a/AD-933b` line replaced by the live `AD-933b-2` `step_4d_follow_up` deferral). (2) `_send_one` initializes `generated_ids: list[str] = []` before the AD-933 escalation block, then — inside the same Tier-2 try, after `reply_text = pipeline.ctx.response_text or reply_text` — captures `generated_ids = list(pipeline.ctx.generated_attachment_ids or [])` from the **SAME** `pipeline.ctx` the AD-933 block already built (no second pipeline), and in the persist block extends the existing `append_message` metadata `{"intent_id": intent.id, "fanout": "ad914"}` with `"generated_attachment_ids": generated_ids` **only when non-empty** (AD-916 established `metadata`-carried attachment refs on chat messages). Tier-2: a skipped (no reply / no agent) or raised escalation leaves `generated_ids == []` and the persisted metadata **byte-identical** to the AD-914 baseline; the `{agent_id, callsign, text}` return shape is unchanged.

**Why `step_4d_follow_up` stays out (the subset grows by exactly one).** `step_4d_follow_up_parse` schedules a synthesized user-turn via the runtime's `conversation_pacing_scheduler` — in a 1:1 the target is unambiguous, but in a multi-agent room "who is the follow-up addressed to / who re-speaks" is undefined, so it is deferred as forward marker **AD-933b-2** (and UI rendering of group-generated images, which AD-916's `metadata` ref carriage now makes possible, is **AD-933b-3**). The right answer is the minimal subset extension, not a follow-up redesign. **One obsolete-contract test was updated** — the AD-933 `test_run_escalation_only_invokes_only_the_subset` asserted the OLD 5-step `_ESCALATION_SUBSET`; that constant was re-pointed to the 6-step subset (4c first), preserving its named intent (escalation runs ONLY the subset; the other 11 steps never fire) — the same obsolete-contract-test pattern as AD-933a's `test_group_path_writes_no_1to1_episode` re-point, not scope creep. +5 pytest (`tests/test_ad933b_group_image_gen.py`, BF-287 real `ChatThreadStore`+`IntentBus`+`DmSanityGate`; `dispatch_image_gen` monkeypatched at its source module `probos.cognitive.image_gen_dispatch` because step_4c imports it function-locally): spy-level `run_escalation_only()` runs exactly `{4c,4e,4i,4h,4f,4g}` (other 11 not) + `run()` still 17 byte-identical; fan-out-level group `[GEN_IMAGE a cat]` surfaces `metadata.generated_attachment_ids == ["sha-xyz"]` (marker stripped), disabled tier honest-degrades (operator message appended, no ref key, no crash), and a plain reply leaves metadata exactly `{"intent_id","fanout"}`. Focused gate **5 passed**; blast-radius `-k "thread or chat or fanout or reply or pipeline or image or escalat"` **860 passed / 4 failed / 1 skipped** (the 4 fails are the pre-existing `test_skill_agent.py::TestSkillPipeline` serial full-suite isolation flakes — `Event loop is closed` cross-test pollution — **green 7/7 in isolation**, about skill-pool spawning, zero connection to AD-933b). No UI change → no Vitest. **COMMITTED LOCAL ONLY — NOT pushed.** Forward markers: **AD-933b-2** (in-chat `[FOLLOW_UP]`), **AD-933b-3** (UI render of group-generated images), **AD-934** (in-chat `[THINK]`/`[DELIBERATE]`).

### AD-933a: Group-anchored episodic write for the fan-out — add a Tier-2 episodic-write loop at the END of `group_chat_fanout` so every group-chat reply persists ONE `channel="chat"` / `session_type:"group"` episode tagged with the thread id, mirroring the AD-719 @-mention fan-out but group-anchored; closes the learning-loop hole AD-933 left open — **no `_send_one` / `DmReplyPipeline` / `_store_action_episode` / `step_5` / 1:1-path / facilitator / `IntentMessage` / Ward Room change**

**The verified no-episode gap.** A group-chat fan-out reply wrote **no episode at all** — a learning-loop integrity hole. Two paths both decline the write: (1) the fan-out sends `direct_message` with `params["from"]="hxi_profile"` (`thread_fanout._send_one`), and the agent's universal episodic safety-net `_store_action_episode` (`cognitive_agent.py:8034`) **skips exactly that case** (`if intent.intent == "direct_message" and source in ("hxi_profile","captain"): return`), deferring to the pipeline's `step_5_episodic_store`; (2) AD-933 deliberately **excluded** `step_5` from the group escalation subset because `step_5` hardcodes `session_type:"1:1"`/`channel:"dm"`/`participants:["captain", callsign]`, which would **mislabel** a multi-agent turn. So neither path fired — agents could not remember what they said in a room (no episodic recall, no dreaming consolidation, no wellness/divergence analysis), violating the standing invariant "every execution path stores an episode or the learning loop breaks."

**The mirror-AD-719 fix.** The AD-719 @-mention fan-out (`routers/chat.py` ~L215–256) already solves the identical problem on the 1:1-chat substrate: per resolved reply it writes one group-shaped episode (`dream_adapter.build_episode(...)` when a `dream_adapter` is present, else a direct `Episode(...)` with `AnchorFrame(channel="chat", trigger_type="at_mention_fanout")`), inside a Tier-2 `try/except` that logs and continues. AD-933a mirrors this exactly at the END of `group_chat_fanout` (after `replies = await asyncio.gather(...)`), but group-anchored: `episodic_memory = getattr(runtime, "episodic_memory", None)` (skip entirely if `None`); participants resolved **once** as `["captain"] + [callsign-or-agent_id per crew speaker]`; then for each reply — **skipping** the sentinel set `{"(no response)", "(delivery failed)", ""}` and any reply with no `agent_id` — build the episode (prefer `runtime.dream_adapter.build_episode(episode_input, {"response":…, "agent_ids":[…]}, t_start, t_end)` when a `dream_adapter` exists; else a direct `Episode(timestamp=time.time(), user_input="[group chat] Captain: …", dag_summary={}, outcomes=[{"intent":"direct_message","success":True,"response":reply["text"][:500],"session_type":"group","callsign":…,"source":"group_chat_fanout"}], agent_ids=[reply["agent_id"]], duration_ms=…, source="group_chat_fanout", anchors=AnchorFrame(channel="chat", trigger_type="group_fanout", participants=…, chat_thread_id=thread_id))`), then `await episodic_memory.store(episode)` inside a `try/except Exception` that logs `AD-933a: group episode store failed …; continuing` and **NEVER raises** — the fan-out always returns all replies even if a store fails. `t_start = time.monotonic()` is captured at the TOP of the function (new module-level `import time`); `t_end` right after the gather. `source="group_chat_fanout"` is a distinct memory tag, parallel to AD-719's `"multi_agent_chat"`, so group episodes are trivially filterable for recall/dreaming.

**Why `step_5` stays excluded (the fix is additive, not a `step_5` generalization).** The right answer is **not** to teach `step_5_episodic_store` to write group episodes: `step_5` is the 1:1-pipeline's episodic writer, hardcoded to `session_type:"1:1"`/`channel:"dm"` and `participants:["captain", callsign]`, and it runs inside `DmReplyPipeline.run()` (the 1:1 `/chat` path). Reusing it for group would require a context-conditional that mutates the 1:1 contract — a Defense-in-Depth and Single-Responsibility violation — and AD-933 already established that the group fan-out runs only the **channel-agnostic** escalation subset (`step_5` is deliberately out). AD-933a instead writes a **dedicated, group-shaped** episode at the fan-out call site, where the thread id, participants, and group semantics are all in scope, leaving the 1:1 pipeline and `step_5` byte-identical. The preferred `dream_adapter.build_episode` branch mirrors AD-719 for DRY/consistency (and runs in production, where a `dream_adapter` exists); the direct `Episode(...)` fallback carries the explicit group anchors and is the branch exercised by the BF-287 tests (their runtime has no `dream_adapter`). **One obsolete-contract test was updated** — the AD-933 `test_group_path_writes_no_1to1_episode` asserted `recorder.stored == []` (the pre-AD-933a "no episode at all" behavior that AD-933a's entire purpose overturns); it was re-pointed to "group episodes ARE written now, and NONE are 1:1-labelled" (`session_type != "1:1"`, `channel != "dm"`), preserving its named intent (step_5/the 1:1 writer never fires on the group path). This is the direct, necessary consequence of the spec'd behavior change — the same obsolete-contract-test pattern as AD-933's own `test_bf296` re-point, not scope creep. +6 pytest (`tests/test_ad933a_group_episode.py`, BF-287 real `ChatThreadStore`+`IntentBus`+a real-but-fake recording `episodic_memory`, no `dream_adapter`); focused gate **6 passed**, blast-radius `-k "thread or chat or fanout or episod or dream"` **1285 passed / 1 skipped**. Forward markers: **AD-933b** richer escalation subset, **AD-934** in-chat `[THINK]`/`[DELIBERATE]`.

### AD-933: Wire the AD-726 one-shot escalation ladder into the AD-914 group-chat fan-out — refactor `DmReplyPipeline.run()`'s 17-step tuple into `_full_steps()` + `_run_steps(steps)` (run() **byte-identical**) and add a public `run_escalation_only()` that runs only the **channel-agnostic escalation subset** (`[ACTION]` / notebook / mesh-read / artifacts / `[CREATE_TASK]`); `thread_fanout._send_one` runs that subset on the raw reply so a group turn can escalate, not just ship a Tier-1 reply — **no `run()` behavior change, no step-body change, no `DmReplyContext` change, no facilitator / `IntentMessage` / vision / Ward Room change**

**The Tier-1-only group seam, closed.** The 1:1 chat path (`routers/agents.py:agent_chat`) and the group fan-out (`routers/thread_fanout.py:group_chat_fanout._send_one`) are structurally identical up through `result = await runtime.intent_bus.send(intent)` — both build `IntentMessage(intent="direct_message", …)` and read `result.result`. But the 1:1 path then constructs a `DmReplyContext` + `DmReplyPipeline` and runs the **post-LLM escalation + cleanup ladder** (AD-726: inline mesh reads AD-869, `[ACTION]` dispatch AD-745, `[CREATE_TASK]` AD-845, artifact extraction AD-797, notebook AD-911, plus 1:1-specific steps), whereas `_send_one` took `reply_text = str(result.result)` **RAW** and persisted it — it **never constructed the pipeline**. So group chat was **Tier-1 only**: an agent in a room could not resolve an inline mesh read, dispatch a browser `[ACTION]`, or open a task via `[CREATE_TASK]`. `DmReplyPipeline` was already **extracted** from the endpoint (AD-726) precisely so it could be reused; the fan-out simply never got wired to it. AD-933 is that wiring.

**Three execution modes, one source of truth.** The post-LLM ladder now has three callers, mapped explicitly: (1) **1:1 `/chat`** → `run()` (the full 17 steps); (2) **group fan-out** → `run_escalation_only()` (the 5-step subset); (3) **proactive `[GROUP_CHAT]`/`[STATUS]`/`[ARTIFACT]` tags** → the AD-924/927/928 proactive extractors (unchanged). To keep them DRY, `run()`'s hardcoded 17-step tuple is extracted into `def _full_steps(self) -> tuple[Callable, ...]` and the `for step in (…)` loop body into `async def _run_steps(self, steps)` (the verbatim AD-726 per-step `try/except … logger.warning("AD-726: pipeline step %s raised …")` guard); `run()` becomes `await self._run_steps(self._full_steps())` — **byte-identical** (same 17 steps, same order, same guard; a regression-guard test asserts `run()` still invokes all 17 in order). `_escalation_steps()` returns the subset in run()-order — `step_4e_action_dispatch`, `step_4i_notebook_parse`, `step_4h_mesh_read_parse`, `step_4f_extract_artifacts`, `step_4g_create_task_parse` — and `run_escalation_only()` runs exactly that under the same guard. In `_send_one`, `sanity_gate = getattr(runtime, "dm_sanity_gate", None)` is resolved **once** before the `asyncio.gather` (DRY; `step_4g` needs it), and **only when** a real `result.result` came back **AND** `agent is not None`, it builds `DmReplyPipeline(DmReplyContext(…, sampling_state=None, avatar_event_bus=None, chat_thread_id=thread_id))`, `await pipeline.run_escalation_only()`, and adopts `reply_text = pipeline.ctx.response_text or reply_text` (now tag-stripped + may carry the `(Task opened: <id>)` suffix) before the existing `store.append_message(…)`. The whole block is **Tier-2 honest-degrade** (an inner `try/except` plus the per-step guards inside the pipeline) so any failure ships the raw `reply_text` rather than silencing a reply.

**Why the *subset*, not the full `run()`.** Running the full pipeline verbatim on group replies is **wrong** because several steps carry 1:1 semantics — the decisive one is `step_5_episodic_store`, which hardcodes `session_type:"1:1"`, `channel:"dm"`, `participants:["captain", callsign]`; firing it on a multi-agent group reply writes **mislabeled** episodes — a learning-loop integrity regression. `step_6` (working memory) similarly records `"Captain DM"`, and `step_1` (sanity retry), `step_2/3` (games), `step_4b` (outbound DM), `step_4c` (image-gen), `step_4d` (follow-up), `step_7` (divergence), `step_8` (mark-emitted/avatar), `step_9` (emotion) are all 1:1/avatar-scoped. So AD-933 runs only the **channel-agnostic** steps: each is a strict no-op for any reply lacking its marker, and the markers are emitted only by specifically-taught agents (e.g. Yeo for `[CREATE_TASK]`), so the subset is inherently bounded and safe outside the 1:1 path. None of the module's ordering invariants (sanity-before-games, self-check-before-episodic, divergence-before-emit) involve these five, so the subset is internally order-independent; run()-order is kept for clarity.

**Non-goals / forward markers.** **`DmReplyPipeline.run()` behavior byte-identical** (all 17 steps, same order, same guard); the 1:1 `/api/agent/{id}/chat` path **unchanged** (still calls `run()`); **no step-method body change**, no `DmReplyContext` field change, no `build_response()` change. **NO** facilitator / who-speaks-order / `IntentMessage` / vision-pipeline / persist-call change. **NO** AD-632 chain touch (`_CHAIN_ELIGIBLE_INTENTS`, `_should_activate_chain`, `_execute_chain_with_intent_routing`, `_pending_sub_task_chain`) — in-chat `[THINK]`/`[DELIBERATE]` via the dormant escape hatch is the deferred **AD-934**. **NO** AD-925 task-room logic (it fires downstream automatically when a created task fans to ≥2 crew); **NO** Ward Room. A correct **group-anchored** episodic write is forward marker **AD-933a**; an optional richer subset is **AD-933b**. **Tests: +8 pytest** (`tests/test_ad933_group_chat_escalation.py`, BF-287 — real `WorkItemStore` (in-memory SQLite, started), real `ChatThreadStore` on `tmp_path`, real `IntentBus(SignalManager(reap_interval=1.0))` with subscribed `direct_message` handlers, real `DmSanityGate` on `runtime.dm_sanity_gate`, real-but-fake registry/callsign stubs — no MagicMock at the substrate boundary): group `[CREATE_TASK]` opens a dispatchable `yeo-delegated` work item + persists the reply tag-stripped with `(Task opened: <id>)`; plain reply is a no-op (zero work items); `dm_sanity_gate=None` honest-degrades; the group path writes NO 1:1-labelled episode (recording `episodic_memory` proves `step_5` excluded); an AD-869 mesh marker runs/strips without hanging (empty pool → `step_4h` fast-degrades, no bus TTL wait); `run_escalation_only()` invokes ONLY the 5-step subset and `run()` still invokes all 17 in order (spy guards); the fan-out return shape `{agent_id, callsign, text}` is preserved with the mutated `text`. The `test_bf296` `inspect.getsource(run)` structural guard was re-pointed at `_full_steps` (the tuple's new home — its intent, step_4b present + ordered, is preserved). Focused gate **8 passed**; blast-radius `-k "thread or chat or fanout or dm or reply or pipeline or create_task or escalat"` **1164 passed | 1 skipped**, zero regressions. No UI change → no Vitest. **COMMITTED LOCAL ONLY — NOT pushed** (the Captain decides the push). Forward markers: **AD-933a** (group-anchored episodic write), **AD-933b** (richer subset), **AD-934** (in-chat `[THINK]`/`[DELIBERATE]`).

### AD-932: Discoverable "+ Add people" on an empty/fresh 1:1 chat — a small `EmptyChatAddPeople` affordance, mounted by `ProfileChatTab` only when `!activeThreadId`, that **materializes** the 1:1 thread (`createThread([agentId])`) so the existing AD-917 `GroupChatHeader` (+ its `AddParticipantPopover`) takes over — a materialize-then-hand-off, frontend-only, **no `GroupChatHeader` contract change, no backend, no Ward Room**

**The undiscoverable-add gap on a fresh 1:1, closed.** The AD-917 in-chat group controls mount in `ProfileChatTab` only as `{activeThreadId && <GroupChatHeader threadId={activeThreadId} />}`, and `GroupChatHeader` early-returns `if (!thread) return null`. A brand-new 1:1 has **no thread until the first message is sent** (`get_or_create_default_for_agent` creates the row server-side on the first `agent_chat` turn), so on an empty 1:1 `activeThreadId` is `undefined` → the header never mounts → there is **no add-participant control at all**. The only pre-AD-932 path to "add someone" was send-a-message-first-then-find-the-thin-header-bar — exactly the Captain's "undiscoverable" observation.

**Materialize, then hand off (don't duplicate the picker).** AD-932 adds a focused `ui/src/components/profile/EmptyChatAddPeople.tsx` mounted via one sibling line `{!activeThreadId && <EmptyChatAddPeople agentId={agentId} />}`. On click it creates the 1:1 thread, writes it into the store, and points the agent at it — `setChatThread(t)` + `setThreadForAgent(agentId, t.id)` — which makes `activeThreadId` resolve, so the **existing** `GroupChatHeader` (and its `AddParticipantPopover`) mounts on the next render and the Captain uses *that* picker to add the second crew member. AD-932 only carries the empty state from "no thread" → "thread exists, header visible"; it does **not** re-implement the picker. Auto-opening the picker on materialize would need a new `GroupChatHeader` prop (`defaultPickerOpen`); to keep the header's contract unchanged that is deferred as forward marker **AD-932a**.

**Why `createThread`, not a hidden default.** There is no client-callable "get-or-create my 1:1 thread" endpoint — `get_or_create_default_for_agent` is invoked only server-side by the `agent_chat` path. When `ProfileChatTab` shows the empty state the client genuinely has **no** thread for the agent (`threadId` prop undefined AND `threadIdByAgent.get(agentId)` empty — the exact gate for rendering the button), so `createThread({ title: callsign, participants: [agentId] })` is the correct client-side materialization; the row it mints (`participants=[agentId]`, `project_id=NULL`) is also what the server's default lookup matches. The known minor edge (a second `participants=[agentId]` row if a server default existed but was never hydrated) is Tier-2 acceptable — the Captain's intent is a group, so the next `addParticipant` turns it into `[agentId, X]`; no backend de-dup is added here.

**Disjoint by construction; self-gated.** The mount is mutually exclusive with `GroupChatHeader` (one iff `activeThreadId`, the other iff `!activeThreadId`) and disjoint from the AD-929 Files rail (`showWorkspaceFiles = !!activeThreadId && isWorkspaceRoom(...)` is `false` when `!activeThreadId`), so it never co-renders with the rail, `MeetingView`, or the meeting gates (all `activeThreadId`-gated). The component self-gates on `agent?.isCrew` (only a crew 1:1 can become a group; non-crew / unknown host → renders nothing). `createThread` honest-degrades to `null` → keep the button, no store write. HXI #3 — inline-SVG `UserPlus` (reused; **no `Glyphs.tsx` exports**, the export-count test stays green), amber `#f0b060` / dim, glow-on-hover, no emoji.

**Non-goals / forward markers.** **Independent of AD-931** (builds on AD-917/threadApi; touches only `profile/`, imports nothing from `chats/`). **NO `GroupChatHeader` change** (forward marker **AD-932a** = auto-open picker). No full-`ProfileChatTab` render test (the `groupsend`/`bf294b` precedent: "too heavy to render" → component-isolation tests only). **NO Ward Room DM convergence**, chain-pipeline, `ward_room/*`, or AD-574c-i. No backend / REST / pytest / new endpoint / `get_or_create_default_for_agent` exposure. No AD-929 Files-rail / AD-925 task-room change. No `LeftRail.tsx` wire. No eager thread creation on profile open (materialized only on the explicit click). **Tests: +8 Vitest** (`profile/__tests__/EmptyChatAddPeople.test.tsx`, BF-287 real store + mocked `threadApi.createThread`): renders for crew; nothing for non-crew / unknown agentId; click → `createThread` once with `{title, participants:[agentId]}`; success writes `setChatThread` + `setThreadForAgent`; honest-degrade on `null`; clickable; no-emoji. Full Vitest **1270 passed | 1 skipped** (215 files; baseline 1262 + 8, zero regressions); `npm run build` clean (`tsc -b`). **COMMITTED LOCAL ONLY — NOT pushed** (the Captain decides the push). Forward marker: **AD-932a** (auto-open the picker on materialize).

### AD-931: Unified CHATS surface — one Teams/Slack-style home listing BOTH 1:1 and group conversations with a prominent "+ New chat" picker; **repurposes** (moves + renames) the AD-919 `GroupChatListPanel` into `ChatsPanel`, widens the filter to `isChat` (1:1 + group, **excluding** AD-925 task rooms), and branches the create flow on selection count — frontend-only, no backend, **NO Ward Room DM convergence** (the two DM systems stay a deliberate pipeline A/B test)

**The "no way to start a chat" gap, closed — and the two-mental-models seam removed.** The Captain's observations (verified against HEAD): (1) `GroupChatListPanel` (AD-919) was **visibility + Join only** — it listed group chats but had **no create affordance** anywhere in the HXI; (2) the TopNav exposed "GROUP CHATS" as a thing apart from 1:1 DMs, whereas Teams/Slack present **one Chats list** holding both — a 1:1 lived only inside an agent's profile chat tab and never appeared in a list. AD-931 is the single unified entry point: one **CHATS** panel that lists 1:1 + group with a **"+ New chat"** picker.

**Repurpose + rename, not a sibling.** A second panel would violate "one chats home," so AD-931 **moves** `groupchat/GroupChatListPanel.tsx` → `chats/ChatsPanel.tsx`, extracts the five pure helpers (`crewParticipantIds`/`isAgentCreated`/`isGroupChat`/`captainJoined`/`hostAgentId`) + constants verbatim into `chats/chatFilters.ts`, and renames the store flag `groupChatListOpen` → `chatsOpen` (+ `openGroupChatList`/`closeGroupChatList` → `openChats`/`closeChats`) and the TopNav `GROUP CHATS` (testId `group-chats-toggle`) → `CHATS` (`chats-toggle`). The internal rename (not just the label) is because once the panel lists 1:1s the old name is actively misleading; grep confirmed the blast radius is **only** `useStore.ts`, `App.tsx`, the panel, and its test (no App-level test references the flag). `LeftRail.tsx` stays the forward marker AD-719b-parent-wire (untouched).

**The unified `isChat` filter — admits 1:1s, excludes task rooms.** A new pure `isChat(thread, agents)` widens AD-919's `isGroupChat`: `!thread.task_id && (isAgentCreated(thread) || crewParticipantIds(thread, agents).length >= 1)`. 1:1s appear because `get_or_create_default_for_agent` (`threads/__init__.py:636`) inserts a real listed `chat_threads` row (`participants=[agentId]`, `metadata.is_default=true`, `task_id=NULL`), and `GET /api/threads` (`list_threads`) returns ALL non-archived rows with **no participant filter** — so this is frontend-only, no backend change. The `!task_id` gate **excludes AD-925 task rooms** (`crew_executor.py:319` sets `task_id`), so the list reads like Teams' *Chat* list, not its *Teams/Channels* list. Within a row, `isGroupChat` still distinguishes: a **group** keeps the AD-919 treatment (multi-avatar strip, agent-created badge, **Join** when `!captainJoined`); a **1:1** shows a single avatar + callsign and **no Join** (Join would `add_participant('captain')`, meaningless for a 1:1).

**"+ New chat" branches on selection count (avoids the duplicate-1:1 trap).** `NewChatModal` **reuses** the AD-917 `AddParticipantPopover` as its selection surface — each `onAdd` accumulates into `selected[]`, the running selection is fed back as `existingParticipantIds` so picked agents drop out, removable chips above the popover, a **"Start chat"** CTA disabled until `selected.length >= 1`. On confirm: **1 agent → `openAgentProfile(agentId)`** (do NOT `createThread` — the server-side `get_or_create_default_for_agent` owns the 1:1 default; minting a second `participants=[agentId]` row would diverge from the `agent_chat` default lookup); **2+ agents → `createThread({ title, participants })`** (`title` = selected callsigns joined, fallback `"New group chat"`) then `setThreadForAgent(selected[0], thread.id)` + `openAgentProfile(selected[0])` (verbatim AD-919 host pattern); `createThread` honest-degrades to `null` → keep the modal open (Tier-2), no throw.

**Non-goals / forward markers.** **NO Ward Room DM convergence** — Ward Room DMs (`channel_type=='dm'`, `DmActivityLog`, the **chain pipeline**, `ward_room/*`, **AD-574c-i**) are untouched: the two DM systems are a **deliberate A/B test** (Ward Room = chain pipeline, 1:1 Chats = one-shot pipeline), so convergence is **gated on a pipeline decision**, not folded in here. No backend / REST / pytest (every endpoint already exists). No `LeftRail.tsx` wire. No `Glyphs.tsx` exports (reuse `UserPlus`/`Close` + local inline SVGs — the export-count test stays green). No `createThread` for the 1-agent path. No `GroupChatHeader`/`ProfileChatTab` change. The discoverable in-chat add-people on an empty 1:1 is the sibling **AD-932** (intentionally split out). **Tests (Vitest, UI hard rule): +25 across three new files** — `chats/__tests__/chatFilters.test.ts` (8 — `isChat` admits 1:1 / group / agent-created, excludes task room + captain-only; `isGroupChat`/`hostAgentId`/`captainJoined` parity), `chats/__tests__/ChatsPanel.test.tsx` (11 — migrated from the 10 AD-919 panel tests with test #1 FLIPPED to "lists BOTH 1:1 and group; excludes task room", + a 1:1-row-no-Join + 1:1-click-opens-host case), `chats/__tests__/NewChatModal.test.tsx` (6 — button opens modal + popover; Start gate ≥1; 1→`openAgentProfile` no `createThread`; 2+→`createThread` + open host; chip remove; no-emoji), **superseding** the deleted 10-test `groupchat/__tests__/GroupChatListPanel.test.tsx` (net suite ≈ +15). Full Vitest **1262 passed | 1 skipped** (214 files; baseline 1247 + 25 − 10 superseded, zero regressions); `npm run build` clean (`tsc -b`). **COMMITTED LOCAL ONLY — NOT pushed** (the Captain decides the push). Forward marker: **AD-932** (discoverable in-chat add-people on an empty 1:1).
 — `POST /api/work-items/{work_item_id}/inputs` populates the AD-926 `WorkItem.metadata["input_attachments"]` convention via the SHARED chat uploader and a single read-merge-write (the WRITE half of the Task Workspace Rooms Inputs folder; the AD-926 read endpoint + the AD-929 Files rail already work)

**The population path AD-926 deferred.** AD-926 shipped the READ (`GET /api/threads/{id}/inputs`) and AD-929 the display (the Files rail), but the authoritative `WorkItem.metadata["input_attachments"]` source was a **defined-but-dormant seam** — nothing WROTE it (`git grep AD-926a` returned only two forward-marker comments). The Captain asked: "attach multiple files as context inputs for a task." AD-926a is the missing WRITE half — a new `POST /api/work-items/{work_item_id}/inputs` (multipart `files: list[UploadFile]`) on the **existing** `/api` workforce router (NOT under `threads.py`: inputs are owned by the work item, and a thread learns about them through its `task_id`).

**Reuse the uploader, never a parallel one.** Each file routes through the SHARED `chat._validate_and_store_attachment` — the same AD-720a defense-in-depth chain the chat multipart endpoint uses: feature gate → MIME allowlist → size cap → sha256 → magic-byte `validate_attachment_bytes` → idempotent content-addressable `AttachmentStore.write` — with the DEFAULT `origin="chat_attachment"` (operator intent, never age-reaped, last to LRU-evict; a new `task_input` origin would coerce to `chat_attachment` anyway per the `_normalize_origin` allowlist, so none is added). On success a ref `{content_hash, mime, filename}` (the exact shape `_collect_task_inputs` reads, `source="task"`) is appended; `filename` comes from the `UploadFile` (the uploader's success dict has none).

**Single read-merge-write (the `update_work_item` REPLACE crux).** `WorkItemStore.update_work_item(id, metadata=…)` writes the `metadata` column as a WHOLE-COLUMN REPLACE (`json.dumps`), with no native merge and no `BEGIN IMMEDIATE` / optimistic concurrency. So the merge MUST happen at the call site: validate+store ALL N files FIRST, accumulate the refs, then ONE `get_work_item` → `dict(metadata)` → append the new refs (DEDUPE by `content_hash`, so an idempotent re-upload is a no-op) → `update_work_item(metadata=merged)`, preserving every other metadata key plus any existing `input_attachments`. Storing all files before the single write eliminates the intra-request race; the cross-request last-writer-wins is a documented bounded seam (an operator-driven manual action, rare concurrency) → a store-level atomic `append_input_attachments` (`BEGIN IMMEDIATE`, mirroring `ChatThreadStore.set_title`) is the forward marker **AD-926a-2**. `update_work_item` is deliberately NOT refactored here.

**Honest-degrade per file; operator authority.** A rejected file (oversize / mime mismatch / disallowed) is collected into `skipped` rather than 500-ing the whole request — good files still attach. 503 when `work_item_store` is None, 404 when the work item is missing (mirroring the sibling work-item routes). Authority is operator/Captain-facing and mirrors the other work-item mutations (PATCH / transition / assign / delete have no per-caller authority check): attaching a file is additive, reversible, low-risk → **no consensus gate** (Safety Budget axiom), stated in the docstring.

**UI — the "+ Attach" affordance.** `WorkspaceFilesRail` gains a `taskId?` prop (`ProfileChatTab` passes `workspaceThread?.task_id`); the Inputs section shows a multi-file "+ Attach" button (a hidden `<input type="file" multiple>` + inline stroke-SVG, HXI #3 no-emoji, amber `#f0b060`) ONLY when `taskId` is set — a ≥2-crew room without a bound work item has no place to hold inputs (a forward-marker constraint, stated). A new `inputsApi.attachTaskInputs(workItemId, files)` posts ONE multipart `files[]` request (mirroring `ProfileChatTab.uploadAttachment`) and refreshes the rail's local `inputs` from the returned list.

**Non-goals / forward markers.** No input edit / versioning (inputs are immutable context; the editable side is artifacts/outputs, AD-797/AD-927). No delete-input in v1 (**AD-926a-1**). No store-level atomic merge — `update_work_item` is not refactored (**AD-926a-2**). No new attachment origin. No kanban / `WorkBoard` card attach button. No change to the `AttachmentStore` Protocol, the `WorkItem` schema/dataclass, the AD-926 read endpoint / `_collect_task_inputs`, the AD-925 create path, or the message-attachment (`source="message"`) path. No consensus gate / quorum, no new `EventType`, no new config field, no new REST router (extend the workforce router). **Tests:** `tests/test_ad926a_task_file_upload.py` (+10, BF-287 — real `WorkItemStore` + real `FilesystemAttachmentStore` + real `ChatThreadStore` + real `SystemConfig`, `UploadFile` built from `io.BytesIO`, the chat uploader store cache seeded so the write lands in the test store: two-file append + read-back, bytes land by sha256, second-upload append preserving a sentinel metadata key, within-request dedupe, mixed good/bad per-file skip, disallowed-mime skip, 404, 503, empty-files no-op, integration via `GET /api/threads/{id}/inputs` `source="task"`) + `ui/src/components/inputs/__tests__/inputsApi.attach.test.ts` (+5) + `ui/src/components/workspace/__tests__/WorkspaceFilesRail.attach.test.tsx` (+5). Focused pytest **28** (10 AD-926a + 10 AD-926 + 8 AD-925); blast-radius `-k "workforce or work_item or thread or chat or attachment or input"` **797 passed**; full Vitest **1247 passed | 1 skipped** (212 files; baseline 1237 + 10, zero regressions); `npm run build` clean. **COMMITTED LOCAL ONLY — NOT pushed** (Task Workspace Rooms epic held for Captain review/push). Forward markers: **AD-926a-1** (delete-input), **AD-926a-2** (atomic store-level append).

### AD-930: Crew presence layer — a Microsoft-Teams-style per-crew presence dot (`offline | online | working | in_meeting`) computed by a read-only polled `GET /api/crew/presence` that **aggregates three verified-existing signals (liveness, `meeting_active` thread membership, recent `last_active`) and invents no telemetry**; the last Teams-signature gap after group chat (AD-913→919), meetings (AD-920→923), and the Task Workspace Rooms epic (AD-925→929)

**The presence dot the Captain asked for.** The request: "a good indicator to see if the agent is actively working or in a meeting." Every signal a presence layer needs already exists, so AD-930 is **aggregation, not new instrumentation**. `online`/liveness = `BaseAgent.is_alive` (`AgentState` ∈ {ACTIVE, DEGRADED}, `agent.py:101`) over `registry.all()` filtered by `is_crew_agent`; `in_meeting` = the agent is a participant of a non-archived chat thread whose `metadata.meeting_active` is set (AD-920); `working` = `AgentMeta.last_active` (`types.py:40`, bumped in `update_confidence()` after each completed op) within a new `CommunicationsConfig.presence_working_window_seconds` (default 90.0).

**Decision 1 — the 4-state model + precedence.** `PresenceState = offline | online | working | in_meeting`. **Liveness is the floor:** a not-alive agent is `offline` regardless of thread membership (SPAWNING/RECYCLING/absent). Among alive agents the precedence is the Captain's stated order `in_meeting > working > online` — an agent both recently-active *and* in a meeting reads as `in_meeting` (matching Teams' "In a meeting" busy variant). Keyed by `agent_id` (the registry instance id, which is also what thread `participants` and `meta` are keyed on).

**Decision 2 — `working` is an HONEST recent-activity proxy, not an in-flight flag.** There is **no canonical per-agent "currently executing right now" backend source at HEAD** — `avatars/telemetry.py:24` states it verbatim (the avatar `load` is a TTS-speaking approximation, not task work). The real signal that *does* exist is `AgentMeta.last_active`, bumped after each completed operation (tool dispatch, `CognitiveAgent.handle_intent`, the proactive loop) and already consumed as a recency signal at `introspective_telemetry.py:111`. So v1 `working` = "completed an operation within `presence_working_window_seconds`" — a last-completed-op proxy, explicitly NOT a fabricated in-flight flag. A true in-flight signal is the forward marker **AD-930a**. The window field ships ON by default (read-only/computed, not a transitional behavioral flag).

**Decision 3 — one read-only POLLED endpoint on the existing router, not a stream.** `GET /api/crew/presence` is appended to the **existing** `/api/crew` router (`crew.py:32`, no new router) → `{"presence": {agent_id: state}, "count": N}` for crew only. A polled GET is the v1 path (simplest, least-invasive, exactly-once computed server-side) over a new WebSocket channel or riding the fleet avatar-telemetry stream. Tier-2 honest-degrade throughout: `registry None` → `{}`,`0`; `chat_thread_store None` or a meeting-scan failure → no `in_meeting` (agents fall through to `working`/`online`); config-missing → the 90s default.

**Decision 4 — UI: a pure `PresenceDot` + a roster-scoped poll + an optional badge overlay.** A pure no-emoji `PresenceDot` (green `#60c070` / amber-pulse `#f0b060` / blue `#5090d0` / dim `#666680`; `data-presence` + `data-pulse` test hooks; HXI #3 stroke/SVG, HXI #4 motion=state — the amber pulse encodes "working"). A `presence: CrewPresenceMap` store slice + `fetchPresence` (mirrors the `openCrewManifest` fetch+set idiom, minus open/close — presence is ambient data, not a panel) polled every 10s **while `CrewRosterPanel` is open**, a dot beside each `CrewRow` callsign, and an **optional** `presence?` prop on the reusable `AgentAvatarBadge` (omitted = byte-identical original badge, backward-compatible — no existing call site/test changes). v1 only the roster feeds presence; wiring it into every badge surface app-wide needs a global poll → forward marker **AD-930b**.

**Non-goals / forward markers.** No true in-flight "working" signal / new telemetry infrastructure (**AD-930a**). No activity feed / status messages (that is AD-928's `[STATUS]`). No global app-wide presence poll wiring every badge surface (**AD-930b**). No new WebSocket channel, no riding the fleet avatar-telemetry stream. No `degraded`/`away` states (DEGRADED agents are `is_alive`, so they read `online`/`working`). No change to `AgentState`/`LifecycleState`, `BaseAgent`, `AgentMeta`, `ChatThreadStore`, the `/api/crew/roster` or `/api/ontology/crew-manifest` endpoints, `MeetingView`, `ParametricAvatar`/`CrewVRM`, consensus/trust, `IntentMessage`, or `Glyphs.tsx`. No new `EventType`, no new REST router (extend `crew.py`), no new config class (extend `CommunicationsConfig`), no new store method beyond `fetchPresence`. (Verify-first correction: the spec assumed `CommunicationsConfig` used `field_validator`s for the status caps — it does not; the new `presence_working_window_seconds` is a plain field with a sensible default, matching the existing class style, so no invented validator.)

**Tests:** `tests/test_ad930_presence.py` (+9, BF-287 — a real `ChatThreadStore` on `tmp_path` + a real-but-fake `_FakeRegistry` of `_FakeAgent` duck-stubs with controllable `is_alive`/`meta.last_active` + a real `CommunicationsConfig`, direct-`await` of `crew_presence` over a `SimpleNamespace` runtime, no MagicMock at the store/config boundary): `in_meeting`; `working` (just inside 90s); `online` (just outside 90s); `offline` (not alive); non-crew excluded; precedence (`in_meeting` beats `working`); `registry None`; `store None` degrade; count integrity. **+12 Vitest** across three new files — `PresenceDot.test.tsx` (6), `CrewRosterPanel.presence.test.tsx` (3), `AgentAvatarBadge.presence.test.tsx` (3). Backend focused gate 20 (9 AD-930 + 11 AD-924); full Vitest **1237 passed | 1 skipped** (210 files; baseline 1225 + 12, zero regressions); `npm run build` clean. **COMMITTED LOCAL ONLY — NOT pushed** (held for Captain review/push alongside AD-926a). Forward markers: **AD-930a** (true in-flight signal), **AD-930b** (global app-wide badge poll).

### AD-928: Show-your-work status protocol — one `[STATUS]…[/STATUS]` tag family with an optional `final` flag posts a `metadata.kind="status"` message into the agent's task room (reusing the AD-927 participation resolver unchanged), giving a task room the Microsoft-Teams "activity feed" feel; message-only v1 (the AD-928 row of the Task Workspace Rooms epic AD-925 → AD-929, filled after Captain deferral)

**The activity feed of the workspace room.** AD-925 auto-creates the task room, AD-926/927 give it Input + Output folders, AD-929 assembles them into a Teams-channel surface. AD-928 adds the **narration**: as the crew work a task they post meaningful progress into the room and a clear final result, so a human watching sees the work happen and a crisp completion — the Teams activity-feed feel. The mechanism mirrors AD-924/AD-927 exactly: a rank-gated, anti-flooded proactive action tag that posts into the agent's bound task room, plus a `federation.md` standing order that teaches the norm. It is assembly over existing primitives (`ChatThreadStore.append_message`, the AD-927 resolver), not new substrate.

**Decision 1 — ONE `[STATUS]` tag with an optional `final` flag, NOT two tags.** A single paired tag carries both progress narration and the final result: `[STATUS]body[/STATUS]` → `metadata={"kind":"status"}`; `[STATUS final]body[/STATUS]` → `metadata={"kind":"status","status_final":true}`. A new module-level `_STATUS_PATTERN` (`r'\[STATUS(\s+final)?\s*\](.*?)\[/STATUS\]'`, `DOTALL|IGNORECASE`; `bool(m.group(1))` is the final flag, g2 is the body) mirrors `_ARTIFACT_PATTERN`. **Why one tag, not `[STATUS]` + `[RESULT]`:** DRY — a second tag doubles the regex / extractor / rank-gate / cap surface for a semantic that is just a boolean on the same message kind. The activity-feed feel = a stream of `kind="status"` messages plus one distinctly-marked completion (`status_final`); a flag on the same kind gives a UI consumer (AD-928b) exactly what it needs without a second code path. Mirrors AD-923's deterministic `metadata.meeting_end` marker.

**Decision 2 — binding: REUSE `_resolve_agent_task_room(agent.id)` identically.** The proactive turn carries no inherent thread. Bind the status post to the agent's task room with the **existing AD-927 participation resolver, unchanged**: the most-recently-active non-archived thread that has a `task_id` and lists the agent as a participant. No room → honest-degrade (suppress, strip the tag, no post, no crash). This builds on AD-925 (the agent IS a participant of its auto-created room). The resolver takes the agent **id string** (`_resolve_agent_task_room(agent.id)`), mirroring the artifact extractor call exactly.

**Decision 3 — post via `ChatThreadStore.append_message` (message-only).** Post the status as a normal thread message authored by the agent (`role="agent"`). `append_message` is SYNC, returns `ChatThreadMessage | None` (`None` if the thread is missing — built-in honest-degrade), and its `to_dict()` carries `metadata`, so the existing `GET /api/threads/{id}/messages` already surfaces the marker for any future UI consumer (AD-928b). No new store method, no new endpoint.

**Decision 4 — rank gate Lieutenant+, caps `status_max_per_turn=3` + `status_max_bytes=4096`.** Three NEW `CommunicationsConfig` fields after `artifact_max_bytes`: `status_min_rank="lieutenant"` (status narration and artifact output are the two task-room work-output channels — same tier reads cleanly; the whole proactive-action surface is already Lieutenant+, so Lieutenant is the floor of "can take proactive actions at all", and dropping to Ensign would make status the *only* action an Ensign can take), `status_max_per_turn=3` (status is frequent by design — narrate *milestones* — so 3 allows "started / progress / done"; the 4th+ suppressed `rate_limited`), `status_max_bytes=4096` (4 KiB — status lines are short prose, smaller than the artifact 256 KiB; oversize → suppressed `too_large`). The gate is wired INLINE in `_extract_and_execute_actions` immediately after the AD-927 artifact gate, reusing the in-scope `_RANK_ORDER_DM`. The bare opening tag of a rank-gated-out status is later removed by the pre-existing BF-203 hallucinated-tag cleanup — correct, the status path simply never ran (so the gated-out test asserts no-post + no-action, NOT tag survival).

**Decision 5 — UI status-chip render is DEFERRED to forward marker AD-928b.** Rendering `metadata.kind="status"` distinctly in the transcript is not cheap at HEAD: `ChatMessage` has no `metadata`/`kind` field; the thread-message load path renames `body → text` and does not carry `metadata` into the conversation; the render is a heavy inline `messages.map` loop in `ProfileChatTab`; and there is an open live-refresh question (does the room transcript poll for agent-posted messages between Captain turns?). Because the backend persists the marker and the existing `GET messages` endpoint already surfaces it, **AD-928b is a pure presentational follow-up with no backend dependency** (mirrors the epic's backend-first discipline: AD-927 shipped off the existing `ArtifactDrawer`, AD-926 shipped the `InputsList` unmounted).

**Decision 6 — message-only v1; lifecycle transition is forward marker AD-928a.** The roadmap row aspires to tie the final result to the work-item lifecycle (`in_progress → done`). **v1 is MESSAGE-ONLY: no work item is transitioned.** Verified: `grep work_item|current_task|task_id|transition_work_item` over `proactive.py` returns ONLY the two `task_id` references inside the AD-927 resolver — there is no clean current-work-item handle in the proactive context (the same gap AD-925/AD-927 documented). A lifecycle transition is a validated state-machine write (`transition_work_item`, AD-498/BF-72/AD-861) that needs the work-item id, which is not available here; it stays with `CrewTaskExecutor` / a richer binding → **AD-928a** (shares the AD-927a gap).

**Non-goals / forward markers.** No work-item lifecycle transition (**AD-928a**). No UI — no status-chip render, no `ChatMessage` type change, no `ProfileChatTab` edit, no store-slice / live-refresh change (**AD-928b**). No presence layer online/working/in-meeting (**AD-930**). No second tag (`[RESULT]`) — one `[STATUS]` family with a `final` flag. No change to `_resolve_agent_task_room`, `ChatThreadStore.append_message`, the `[ARTIFACT]`/`[GROUP_CHAT]` extractors, `AgentGroupChatService`, the intent bus, `CrewTaskExecutor`, `CrewOrchestrator`, any consensus/trust path, `BaseAgent`, or `IntentMessage`. No new REST route, no new `EventType`, no new store method, no new config class (extend `CommunicationsConfig` only). No binary/attachment status — short text only. **Dormancy:** unlike AD-925's auto-create wiring, `[STATUS]` is NOT gated by `agentic_dispatch.orchestrator_enabled` and does NOT depend on `CrewTaskExecutor` — it fires whenever any Lieutenant+ agent that participates in a task room emits the tag (participation-based binding), so the capability is live the moment a task room with participants exists. **Tests:** `tests/test_ad928_show_your_work.py` (+13, BF-287 — a real `ChatThreadStore` on `tmp_path` behind a `MagicMock(spec=ProbOSRuntime)` shell, `_FakeAgent`/`_Clock` duck stubs, rank driven by injected trust). Focused gate 35 (13 AD-928 + 11 AD-927 + 11 AD-924); blast-radius `-k "proactive or status or thread or chat or group or artifact or standing or manual"` green. **COMMITTED LOCAL ONLY — NOT pushed** (the whole epic AD-925 → AD-929 is held for Captain review; HEAD now 6 ahead of origin/main = AD-924).

### AD-929: Unified workspace view — a collapsible right-hand "Files" rail inside `ProfileChatTab` (Inputs + Outputs), gated to workspace rooms, so a task room reads like a Microsoft Teams channel (Conversation + Files); the surface that completes the Task Workspace Rooms epic (AD-925 → AD-929)

**The assembly AD: bind Conversation + Files into one surface.** AD-925/926/927 delivered the substrate (auto task room) and both file folders (Inputs, Outputs), but the pieces were never assembled — `InputsList` was mounted nowhere, and the artifact Outputs (`ArtifactDrawer`, AD-797) lived only in the `CompactApp` tray, absent from the full HXI where task/group rooms actually open (AD-919 routes a room to `AgentProfilePanel` → `ProfileChatTab`). The Captain's refined goal: "an experience like HR collaborating in Microsoft Teams." Teams maps a project channel to Conversation (the transcript) + Files (shared inputs + produced outputs) + Meet (already built, AD-920 `MeetingView`). AD-929 is frontend-only — every endpoint already exists (`/api/threads/{id}/inputs` AD-926, `/api/artifacts/thread/{id}` + `/content` AD-797) — so **no backend, no pytest, no new REST route, no new EventType, no new store slice**.

**Decision A — layout = a collapsible right-hand "Files" rail inside `ProfileChatTab` (chosen over a tab bar).** `ProfileChatTab` is the **only mount common to both hosts** — `CompactApp` (Yeo tray) *and* `AgentProfilePanel` (full HXI) — so putting the Files surface inside it means the rail travels with the conversation into both hosts with one component and no host-specific layout surgery. The rail keeps the **conversation as the primary column** (Teams defaults a channel to its Conversation) and adds a right rail with two stacked sections — INPUTS (top) over OUTPUTS (bottom) — reusing the HXI right-rail idiom already established by `ArtifactDrawer`. **A tab bar atop the room was rejected:** it hides the conversation when viewing Files (a live collaboration room wants Files co-visible with the transcript), and `AgentProfilePanel` already has a **per-agent** tab bar (Chat | Work | Memory | Profile | Health | Self-image) — a per-thread "Files" tab there is a category error (those tabs describe the *agent*, not the *room*) and it does nothing for `CompactApp`, which has no tab bar.

**Decision B — gate = workspace room only (no rail on a 1:1 DM).** A new **pure** `isWorkspaceRoom(thread, agents)` (`ui/src/components/workspace/isWorkspaceRoom.ts`) returns `true` when `thread.task_id` is set (an AD-925 auto task room — authoritative) **OR** it has **≥2 crew participants** (a group room; AD-917 turns a 1:1 into a group at the 2nd crew participant). The crew-count idiom is the **verbatim `GroupChatHeader:36-40` filter** (exclude the `"captain"` sentinel, resolve each id to its agent via `agents.get`, keep only `isCrew`). Both `task_id` and `participants` are on the client store view `AD791aChatThreadView`, so the gate is a pure function over store data (no fetch). A 1:1 DM (one crew participant, no `task_id`) shows **no rail**.

**Decision C — Outputs = the lighter `ArtifactList`, NOT the full `ArtifactDrawer` (the drawer is never double-mounted).** The new `WorkspaceFilesRail.tsx` composes the **presentational** AD-797 `ArtifactList` fed by a self-contained `fetchThreadArtifacts(threadId)` call, so the full `ArtifactDrawer` stays exactly where it is in `CompactApp` (untouched) and the rail uses a *different, lighter* component. The rail owns its own fetch + local state (exactly like AD-926's `InputsList` + `inputsApi` pattern), with **no coupling** to the global `selectedArtifactId` / `artifactsByThread` slice the standalone drawer owns. Outputs rows are read-only "open" actions — selecting an artifact opens `/api/artifacts/{id}/content` in a new tab (parity with `InputsList`'s open-the-bytes model); rich inline `ArtifactViewer` preview stays the standalone drawer's job (forward marker **AD-929a**). INPUTS composes the AD-926 `InputsList` fed by `fetchThreadInputs(threadId)`. Both fetches are Tier-2 honest-degrade to `[]`.

**Decision D — collapse persisted, default-collapsed on first run.** Collapse state persists under `localStorage probos.workspaceFiles.collapsed` (the `ArtifactDrawer` `'1'`/`'0'`/`null` load/persist idiom with a new key), but **defaults to collapsed on first run** (a thin 28px "FILES" rail with a count badge) — a deliberate divergence from `ArtifactDrawer`'s default-expanded, justified by the primary host (`AgentProfilePanel`) being a 420px floating panel where an expanded 300px rail would crowd the chat; the Captain expands on demand (the panel is resizable). HXI #3: **local inline stroke-SVG glyphs** (`strokeWidth 1.5`, amber `#f0b060` / dim `#666680`, no emoji); `Glyphs.tsx`/`Glyphs.test.tsx` are untouched (the AD-917/919 export-count gotcha). `ProfileChatTab` mounts the rail behind the gate via three small edits (import + 3 selectors + wrap the outer return's column in a row with the rail as a `showWorkspaceFiles && activeThreadId` sibling); the chat column is byte-identical for 1:1 DMs.

**Non-goals / forward markers.** No backend / no pytest (all endpoints exist). No `ArtifactDrawer` change, no `CompactApp` edit (the benign cosmetic double-Outputs in the tray — rail + standalone drawer for a workspace room — is resolved by gating the standalone `<ArtifactDrawer />` to non-workspace threads = forward marker **AD-929b**, not this AD). No inline `ArtifactViewer` in the rail (**AD-929a**). No per-room "Files" tab in the full-HXI panel (**AD-929c**, Teams-truer for the full HXI only). No show-your-work status protocol (**AD-928**, deferred for Captain review). No presence layer (**AD-930**). No task-level upload (**AD-926a**). No new store slice, no `Glyphs.tsx` change, no `AgentProfilePanel` tab-bar change, no `ConsultationWorkspace` bridge (AD-594a substrate stays separate). **Tests:** `+14` Vitest across two new files — `isWorkspaceRoom.test.ts` (6 — `task_id` room → true; ≥2-crew group → true; 1:1 DM → false; captain-only → false; non-crew → false; undefined → false) + `WorkspaceFilesRail.test.tsx` (8 — Inputs renders; Outputs renders; `fetchThreadInputs`/`fetchThreadArtifacts` called with the threadId; Outputs row opens `/api/artifacts/{id}/content`; collapse persists `'1'` + `data-collapsed`; expand-from-`'0'` + default-collapsed-first-run; no-emoji guard) — real fixtures, `importOriginal` partial mocks of the api modules (BF-287). Full suite **1225 passed | 1 skipped (207 files)** = baseline (1211) + 14, 0 regressions; `npm run build` (tsc -b + vite) clean. **COMMITTED LOCAL ONLY — NOT pushed** (the whole epic is held for Captain review; HEAD now 5 ahead of origin/main = AD-924).

### AD-927: Outputs folder — a crew agent deposits a versioned `text/markdown` artifact into its task room via an `[ARTIFACT name="…"]…[/ARTIFACT]` proactive tag, participation-bound to the AD-925 room and stored in the AD-797 `ArtifactStore` (3rd AD, completing the Task Workspace Rooms foundation AD-925 → AD-927)

**The Output folder of the workspace room.** AD-925 auto-creates the task room and AD-926 surfaces its Input pane; AD-927 adds the missing **agent-facing write path** — a way for a crew agent, during its proactive turn, to say "this is my output" and have it land in the room. The Output substrate already exists end-to-end (AD-797 `ArtifactStore` is `thread_id`-keyed + versioned, bytes live in the content-addressable `AttachmentStore`, and the `ArtifactDrawer` UI already lists + views a thread's artifacts off `activeThreadId`), so AD-927 is assembly, not new substrate: a tag, a regex, an extractor, a rank gate — mirroring the AD-924 `[GROUP_CHAT]` shape.

**Decision A — the verified two-call write, no phantom helper.** There is NO one-call "write bytes + register" helper on `ArtifactStore`. The extractor computes `hashlib.sha256(body).hexdigest()`, writes the UTF-8 body to `attachment_store.write(hash, blob, "text/markdown", origin="agent_artifact")` (async, idempotent, `agent_artifact` = the AD-797 dedicated, most-durable, never-age-reaped origin), then registers it with `artifact_store.add_version(thread_id=room.id, name, content_hash, mime, size_bytes, created_by)` (SYNC, auto-versions `MAX+1`, BF-324 `BEGIN IMMEDIATE`). This is exactly the shape the REST `add_artifact` endpoint proves (client uploads bytes, then POSTs a pre-existing `content_hash`). The body is text in the content-addressable store — NOT inline in an `IntentMessage` (the BF-265/#636 lesson) — with a 256 KiB cap as defense-in-depth.

**Decision B — participation-based binding (the crux), richer binding deferred (AD-927a).** A proactive turn is NOT inherently tied to a chat thread (the agent's proactive think posts to the Ward Room, not the task room), and the agent's current in-flight work-item is not cleanly available in the proactive context (noted in AD-925). The honest, minimal, faithful binding is **participation-based**: `_resolve_agent_task_room` returns the most-recently-active non-archived thread that has a `task_id` AND lists the agent as a participant (`list_threads(include_archived=False)` is already `last_active_at DESC`, so the first match wins; participants is a JSON column, filtered in Python). This builds directly on AD-925 — the auto-created task room sets the crew child-assignees (including this agent) as participants. No resolvable room → honest-degrade (suppress, strip the tag, write nothing, never crash). A richer binding off the agent's current work item is the forward marker **AD-927a**.

**Decision C — Lieutenant+ rank gate + anti-flood, all defaulted.** Three NEW `CommunicationsConfig` fields next to the AD-924 `group_chat_min_rank`: `artifact_min_rank="lieutenant"` (producing a work artifact is a normal deliverable — a lower social bar than *convening* a multi-agent room at Commander+; Ensigns cannot think proactively anyway), `artifact_max_per_turn=3`, `artifact_max_bytes=262144`. The gate is wired INLINE in `_extract_and_execute_actions` immediately after the group-chat gate, reusing the in-scope `_RANK_ORDER_DM`. The per-turn and byte caps are honest-degrade (oversize / overflow → skip that one, suppressed action, never raise). The bare opening tag of a rank-gated-out artifact is later removed by the pre-existing BF-203 hallucinated-tag cleanup — correct, the artifact path simply never ran.

**Non-goals.** No new artifact UI (the AD-797 `ArtifactDrawer` already lists + views off `activeThreadId`; a drawer mount inside a dedicated 3-pane room view is AD-929); no binary `[ARTIFACT]` upload in v1 (`text/markdown` only — binary artifacts keep the existing AttachmentStore-upload + `POST /api/artifacts` path); no richer work-item binding (AD-927a); no show-your-work status protocol (AD-928); no unified 3-pane (AD-929); no new REST route, no new `EventType`, no new `ArtifactStore`/`AttachmentStore` method; no change to `BaseAgent`, `IntentMessage`, the intent bus, or any consensus/trust path. **Tests:** `tests/test_ad927_outputs_folder.py` (+11, BF-287 — real `ArtifactStore` + real `FilesystemAttachmentStore` + real `ChatThreadStore` behind a `MagicMock(spec=ProbOSRuntime)` shell). Focused gate 30 (11 AD-927 + AD-925 + AD-924); blast-radius `-k "proactive or artifact or thread or chat or group or attachment"` 996 passed (1 pre-existing intermittent `test_browse_threads_sort_recent` timestamp flake, confirmed independent of AD-927 via stash + repeat-run). Local-only commit (the Captain reviews the epic before it goes public).

### AD-926: Inputs folder — a read-only `GET /api/threads/{thread_id}/inputs` endpoint surfacing a task room's attached files as an honest union of an additive `WorkItem.metadata["input_attachments"]` convention + AD-916 message attachments (2nd AD of the Task Workspace Rooms epic AD-925 → AD-929)

**The Input folder of the workspace room.** AD-925 makes a task fan-out auto-create one group chat with `chat_threads.task_id` set; AD-926 surfaces that room's **Input folder** — the files the crew use/process — as a read-only backend endpoint plus a thin self-contained UI list. Read-only because inputs are context, never edited in the room.

**Decision A — surface an honest union of two task-scoped sources, because a `WorkItem` has no native attachment slot.** A `WorkItem` (`workforce.py:581`) carries only a generic `metadata: dict` — there is no `attachments` / `content_hash` field (grep returns zero). So v1 returns, both scoped to a room whose `task_id` is set: (1) the **authoritative** additive convention `WorkItem.metadata["input_attachments"] = [{content_hash, mime, filename}]` (`source="task"`), and (2) the **real-today** AD-916 message attachments `msg.metadata["attachments"] = [{content_hash, mime}]` aggregated across the room's messages (`source="message"`). The two are merged and **de-duplicated by `content_hash`, task-level winning** (then message arrival order). Being honest: the work-item source is genuinely useful only once a population path exists; the message source makes the endpoint return live data the day it ships.

**Decision B — `input_attachments` is an additive convention, no schema change, population deferred (AD-926a).** The convention rides the existing free `metadata` dict (already serialized by `to_dict`), so the spec IS the migration — no `WorkItem` field, no DB change. v1 DEFINES and READS the contract; wiring a real task-creation/seed flow that WRITES it is a forward marker (**AD-926a**). The work-item source is therefore a defined-but-dormant seam until population lands — recorded honestly rather than presented as fully live.

**Decision C — honest-degrade at every seam, never raise into the response.** `get_thread` / `list_messages` are sync, `get_work_item` is async, `work_item_store` may be `None`, and `attachment_store` is a `@property` read via `getattr`. A non-task thread (`task_id` unset) returns `inputs: []`; a missing thread → 404; a `None` work-item store or a failed `get_work_item` surfaces message attachments only (Tier-2 log + continue); `size` is best-effort from `FilesystemAttachmentStore.size` (async) with `FileNotFoundError` / an absent store degrading to `size=None`. The response shape is exactly `{thread_id, task_id, inputs:[{content_hash, mime, filename, size, source}]}`.

**Decision D — reuse the existing byte endpoint; ship the UI as a self-contained drop-in.** Downloads reuse `GET /api/chat/attachments/{content_hash}` (`chat.py:985`) — no new blob store, no new content endpoint. The UI is a NEW `ui/src/components/inputs/{inputsApi.ts, InputsList.tsx}` mirroring the artifacts pane (stroke-SVG icons, no emoji per HXI #3, read-only rows, presentational — the caller passes `inputs`). Mounting it into the room / 3-pane surface is **AD-929**, not wired here, so the component stays trivially testable and AD-929 owns fetching + layout.

**Non-goals.** No write / edit / delete / upload of inputs (read-only v1); no population of `input_attachments` (AD-926a); no Output pane / artifacts (AD-927); no unified 3-pane view or mounting into the room / canvas / `GroupChatHeader` (AD-929); no new blob store or content endpoint; no change to the `AttachmentStore` Protocol, the `WorkItem` schema, or the AD-925 create path. **Tests:** `tests/test_ad926_inputs_folder.py` (+10, BF-287 — real `ChatThreadStore` + real `WorkItemStore` + real `FilesystemAttachmentStore`, direct-await `list_thread_inputs` with a `SimpleNamespace` runtime stub) + `ui/src/components/inputs/__tests__/InputsList.test.tsx` (+3, incl. a no-emoji guard). Focused gate 52; blast-radius `-k "thread or chat or attachment or workforce or input"` 746 passed; vitest 3 passed; `npm run build` clean. Local-only commit (the Captain reviews the epic before it goes public).

### AD-925: Auto-create the task-linked workspace room — `CrewTaskExecutor` opens ONE group chat for a ≥2-crew fan-out (FOUNDATION of the Task Workspace Rooms epic AD-925 → AD-929)

**The room IS the workspace.** When `CrewTaskExecutor` (AD-859) fans a parent `WorkItem` out to ≥2 distinct crew agents, AD-925 opens one task-linked group chat so the collaborators share a room while they work. A new `_maybe_open_task_room(parent_id, children)` fires at the top of `run`, immediately after the `if not children: return []` guard — the room exists BEFORE the children execute. This is the assembly-of-existing-primitives foundation the Captain ruled as Option A (extend the chat-thread + `ArtifactStore` world, NOT bridge `ConsultationWorkspace`).

**Decision A — reuse the AD-918 `create_group_chat` path, no parallel creation.** The helper calls the existing synchronous `runtime.agent_group_chat.create_group_chat(creator_id=…, title=…, participants=…, task_id=parent_id)`. Reusing it means the AD-918 per-agent cooldown + sliding-window cap + `_is_crew` participant resolution all apply unchanged — there is no second thread-creation path to keep in sync, and the storm guard is honored for free. Both `runtime.agent_group_chat` and `runtime.chat_thread_store` are public handles the executor reaches via its existing `self._runtime` (no Demeter violation).

**Decision B — the creator is the first crew child-assignee, NOT a synthetic system id.** `create_group_chat` rejects a non-crew creator (`_is_crew(creator_id)` → `error="not_crew"`) and ALWAYS auto-adds the creator as a participant. A synthetic coordinator id (e.g. `GROUP_CHAT_COORDINATOR_ID`) would therefore be both rejected AND, if accepted, would pollute the participant list. Resolution: use the first stable-sorted crew child-assignee as `creator_id` — it passes `_is_crew`, it is a participant anyway, and its `_rate_ok` budget is the natural anti-storm guard. So the final participants are exactly the crew child-assignees, and NO service change is required. A `system_created` bypass was deliberately NOT added (it would either re-introduce a non-crew participant or fork the create path — both worse than reusing a real crew creator).

**Decision C — idempotency via a new `task_id` filter on `list_threads`.** A 3-line additive `task_id` filter on `ChatThreadStore.list_threads` (mirroring the existing `project_id` clause exactly; the `task_id` column already exists since AD-791a) makes the lookup queryable. Before creating, the helper checks `list_threads(task_id=parent_id, include_archived=True, limit=1)` — if a room exists, it does nothing. A retry / re-run of the same parent finds the room and stops, so there is exactly one room per task. This is the queryable primitive AD-926/927/929 will also reuse.

**Decision D — default-OFF, doubly gated.** A new `GroupChatConfig.auto_task_room_enabled: bool = False` gates the whole helper (transitional flag, wave-10 convention #14 + the AD-918 create-storm history; the Captain flips it on after reviewing AD-925..927). The path is doubly gated: even with this flag ON, the crew pipeline that drives the executor (`agentic_dispatch.orchestrator_enabled`) itself ships OFF, so a zero-config boot creates no task rooms. The ≥2-crew gate counts DISTINCT `child.assigned_to` that resolve to crew via the shared public `is_crew_agent(agent, None)` predicate (None-guarded ontology); `< 2` distinct crew → no room. Every branch honest-degrades (Tier-2: log + return) so a disabled flag / missing collaborator / unwired substrate never breaks the fan-out.

**Non-goals.** No Input pane (AD-926); no Output/artifact tag (AD-927); no status protocol (AD-928); no UI (AD-919 already surfaces the room via `metadata.created_by_agent`); no `AgentGroupChatService` change; no `system_created` bypass; no new `EventType`; no change to `CrewOrchestrator` / `maybe_dispatch_crew` / `agentic_dispatch.orchestrator_enabled`; no `first_message` posted (that is AD-928's job). **Tests:** `tests/test_ad925_auto_task_room.py` (+8, BF-287 — real `WorkItemStore` + real `ChatThreadStore` + real `AgentGroupChatService` over the SAME store instance on both the service and the runtime stub; `_Fake*` duck stubs only). Focused gate 50 (8 AD-925 + AD-918 + AD-791 + AD-859); blast-radius `-k "crew_executor or crew_orchestrator or thread or chat or group or workforce"` 683 passed. Local-only commit (the Captain reviews the epic before it goes public).

### AD-924: Agent-facing group-chat trigger + crew awareness — direct `create_group_chat` dispatch from a proactive `[GROUP_CHAT]` tag, Commander+ rank policy in `CommunicationsConfig`, a dedicated manual (follow-up to the closed Ad-hoc Crew Collaboration epic AD-913 → AD-923)

**The emitter AD-918 deferred.** AD-918 wired the group-chat service as a bus LISTENER (`create_group_chat` subscriber) with no EMITTER — nothing parsed agent proactive output into a create, and the crew were never told the capability exists, so a crew agent literally could not open a group chat (only the Captain could, via the AD-917/919 HXI). AD-924 adds the missing three layers, mirroring exactly how the Ward Room DM capability was made usable (proactive tag extractor → standing-order instruction → crew manual).

**Decision A — dispatch is a DIRECT, synchronous `create_group_chat` call, NOT the bus.** A new `ProactiveCognitiveLoop._extract_and_execute_group_chats` (mirroring `extract_and_execute_dms`) calls `runtime.agent_group_chat.create_group_chat(creator_id=…, title=…, participants=…, first_message=…)` directly. Rationale: this matches how proactive dispatches EVERY other action (DMs → `ward_room.create_thread`, notebooks → `_records_store`, endorsements → `ward_room_router`) — direct service calls, not bus broadcasts — and `create_group_chat` is synchronous with a typed `GroupChatCreateResult` (no `IntentMessage` construction, no async round-trip). Critically it HONORS the AD-918 storm guard: `create_group_chat` runs `_rate_ok` itself, so the per-agent cooldown + 5/hour sliding-window cap apply on the direct path identically to the bus path — so there is NO new cooldown dict in `proactive.py` (DRY; the service owns the limiter). The bus `handle_intent` path is left UNTOUCHED as the forward marker for future decomposer/NL discovery.

**Decision B — the tag is `[GROUP_CHAT title="..." @cs,@cs] ... [/GROUP_CHAT]`, one module-level regex.** `title="..."` is quoted so it tolerates spaces; the participant blob is split on `[,\s]+`, each ref `lstrip('@')`, and passed raw (the service resolves each by agent_id OR callsign, dropping non-crew/unresolvable refs Tier-2). Requiring ≥1 named participant means the auto-added creator guarantees the 2+-crew floor; a title-only tag does not match and degrades cleanly (no room). The matched tag is stripped from the returned text regardless of outcome.

**Decision C — the rank POLICY lives in `CommunicationsConfig`, not `GroupChatConfig`.** A NEW `CommunicationsConfig.group_chat_min_rank: str = "commander"` sits next to `dm_min_rank`/`recreation_min_rank` (the comms rank policy), gated INLINE in `_extract_and_execute_actions` immediately after the DM block (reusing the in-scope `_RANK_ORDER_DM`), exactly mirroring the DM gate. `GroupChatConfig` keeps ONLY the AD-918 rate limits — the capability's spam budget and the comms rank policy are distinct concerns. Default `"commander"` gives Commander+ out of the box, config-overridable.

**Decision D — a dedicated `config/manuals/group-chat.md`, NOT an extension of `ward-room.md`.** The Captain ruling for this epic was explicit: the substrate is `ChatThreadStore` (AD-791), NOT the Ward Room — folding group chat into `ward-room.md` would conflate two distinct communication fabrics. `recreation.md` is the established focused-single-capability-manual precedent, and `seed_manuals` globs `*.md` so the new file is drop-in with zero wiring. The manual covers the `[GROUP_CHAT]` tag AND the AD-920→923 meeting experience (Start Meeting, avatar gallery, sequenced voice, push-to-talk). The standing order is a new `### Group Chat (Ad-Hoc Collaboration)` block in `federation.md`'s `## Communications` section (ASCII-only per the Encoding Safety rule) carrying the anti-flood guardrail ("2+ crew genuinely working a task, NOT idle proactive thinking, one room per collaboration").

**Decision E — `task_id` is omitted in v1 (forward marker).** The proactive loop has no clean handle on the agent's current work-item id (per AD-918: `work_item_id` reaches an agent only via `intent.params["work_item_id"]` during work-item dispatch; there is no persistent `current_task_id`). v1 omits `task_id`; `create_group_chat` already makes it optional. The seam for a future AD that threads the active work-item id into the proactive context is noted.

**Non-goals.** No UI (AD-917/919 cover the Captain-facing surface); no change to the AD-918 service internals (AD-924 only *calls* `create_group_chat`); no agent-to-agent auto-reply loop (the extractor opens the room + posts the optional first message, nothing more — an agent-created room has no Captain post, so AD-914 fan-out does not auto-run); no consensus gate (opening a room is reversible + low-risk — Safety Budget axiom; the cooldown + cap are the spam control, matching AD-918's `requires_consensus=False`); no bus/`handle_intent` change; no decomposer/NL exposure. **Tests:** `tests/test_ad924_group_chat_trigger.py` (+11, BF-287 — real service + real store + real `CallsignRegistry`, `MagicMock(spec=ProbOSRuntime)` shell only, all room assertions against the real store). Focused gate 27 (11 + 16 AD-918); blast-radius `-k "proactive or group or thread or chat or standing or manual"` 978 passed.

### AD-923: Meeting presence + speaking indicator + transcript writeback — wrapper-treatment speaking highlight, presence header, deterministic `role:'system'` end marker (CLOSES the Ad-hoc Crew Collaboration epic AD-913 → AD-923)

**Final AD of the ad-hoc crew-collaboration epic.** The visual + record completion of the "video call": the active speaker's avatar lights, the others dim, a presence header shows who's in, and ending the meeting writes a transcript marker back to the thread. Purely-additive FRONTEND — NO backend change, NO new pytest (the `POST /messages` endpoint, the store method, and the `system` role enum all already exist).

**Decision A — the who's-speaking highlight is a WRAPPER treatment, `CrewVRM` untouched.** `MeetingView` gains ONE optional prop `speakingAgentId?: string | null` (kept optional so AD-920's `MeetingView.test.tsx`, which renders `<MeetingView threadId=… />`, stays green); `ProfileChatTab` passes the value already destructured from `useMeetingVoice` (AD-921) and already live at the mount (it feeds the AD-922 mic echo gate). Per-slot it threads `speaking={id === speakingAgentId}` + `someoneSpeaking={speakingAgentId != null}`; `AvatarSlot` applies an amber ring (`boxShadow`) + `meetingSpeakingPulse` `@keyframes` + full opacity on the speaker, `opacity:0.5` dim on non-speakers while someone speaks, and a neutral idle state — all on the INNER avatar container `<div>`, never on `CrewVRM`. `data-speaking` on the slot root + `data-dim` on the inner container give jsdom-safe assertions (the animation itself is browser-real; jsdom ignores it). HXI #4 motion = state, amber active, co-located `<style>` idiom (mirrors `CrewCollaborationPanel.tsx:186`), no emoji.

**Decision B — presence is a count header + a Captain-present chip, no new code for join/leave.** A `meeting-presence` header renders `{crewIds.length} in meeting` + a `captain-present` chip (`You (Captain)` — the Captain is the viewer, excluded from the gallery, always present). Join/leave needs no new wiring: the gallery already re-renders on `thread.participants` change (AD-920).

**Decision C — transcript writeback is a DETERMINISTIC, client-side `role:'system'` marker — no LLM, no backend.** A thin `threadApi.appendMessage(threadId, {author_id, role, body, metadata?})` wrapper (Tier-2 honest-degrade → `null`) POSTs the EXISTING `/api/threads/{id}/messages` endpoint. In `GroupChatHeader.handleToggleMeeting`, on END ONLY and BEFORE `setMeetingActive(false)`, it appends `"Meeting ended — N participant(s): <callsigns>."` with `role:'system'`. `role:'system'` is already a valid enum value (`threads.py:71`) and SKIPS the AD-914 `role=="captain"` fan-out gate (`threads.py:291`) — no agent dispatch. Honest-degrade: a `null`/failed append NEVER blocks ending the meeting (the `await` proceeds to clear the flag regardless). The marker persists server-side and appears on next thread load — an immediate in-transcript echo would have to lift `ProfileChatTab`'s local message state, deferred to AD-923-3.

**Decision D — the marker carries crew count + callsigns, not a start time.** `meeting_started_at` is not persisted (AD-920 stores only the boolean flag), so a deterministic v1 marker uses what IS available — participant count + callsigns — rather than fabricating a duration. Start-time persistence is AD-923-2.

**Boundaries.** NO agent raise-hand / turn-request primitive (the lit avatar IS the turn signal). NO LLM-generated summary (AD-923-1). NO live in-transcript echo (AD-923-3). NO `meeting_started_at` / backend change / new store method. NO `CrewVRM` change, NO audio recording, NO full-duplex barge-in (AD-922 forward marker), NO federation, NO new config, NO `Glyphs.tsx` change.

**Consequence.** +14 Vitest across 3 new files (`MeetingView.speaking.test.tsx` 6 — lit speaker via `data-speaking`, dimmed non-speakers via `data-dim`, neutral idle, presence count, Captain-present chip, no-emoji; `GroupChatHeader.transcript.test.tsx` 4 — End appends a `role:'system'` marker AND clears `meeting_active`, append fires before clear, Start does not append, no-emoji on the composed body; `threadApi.appendMessage.test.ts` 4 — POST endpoint/method/serialized `role:'system'` body, dict-on-ok, `null` on `!res.ok`, no-emoji). One-line completion of AD-920's `GroupChatHeader.meeting.test.tsx` `threadApi` mock (adds `appendMessage: vi.fn()`) since the End path now calls it. Full UI vitest green (1208 passed / 1 skipped, +14); `npm run build` (tsc -b + vite) green.

**Epic complete.** AD-923 closes the **Ad-hoc Crew Collaboration (group chat → meeting)** epic (AD-913 → AD-923). Phase 1 (text): participant management, group fan-out, turn-taking facilitator, file sharing, group-chat UI, agent-initiated chats, visibility + join (AD-913 → AD-919). Phase 2 (meeting voice + avatars): meeting mode + avatar gallery, sequenced meeting voice, Captain voice input, presence + speaking indicator + transcript writeback (AD-920 → AD-923). The Captain can create/join a group chat, promote it to a live meeting, see/hear the crew speak in turn with the speaker lit, talk back by voice, and end the meeting with a transcript marker written back to the thread. Forward markers: AD-923-1 (LLM summary on End), AD-923-2 (persist `meeting_started_at` for start-time + duration), AD-923-3 (immediate in-transcript echo), agent raise-hand / explicit turn-request, full-duplex barge-in.

### AD-922: Captain voice input to the meeting — push-to-talk STT → AD-914 group fan-out, echo-gated on `speakingAgentId`

**Context.** AD-921 made the crew speak their replies; the meeting was one-way (Captain types, crew talk). AD-922 closes the loop: the Captain TALKS, STT transcribes, and the transcript goes through the AD-914 GROUP fan-out — a two-way voice "call."

**Decision (purely-additive frontend; no backend change).** The genuine net-new is a meeting-scoped push-to-talk affordance — NOT a routing change — because `sendText` (`ProfileChatTab.tsx`) ALREADY self-routes to the group via the AD-917 branch (≥2 crew → `POST /api/threads/{id}/messages`). Three pieces: (1) NEW pure-DI hook `ui/src/audio/useMeetingMic.ts` — arms `armTransformersStt` (BF-301; not the deprecated `whisperStt`) for ONE VAD-bounded utterance, a one-shot `onTransformersTranscript` listener torn down before `submit` on every exit path (BF-319), gate inputs via live refs (BF-292), opens no new mic (taps the app-wide `startVoiceActivity` stream), reuses `getMicPermissionState`/`onMicPermissionState`, honest-degrades to typing. (2) NEW presentational `MeetingMicButton.tsx` — local inline mic SVG (HXI #3; `Glyphs.tsx` untouched). (3) EDIT `ProfileChatTab.tsx` — mount the hook with `submit: sendText`, render the button gated on `meetingActive && supported`.

**Echo gate (the load-bearing safety call).** The mic refuses to arm while ANY agent speaks, using `speaking = useMeetingVoice.speakingAgentId != null` — the meeting-wide signal, NOT BF-300's `ttsActiveRef` (host-filtered: `event.agent_id !== agentId` → skip, so it misses non-host speakers in a multi-agent meeting). This stops the Captain's mic transcribing the crew's TTS. Full-duplex barge-in (Captain interrupting agents mid-utterance) is explicitly DEFERRED to AD-923.

**Boundaries.** NO new dispatch branch / parallel POST path (`sendText` self-routes). NO change to agent voice-out (AD-921 `useMeetingVoice`/`meetingVoice.ts` consumed read-only). NO new STT engine / VAD / `getUserMedia` (reuse `transformersStt` + the app-wide mic; `voiceActivity.ts`/`transformersStt.ts`/`whisperStt.ts` untouched). NO change to the conversation/wake-word 1:1 wiring (`conversationController.ts`, `IntentSurface.tsx` STT — no AD-705a regression). NO presence/raise-hand/transcript-writeback (AD-923). NO backend change (the `POST /messages` endpoint already exists).

**Consequence.** +17 Vitest (`ui/src/audio/__tests__/useMeetingMic.test.tsx` 10 — `renderHook` + mocked `transformersStt`/`speechInput`/`wakeWord`, fake `submit`: transcript→submit-with-trimmed-text, arm/disarm one-shot, not-armed when meeting inactive / while an agent speaks / when SR unavailable / when mic denied, second-toggle cancels, empty-transcript no-submit, one-shot listener torn down; `ui/src/components/profile/__tests__/MeetingMicButton.test.tsx` 6 — render/aria/disabled/muted/no-emoji; `ui/src/components/profile/__tests__/MeetingMic.routing.test.ts` 1 — AD-917 `routeSend` mirror proving the transcript POSTs to `/api/threads/{id}/messages`, not `/api/agent/{id}/chat`). Full UI vitest green (1194 / 1 skipped); `npm run build` green. (Implementation authored by the Builder subagent; completed + tested + committed by the orchestrator after the subagent crashed pre-test/pre-commit — diff reviewed, gates re-run.)
...
**Decision A — persist `metadata.meeting_active` via a scoped, additive writer (NOT a generic metadata PATCH).** New `ChatThreadStore.set_meeting_active(thread_id, active)` does a `BEGIN IMMEDIATE` read-modify-write that MERGES the JSON `metadata` column, mirroring `set_title(lock=True)`: `active=True` sets `meeting_active=True`, `active=False` deletes the key (clean "not in a meeting"); sibling keys (`created_by_agent` AD-918, `title_locked` AD-794) are preserved; returns the updated `ChatThread` or `None`. A new `UpdateThreadRequest.meeting_active: bool | None` plus an EARLY branch at the top of the `PATCH /api/threads/{id}` handler routes the flag and returns `to_dict()` (404 on missing). Scoping to one flag is **Defense-in-Depth** — a generic arbitrary-metadata PATCH would let a client clobber `created_by_agent`/`title_locked`. It survives reload and is visible to other surfaces (AD-919 `GroupChatListPanel`, the future AD-921/922/923 voice path).

**Decision B — `MeetingView` gallery iterates `thread.participants`** (crew only, excludes the `"captain"` sentinel), one `AvatarSlot` per participant: `<Canvas><CrewVRM …/></Canvas>` when the agent has a `vrm_url`, else the fallback. Reuses `CrewVRM` per participant (the roadmap's prescription) and the proven `CrewAvatarPopout` `showVRM ? CrewVRM : fallback` shape.

**Decision C — VRM-absent fallback is `AgentAvatarBadge`, NOT `ParametricAvatar`.** `.vrm` binaries are operator-provided/gitignored, so CI and most dev envs have **zero** `.vrm` — nearly every slot degrades. `AgentAvatarBadge` is a pure `<span>` (no WebGL, no R3F), so the gallery renders cleanly with no assets; `ParametricAvatar` is itself a 3D Canvas (N of them = N WebGL contexts) — wrong fallback at gallery scale. Two triggers: no `appearance.vrm_url`, or `CrewVRM.onLoadError` fires.

**Decision D — Start/End Meeting toggle lives in `GroupChatHeader`** (reuses its `thread`/`agents`/`setChatThread` wiring + `crewParticipants`), shown when `crewParticipants.length >= 1`, using a **local inline stroke-SVG video glyph** (HXI #3) — NOT a `Glyphs.tsx` export, avoiding the `Glyphs.test.tsx` export-count bump (the AD-917/919 gotcha). Amber when active, `aria-pressed` reflects the flag.

**Decision E — `MeetingView` mounts inside `ProfileChatTab`** directly below the header, gated on a reactive `meetingActive` selector over `metadata.meeting_active`. The chat is hosted per-crew-agent (`ProfileChatTab agentId=…`); the gallery iterates ALL participants (not just the host), and because the flag is persisted on the SHARED thread, opening the same group thread from any participant's profile shows the same meeting + gallery. A unified host-independent meeting surface is an AD-923 concern — not built here. No new overlay/modal/route.

**Decision F — telemetry binding via `useFleetAvatarTelemetry`** (frames → the shared `setAvatarTelemetryFrame` sink, fan-out by `agent_id`) so the stream is live while the meeting is open, independent of whether `CognitiveCanvas` is mounted (idempotent double-sink is benign). v1 per-avatar `AgentSignals` come from the existing **`deriveAgentSignals`** — NO telemetry-payload→signals converter (that's AD-921/923; the populated `avatarTelemetry` map is the forward binding those consume).

**Build finding — `tsc -b` caught type drift the editor language service missed.** `appearance` is a field of `AgentProfileData`, NOT the store's base `Agent` (which also lacks `department`). `MeetingView` therefore reads appearance via a narrow runtime-field cast (the established `GroupChatHeader` `department` precedent) and casts the `deriveAgentSignals` store argument exactly like `voice.ts:323` (`useStore.getState() as unknown as Parameters<typeof deriveAgentSignals>[1]`). The editor reported no errors on both; only the strict project build surfaced them — a reminder to always run `npm run build`, not just trust the language service.

**Tests:** +8 pytest (`tests/test_ad920_meeting_active.py`, real `ChatThreadStore` on `tmp_path` + real FastAPI router via `TestClient`, BF-287) + 17 Vitest across 3 files (`MeetingView.test.tsx` 8, `GroupChatHeader.meeting.test.tsx` 5 SEPARATE file, `threadApi.meeting.test.ts` 4; R3F/`CrewVRM`/fleet-hook mocked per the `CrewAvatarPopout.test.tsx` precedent — no WebGL, no `.vrm`; no-emoji guard each). Backend §1 gate green (43); full UI suite 1159 pass/1 skip; `npm run build` green; thread/chat blast radius green (468).

### AD-919: Group-chat visibility surface + join — focused `GroupChatListPanel` over the dormant LeftRail

**Context.** AD-913→918 built the full group-chat backend + in-chat UI, including agent-initiated chats tagged `metadata.created_by_agent`. But the Captain had no surface to *discover* group chats — especially the ones agents started on their own — and no one-click way to join them. The roadmap framed AD-919 as "wire the dormant LeftRail (AD-719b) into a live group-chat list."

**Decision A — a focused `GroupChatListPanel`, NOT the LeftRail wire.** `LeftRail.tsx` is built but not imported by `App.tsx`; its `LeftRailThread = {thread_id, title, is_dm?}` has no `participants`/`metadata`/`created_by_agent`/Join concept, and wiring it into the root flex is a persistent-left-column layout reflow — high blast radius for the last Phase-1 AD. Delivering through it would force extending its props/types/render (breaking its "self-contained presentational shell" contract + its vitest tests). So the full LeftRail parent-wire stays forward marker **AD-719b-parent-wire** (default-flip **AD-719b-2**), untouched; AD-919 ships a new self-contained panel surfaced by a `TopNav` toggle (NotebooksPanel precedent — open-flag self-gate + NavButton + unconditional mount), the smallest additive `App.tsx`/store edit.

**Decision B — frontend-only, no backend change.** `GET /api/threads` already returns `metadata` (incl. AD-918 `created_by_agent`), `participants`, `title`, and `task_id` via `ChatThread.to_dict()`; the store's `AD791aChatThreadView` already carries `metadata?`. The group-vs-1:1 split is a pure client predicate, so no `routers/threads.py` change, no new endpoint, no pytest.

**Decision C — group predicate + the Captain sentinel.** A thread is a group chat when `metadata.created_by_agent` is truthy OR it has ≥2 crew participants (id ≠ literal `"captain"` and `agents.get(id)?.isCrew`). Join calls `addParticipant(threadId, "captain")` via the single `CAPTAIN_PARTICIPANT_ID` constant — the v1 sentinel, verified consistent across AD-914 (the `is_crew_agent` fan-out gate excludes it), AD-917 (`GroupChatHeader` strips it), and era-4/5 (Captain posts use `author_id="captain"`). Adding `"captain"` does not change the crew count (it is excluded), so it never spuriously trips the AD-914 ≥2-crew gate. (era-5 note: a future canonical captain DID / `is_captain()` helper makes this a one-line swap.)

**Decision D — open-on-click routes per-host via `threadIdByAgent`, not `activeThreadId`.** `ProfileChatTab` resolves its thread from `threadId ?? s.threadIdByAgent.get(agentId)`, and the chat tab is mounted inside one crew agent's profile panel — so an AD-917 group chat is always hosted in a crew "host" agent's tab. Open-on-click therefore picks host = first crew participant, then `setThreadForAgent(host, thread.id)` + `openAgentProfile(host)`. Wiring the row to the store's top-level `setActiveThread`/`setChatThread` would be a **silent no-op** (`ProfileChatTab` ignores them for thread selection). If no crew host resolves (agents not yet hydrated) the row still renders but the open is a Tier-2 no-op.

**Decision E — keep `Glyphs.tsx` untouched.** Reuse the shared `UserPlus` glyph for Join + a local inline stroke-SVG header glyph (the LeftRail `GlyphAgents` precedent), so `Glyphs.tsx`/`Glyphs.test.tsx` (the export-count assertion) need no change.

**Boundaries.** AD-919 is the LAST Phase-1 AD — no meeting/voice/VRM avatars (Phase 2, AD-920+), no `LeftRail.tsx` change, no `App.tsx` layout reflow, no backend change, no `Glyphs.tsx` change, no consensus gate (Join is a reversible Captain-authority participant add — Safety-Budget / Minimal-Authority). Un-joined agent-created chats sort to the top (HXI #9). **+10 Vitest** (`GroupChatListPanel.test.tsx`, real store + `vi.mock` of `threadApi` per BF-287, no-emoji guard); full UI suite 1142 pass / 1 skip + `npm run build` green.

### AD-918: Agent-initiated group chats — `create_group_chat` intent + bare-callable handler + rate-limited service

**Context.** AD-913→917 gave the Captain a full group-chat surface, but every chat was still Captain-seeded: `ChatThreadStore.create_thread` was only called by the 1:1 default-thread path and the REST handler, and AD-914 fan-out fires only on a Captain post (`role=="captain"`). A crew agent mid-task had no way to say "Bones and I need to coordinate — open a room." The AD-719a Ward Room contract froze this ("agent-to-agent without a Captain seed deferred to AD-719a-2"). AD-918 lifts that deferral on the **ChatThreadStore** substrate (the Captain ruling — NOT Ward Room).

**Decision.** A governed `create_group_chat` **intent** (roadmap-prescribed) handled by a **bare-callable bus subscriber** — `IntentHandler` is a `Callable[[IntentMessage], Awaitable[IntentResult | None]]`, and non-agent handlers already subscribe directly (`yeoman.py:242` `proactive_scan` precedent). The coordinator is a synthetic subscriber under a stable id (`group_chat_coordinator`): NO new agent, NO `_create_pools` wiring, NO registry entry. All logic lives in a new stateful `AgentGroupChatService` (`src/probos/threads/agent_group_chat.py`), constructor-injected and real-fixture testable (BF-287), mirroring the AD-915 pure/impure split. This honors the **uniform transport invariant** (Design Principle #10 — agent actions route through the bus, not a side-channel; the BF-265/267 lesson) over a bare `runtime.<svc>.create()` call from agent code. Rejected: a full `GroupChatCoordinatorAgent` utility agent (heavier `_create_pools` + registry + onboarding, no v1 benefit since decomposer/Captain-NL discovery is deferred).

**Consensus — `requires_consensus=False`, justified, not gated.** Creating a chat is reversible (`ChatThreadStore.delete_thread`) and low-risk. Per the **Safety Budget** axiom (consensus is risk-proportional) and **Minimal Authority**, a reversible non-destructive create does not warrant a quorum gate — that is reserved for destructive/irreversible intents. The genuine risk is a create-storm, addressed by a per-agent **cooldown** + sliding-window **cap** (BF-163 + BF-257 shape) on a long-lived `_create_times` dict — three new `GroupChatConfig` fields (`agent_create_cooldown_seconds=60.0`, `agent_create_max_per_window=5`, `agent_create_window_seconds=3600.0`). Budget is consumed only after the crew check, so non-crew rejections cost nothing; a rogue agent gets at most 5 rooms/hour ≥60 s apart.

**Persistence + v1/deferred seams.** An additive optional `metadata: dict | None = None` param on `ChatThreadStore.create_thread` extends the INSERT to write the existing (AD-791a) `metadata` column — NULL and `{}` both decode to `{}` via `_row_to_thread`, so every existing caller is unaffected — tagging `metadata.created_by_agent=<id>` (intended discoverability for AD-919's surface + Captain-join; no hidden channel). `task_id` is an **optional caller param** passed through to the existing column; live auto-wiring of an agent's current work item (which reaches the agent only via `intent.params["work_item_id"]` during `work_item_dispatch`) is a documented deferred seam. Decomposer/Captain-NL discovery is deferred — `CREATE_GROUP_CHAT_DESCRIPTOR` is a forward marker, not attached to a registered agent.

**Loop-safety.** Structurally no auto-reply storm in v1: (1) no agent-to-agent reply is wired; (2) the created thread has no Captain post, and AD-914 `group_chat_fanout` fires only on `role=="captain"`, so creation never triggers a fan-out; (3) the optional first message is a single persisted `role=="agent"` row that is NOT dispatched. The created thread is a normal `ChatThread` — AD-913 add/remove and a later Captain post (AD-919) fan out normally.

**Consequence.** +16 pytest (`tests/test_ad918_agent_initiated_group_chat.py`, BF-287 real `ChatThreadStore` + real `IntentBus(SignalManager(reap_interval=1.0))` + real-but-fake registry/callsign stubs + injectable fake clock; real `CallsignRegistry` for the callsign-resolution case): crew-only create + tag + creator auto-add, agent_id + callsign participant resolution, non-crew drop, task_id linkage, single first-message post, cooldown + window-cap + window-reset, empty-title + non-crew rejection (budget not consumed), the bus-handler round-trip, and the created thread driving AD-913 + AD-914 ops with no auto-fan. Focused gate green (66); `-k "thread or chat or fanout or group or intent"` blast-radius green (1005 passed, 4 skipped). No UI, no Ward Room, no new agent/pool, no consensus gate, no fan-out change.

### AD-916: File sharing in group chat — attachment refs + vision threading

**Context.** AD-913/914/915 built the text group chat (participants, fan-out, facilitator), but a chat message could not carry a file, and a vision-capable agent in a group chat could not see a shared image. The Captain's epic calls for file sharing for ad-hoc collaboration.

**Decision.** Two additive seams on the `ChatThreadStore` substrate, no schema/protocol/config change. (1) `AppendMessageRequest` (`routers/threads.py`) gains `attachment_ids: list[str]` — SHA-256 refs of files already uploaded via the existing `POST /api/chat/attachments` endpoint (bytes stored ONCE there; AD-916 references, never re-validates, mirroring `agents.py req.attachment_ids`). A new `resolve_attachment_refs(store, attachment_ids)` (`thread_fanout.py`) resolves each SHA → `{content_hash, mime}` via `FilesystemAttachmentStore.mime_for`; the append handler folds the result into `metadata.attachments` (existing JSON column) before `append_message`. (2) `group_chat_fanout` builds the AD-730/731 `vision_messages` array once from the Captain message's `metadata.attachments` via `build_chat_vision_messages` (filters `image/*` mimes, reuses `build_multimodal_messages` → exact AD-731 `attachment_ref` blocks); `_send_one` sets `params["vision_messages"]` ONLY for `vision_capable=True` participants (`callsign_registry.get_profile(agent_type).vision_capable`, mirroring the `agents.py` DM gate). The agent side already consumes `params["vision_messages"]`; the LLM client resolves the ref from `runtime.attachment_store`.

**Tier-2 honest-degrade throughout.** Unknown SHA → skipped with a warning (never fabricates a mime). Attachments-disabled or any resolution failure → plain message persisted (bytes already stored). No image refs → `vision_messages=None` → byte-identical AD-914 text-only fan-out. Vision-capable gate failure → text-only for that agent. Reuses `chat._get_attachment_store(runtime)` for byte-identical parity with the proven DM path (same on-disk root as `runtime.attachment_store`, so refs resolve); lazy imports avoid `chat.py`↔`threads.py`/`thread_fanout.py` load cycles.

**Boundaries.** v1 persists refs + threads image refs to vision agents; non-image attachments are stored+referenced+link-only — NO PDF/doc text extraction, NO new blob store, NO UI drag-drop (AD-917), NO agent-initiated attachments. `body` stays `min_length=1` (image-only messages need a caption/space until AD-917 defaults it). The AD-730-2 hard-cap/downscale/budget (DM-side) is a forward marker, not in the group fan-out. AD-914/915 dispatch+persist+facilitator paths unchanged; non-attachment messages byte-identical.

**Consequence.** +16 pytest (`tests/test_ad916_chat_file_sharing.py`, BF-287 real `FilesystemAttachmentStore` tmp dir + real `ChatThreadStore` + real `IntentBus`; e2e fan-out cases seed `chat._ATTACHMENT_STORE_CACHE[id(runtime)]`): store+ref roundtrip, ref persisted + survives `to_dict()`, vision participant receives the image ref in fan-out params, non-vision unaffected, non-image stored+referenced, unknown-SHA skip. Focused gate green (70); `-k "thread or chat or fanout or attachment or vision"` blast-radius green (666).

### AD-915: Turn-taking facilitator — relevance-ranked speaking order + convergence-gated termination

**Context.** AD-914's `group_chat_fanout` fans the Captain's turn to ALL crew-agent participants in parallel, once, and stops. With many participants this floods: every agent replies to every Captain turn regardless of relevance, and there is no termination signal when the exchange has converged. AD-915 turns the blind all-at-once into "the most relevant participants reply, in a sensible order, and the exchange stops when it converges" — the shared sequencer AD-921 (meeting voice) will reuse so avatars do not talk over each other.

**Decision.** A new **pure, deterministic, LLM-free** `ChatFacilitator` value-class (`src/probos/cognitive/chat_facilitator.py`: `SpeakerSignals`/`SpeakerScore`/`FacilitationResult`/`ChatFacilitator` with `rank`/`is_converged`/`facilitate`/`from_config`) mirrors the AD-641c `ThreadPriorityScorer` pure/impure split. `rank()` scores speakers by weighted factors (mention 0.40, recency 0.25 = anti-domination fairness, department 0.25 = Jaccard over the captain turn vs the agent's dept+type descriptor, trust 0.10 = optional injected neutral-0.5). `facilitate()` hard-includes any @-addressed agent at the FRONT (overriding both the cap and convergence), truncates NON-mentioned speakers to `max_speakers_per_turn`, and suppresses non-mentioned speakers when `is_converged()` returns True. The impure `_assemble_speaker_signals` (in `thread_fanout.py`, Tier-2 log-and-degrade on every runtime lookup) builds the snapshots; the facilitator is wired INSIDE `group_chat_fanout` between participant resolution and the `asyncio.gather`, which now iterates `result.speaking_order` — AD-914's `_send_one` dispatch+persist primitive is unchanged. A new `GroupChatConfig` (mounted on `SystemConfig` after `ward_room`) carries the cap + convergence knobs; `ChatFacilitator.from_config` reads it (None → all defaults, zero-config boot).

**Honest reuse-vs-new.** (1) AD-641c `ThreadPriorityScorer` scores **threads** (`ThreadPriorityInput` = thread snapshot), not which agent should speak — forcing it to rank speakers would be wrong-shape, so AD-915 adds a NEW pure scorer that *mirrors its pattern*. (2) The AD-583 convergence DETECTORS are substrate-coupled — `check_cross_agent_convergence` reads `RecordsStore` notebook entries and `ThreadEchoAnalyzer` reads Ward Room posts via the async `ThreadManagerProtocol` and detects *pathological echo*, not healthy agreement-termination — neither can be pointed at `chat_thread_messages` rows. Their atomic PURE primitive `probos.cognitive.similarity.jaccard_similarity(set, set)` + `text_to_words(str)` IS directly reusable (the same call `ThreadEchoAnalyzer` makes); threshold `0.6` follows the AD-614 `dm_similarity_threshold` precedent. (3) `WardRoomRouter.check_and_increment_reply_cap` is a **phantom** (does not exist) and the real Ward Room cap (`max_thread_posts`, BF-201) is the wrong substrate — the chat-substrate cap is a NEW `GroupChatConfig.max_speakers_per_turn`, default **0 = off** to preserve AD-914.

**Boundaries.** Text-only ordering — no meeting/voice/avatars/VRM (AD-920/921/922/923), no UI (AD-917), no agent-to-agent auto-reply or multi-round loop (still fans the Captain's single turn, just ordered/truncated/gated; convergence is single-turn suppression, not a self-feeding loop — AD-918). AD-914's dispatch/persist primitive (`_send_one`, the `direct_message` shape, `role="agent"` persistence, `metadata.fanout="ad914"`) is byte-identical; the facilitator only reorders/filters the `agent_ids` list. The `routers/threads.py` endpoint gate (`role=="captain"` AND ≥2 crew) is untouched. `_build_session_history` gains only an additive optional `prior=None` param (the 4-arg AD-914 call is byte-identical). Do NOT touch `WardRoomRouter`, `ThreadPriorityScorer`/`ThreadPriorityService`, `check_cross_agent_convergence`, `ThreadEchoAnalyzer`, or the `chat.py` AD-719 branch. No LLM calls anywhere; no new EventType/store/API route/consensus gate (Captain-authority, fully reversible ordering — Minimal Authority). All defaults make AD-914 inert (cap 0; convergence below `min_messages=4`/`min_agents=2`).

**Consequence.** +21 pytest (`tests/test_ad915_turn_taking_facilitator.py`, BF-287 real `ChatThreadStore` + real `IntentBus(SignalManager(reap_interval=1.0))` + real-but-fake registry/callsign/ontology — no MagicMock): pure ranking/truncation/convergence/mention-override cases, the impure assembly helper (mention detection, recency from prior, department relevance + ontology-absent fallback + neutral trust), and the `group_chat_fanout` integration invariant (AD-914 two-agent path unchanged; converged thread suppresses with the Captain message still persisted and no new agent rows; @-mention in a converged thread dispatches only the mentioned agent; 4-arg `_build_session_history` back-compat). Focused gate green (68); `-k "thread or chat or fanout or facilitat or convergence or priority"` blast-radius green (677).

### AD-914: Group-chat fan-out + cross-agent visibility on ChatThreadStore

**Context.** AD-913 made a thread's participant set mutable, but a multi-participant `ChatThreadStore` (AD-791) thread was still N blind 1:1s: the only reply path (`POST /api/agent/{id}/chat`) targets one agent and injects only client-supplied history, so two agents in the same thread never saw each other. The transient @-mention fan-out (AD-719, `routers/chat.py`) resolves callsigns, passes blind `session_history: []`, and does not persist — the roadmap's `AD-719a-wire` marker deferred wiring it into a persistence layer. AD-914 delivers that on the `ChatThreadStore` substrate, implementing the AD-719a contract (Captain messages are the seed; agents observe each other once both are in the thread; agent-to-agent without a Captain seed stays deferred to AD-918).

**Decision.** Extend the existing `POST /api/threads/{thread_id}/messages` handler (the least-invasive seam — the Captain turn is already persisted there). After the append + AD-793 project-touch, if `body.role == "captain"` AND the thread has ≥2 crew-agent participants (`is_crew_agent(runtime.registry.get(pid), runtime.ontology)` — naturally excludes the literal `"captain"` sentinel and any non-crew/unresolvable id), call a new focused module `routers/thread_fanout.py` (`group_chat_fanout` / `crew_agent_participants` / `_build_session_history`). Fan-out dispatches the `direct_message` intent to each crew participant in parallel via `runtime.intent_bus.send` (targeted request/reply, **not** `broadcast`; mirrors the 1:1 `agent_chat` path), reads `IntentResult.result`, and persists each reply as a `role="agent"` `chat_thread_messages` row (`metadata.fanout="ad914"`). Cross-agent visibility is a server-side `session_history` built from `list_messages(before=captain_msg.created_at, limit=1000)` tail-sliced `[-20:]` (the store is `ORDER BY created_at ASC LIMIT ?`, so a bare limit returns the OLDEST N — the tail-slice is load-bearing), mapped to `{"role": <callsign|Captain|system>, "text": body}` (the `cognitive_agent.py` consumer reads the `text` key; agent rows labelled by callsign). The Captain's current turn rides `params["text"]`, excluded from history. The response gains `per_agent_replies` only when fan-out fires; every other case returns `msg.to_dict()` byte-identical to before.

**Boundaries.** Fan the Captain seed out ONCE in parallel and STOP — no agent-to-agent auto-reply (AD-918), no turn-taking facilitator / convergence gating (AD-915), no file handling (AD-916), no `create_group_chat` intent (AD-918). All agents in a turn receive the SAME history snapshot (sequential turn-taking where B sees A's same-turn reply is AD-915). The chat.py AD-719 branch is untouched (a future AD-914a may unify the shared parallel-dispatch shape — forward marker only). No change to `IntentMessage`/`IntentResult`/`BaseAgent`/`IntentBus` or the `ChatThreadStore` public API (reuses `list_messages(before=...)` + `append_message`). No config field — the history window is the module constant `_FANOUT_HISTORY_LIMIT=20`. The 1:1 `agent_chat` endpoint and the Ward Room `multi_agent.py` contract are untouched (AD-914 implements the WardRoom rules on the `ChatThreadStore` substrate, not in WardRoom).

**Consequence.** +15 pytest (`tests/test_ad914_group_chat_fanout.py`, real `ChatThreadStore` + real `IntentBus(SignalManager(reap_interval=1.0))` + a real-but-fake subscribed handler that records the injected `session_history`, BF-287; no MagicMock at the store/bus boundary). Per-agent dispatch and the whole gate are Tier-2 log-and-degrade — one agent's failure (no subscriber → `(no response)`, raising handler → `(delivery failed)`) never blocks the others, and the gate-check runs inside the guard so even a minimal runtime without a `registry` degrades to a plain append (caught by `test_project_touch.py`, whose `_FakeRuntime` has no registry). Focused gate `test_ad914 + test_ad913 + test_ad791` green (47); thread/chat/fanout/intent blast-radius green.

### AD-913: Chat-thread participant management

**Context.** The ad-hoc crew-collaboration epic (group chat → meeting; Captain's ruling 2026-06-07) is built on the AD-791 `ChatThreadStore` — a SQLite-backed, Teams-style group-DM where `participants` is a first-class JSON `list[str]` column — **not** the Ward Room forum. The store could `create_thread`, `get_thread`, `update_thread`, `set_title`, and `append_message`, but there was **no way to mutate the participant set of an existing thread**, so "start a 1:1, then add crew" and "the Captain joins an agent-created chat" had no substrate. AD-913 is the foundation AD the rest of the epic (AD-914 fan-out, AD-919 join) calls.

**Decision.** Two synchronous, idempotent store methods placed next to the `set_title(lock=True)` `BEGIN IMMEDIATE` precedent: `add_participant(thread_id, agent_id)` appends the agent if absent; `remove_participant(thread_id, agent_id)` drops every copy if present (defensive against a pre-existing duplicate). Each runs a `BEGIN IMMEDIATE` read-modify-write of the JSON `participants` column for race safety, returns the updated `ChatThread` (or `None` when the row is missing — the `update_thread` return-shape precedent), and bumps `last_active_at` via the injected `self._clock()` **only on the mutating path**. Idempotency is a **true no-op**: re-adding a present member or removing an absent one performs no write and no activity bump, but still returns the (unchanged) thread. Two REST routes on the existing `threads` router: `POST /{thread_id}/participants` (body `ParticipantRequest.agent_id: str = ""` + an explicit `if not agent_id.strip(): 400` — deliberately NOT `Field(..., min_length=1)`, which would yield `422`; this makes both *missing* and *empty* → `400`) and `DELETE /{thread_id}/participants/{agent_id}` (an empty path segment simply fails route matching; removing an absent-but-nonempty agent is the 200 no-op). Both return `thread.to_dict()`, `404` on a missing thread, `503` via `_get_store` when the store is absent.

**Boundaries.** No fan-out / cross-agent visibility / message broadcast (that is AD-914) — AD-913 only mutates the participant list. No `AgentRegistry` validation (pure list ops; a future AD may gate `agent_id`). No change to the `ChatThread` dataclass, the `chat_threads` schema, or `to_dict()` (the `participants` column already existed). `api.py` is unchanged (the `threads` router is already imported and registered, so the new routes ride that registration); the legacy `/api/agent/{id}/chat` path and the Ward Room are untouched; no UI (AD-917/919).

**Consequence.** +14 pytest (`tests/test_ad913_participant_management.py`, real `ChatThreadStore` on `tmp_path` per BF-287, no MagicMock at the substrate boundary; a deterministic monotonic clock drives the two `last_active_at` assertions). 8 store-level (append, idempotent no-duplicate, bumps/does-not-bump `last_active_at`, missing-thread `None`; remove, absent-is-noop, missing-thread `None`) + 6 REST-level (add happy/404/400-empty/400-missing, remove happy/404). Focused gate `test_ad913_participant_management + test_ad791_chat_threads` green (32); thread/chat blast-radius green (416, no regressions).

### BF-612: Empty-content 200 → recycle the LLM connection pool and retry once

**Context.** After ProbOS had been up for ~11-18 hours under continuous proactive load, DMs to crew agents began returning `(no reply — agent's LLM endpoint returned empty content; check upstream proxy/endpoint at the configured tier)` (the BF-289 surface). Restarting the **proxy** fixed it; restarting **ProbOS** fixed it; the failure also self-recovered after an idle gap. Live triage isolated the fault to the runtime, not the proxy: a fresh `Invoke-RestMethod` to the Copilot proxy at `127.0.0.1:8080` returned real content even at Ezri's full ~14k-char prompt size, while the same agent through the runtime's pooled `httpx.AsyncClient` got empty content. The Captain confirmed the longer history — ProbOS previously ran for days without LLM connectivity issues, and something introduced the instability. Mechanism: the LLM client holds one long-lived `httpx.AsyncClient` per `base_url|format` with keep-alive sockets. When the proxy rotates its upstream Copilot session in the background (token refresh) it keeps the TCP socket open; the runtime reuses a pooled connection now bound to a dead upstream session, so the proxy answers **HTTP 200 with empty content** — no transport error, so `raise_for_status()` passes and the empty reply surfaces as BF-289. Under continuous load the socket never idles past the keep-alive expiry, so it is reused indefinitely; a proxy restart "fixes" it only by force-closing every socket. (AD-835, 2026-05-31, was the most recent prior touch of `llm_client.py` and matched one reported timeline, but it only added the system-prompt suffix and did not alter connection handling — correlation, not cause.)

**Decision.** Reproduce the proxy-restart fix in-process. (1) Centralise client construction in `OpenAICompatibleClient._build_client(tier)` (single source of truth, shared by `__init__` and the new refresh path). (2) Add `async _refresh_client(tier)` — closes the tier's stale `httpx.AsyncClient` (Tier-2 log-and-degrade: a close failure is logged and a fresh client is installed regardless) and rebuilds it via `_build_client`, so tiers sharing a `base_url|format` pool recover together. (3) In the `call()` fallback loop, after `_call_api` returns, detect an empty-content reply (`not response.content and not response.content_blocks and not response.error and api_format != "ollama"`) and, **at most once per tier per call** (guarded by a `_refreshed_tiers` set), call `_refresh_client` and retry the same tier on a fresh socket. A genuinely empty upstream therefore costs one extra call and then surfaces empty as before (no spin); a stale socket recovers transparently.

**Boundaries.** OpenAI/proxy path only — the Ollama-native path is excluded (local server, no upstream-session rotation). Tool-call replies carry `content_blocks` and are never treated as empty, so the SWE-harness path is untouched. Multimodal/vision replies that legitimately have empty `prompt` text are unaffected because the guard keys on `response.content`/`content_blocks`, not on the request shape. No behavioural change when the endpoint is healthy: a non-empty first reply takes exactly one `_call_api` call as before, and the empty-content reply that survives the single retry still reaches BF-289 unchanged. No new config field; zero-config boot is byte-identical. BF-289's detection/reporting is intentionally left in place as the last-resort surface when even the fresh socket returns empty (genuine upstream outage).

**Consequence.** +12 pytest (`tests/test_bf612_empty_content_retry.py`, real `OpenAICompatibleClient` per BF-287, patching only `_call_api`/`_build_client`): `_build_client` URL-normalisation + auth header; `_refresh_client` installs-new/closes-old/survives-close-failure; empty→content recovery, one-refresh-per-tier guard, no-refresh on healthy reply, tool-call not treated as empty, Ollama path not recycled. `tests/test_llm_client.py` (35) and `tests/test_ad835_tier_adaptation.py` (6) green. The restart-the-proxy ritual is no longer needed for the stale-socket failure mode.

### AD-912: Generalize 1:1 notebook persistence from Yeoman-only to all crew agents

**Context.** AD-911 let the Yeoman durably save a Captain-requested note from a 1:1 chat (`[NOTEBOOK slug]…[/NOTEBOOK]`), but scoped both the teaching hook and the `DmReplyPipeline` persistence to `agent_type == "yeoman"`. The Captain pointed out that creating a notebook entry from a 1:1 conversation is a general crew capability — notebooks are already a universal agent capability on the proactive / Ward-Room path (`extract_and_execute_actions` is not crew-gated and `extract_and_execute_notebooks` already resolves callsign + department generically). Scoping the 1:1 path to one agent was an artificial restriction.

**Decision.** Bring the 1:1 path to parity with the proactive path by removing the two Yeoman-only gates. (1) `CognitiveAgent._conversational_notebook_protocol` (the base hook) now teaches the `[NOTEBOOK topic-slug]…[/NOTEBOOK]` reply tag to **any** agent that has a records store wired (honest-degrade to `""` without one, via `getattr(self, "_runtime", None)` so a runtime-less or store-less agent is never told it can save notes the substrate cannot back — the AD-592 confabulation lesson), instead of returning `""` by default. (2) `YeomanAgent` drops its now-redundant override and inherits the base hook (DRY). (3) `DmReplyPipeline.step_4i_notebook_parse` removes the `agent_type != "yeoman"` gate: it persists for **any** agent via `extract_and_execute_notebooks` (which keys the note by the agent's callsign and runs the AD-550 dedup gate), and a final **safety-net unwrap** strips any markers that survive (no store wired, a write failure, or the proactive loop unavailable) so the Captain never sees a raw block regardless of agent or substrate state.

**Boundaries.** No new store, no new consensus gate, and no change to `extract_and_execute_notebooks` itself (it was already generic — the Yeoman scoping lived only in the hook override and the pipeline gate). The base hook is still overridable, so an agent can tailor the framing (Open/Closed). The honest-degrade gate is unchanged — an agent without a records store is taught nothing and persists nothing.

**Consequence.** +11 pytest (`tests/test_ad912_crew_notebook_generalization.py`, real `RecordsStore` + real `ProactiveCognitiveLoop` + real `DmReplyContext` per BF-287: the base hook teaches any agent with a store / honest-degrades without a store or runtime / is gap-regex-safe, the Yeoman now inherits the base hook, the pipeline step persists for a non-Yeoman crew agent under its own callsign, and the safety-net unwrap keeps the inner text when the proactive loop is unwired). The AD-911 suite was updated to match (the former "non-Yeoman unwraps without persisting" test now asserts the non-Yeoman crew agent persists). `tests/test_ad726_dm_reply_pipeline.py`, `test_ad845`, `test_ad869`, `test_ad870`, `test_bf599`, `test_ad550_notebook_dedup.py`, `test_yeoman_agent.py`, `test_proactive*.py` all green. Any crew agent the Captain DMs can now durably save a note to its own notebook when asked.

### AD-911: Yeoman notebook persistence from 1:1 Captain DMs

**Context.** The Captain asked Yeo (the Yeoman, AD-766) in a 1:1 chat to "save this to your notebook"; Yeo emitted a `[NOTEBOOK spacex-ipo-trade-setup]…[/NOTEBOOK]` block and replied "Done — that's saved." Nothing was persisted. Trace: a 1:1 Captain chat hits `agent_chat` → `direct_message` intent → the **`DmReplyPipeline`** post-LLM cleanup chain, which has **no notebook extractor** (only `[DM]`, `[MESH]`, `[CREATE_TASK]`, etc.). Notebook persistence (`records_store.write_notebook` with the AD-550 dedup gate) lived **only** on the proactive / Ward-Room path (`ProactiveLoop.extract_and_execute_actions`, reached from `ward_room_pipeline`). Worse, the 1:1 system prompt explicitly tells agents "Do NOT use any structured output formats," so Yeo emitting the tag at all was off-script and nothing was listening for it. The Yeoman — whose entire role is record-keeping — could not save a note when the Captain asked. The note survived only as an episodic memory, not a durable record.

**Decision.** Scope the fix to the Yeoman (the Captain's record-keeper) so other agents' 1:1 replies are byte-unaffected. Three additive pieces: (1) An overridable base hook `CognitiveAgent._conversational_notebook_protocol(observation) -> str` (default `""`), appended in `_decide_via_llm`'s `is_conversation` branch next to the BF-599 capability block and the AD-845 task protocol. `YeomanAgent` overrides it to teach the `[NOTEBOOK topic-slug]…[/NOTEBOOK]` reply tag — **honest-degrade**: returns `""` when `runtime._records_store` is absent, so Yeo is never told he can save notes the substrate cannot back (the AD-592 confabulation lesson). All tag text is `_CAPABILITY_GAP_RE`-clean. (2) `ProactiveLoop.extract_and_execute_notebooks(agent, text) -> (cleaned, actions)`, a lean writer that mirrors `extract_and_execute_dms`: parses `[NOTEBOOK slug]…[/NOTEBOOK]`, runs the AD-550 read-before-write dedup gate, calls `write_notebook`, and strips the markers. It deliberately omits the proactive path's AD-552/553/554/555 anti-flooding telemetry (which targets autonomous repetitive writes, not explicit low-frequency Captain requests). (3) `DmReplyPipeline.step_4i_notebook_parse`, registered after `step_4b_dm_outbound_parse` and before `step_5_episodic_store` (so the stored episode shows cleaned text): for the Yeoman it calls the new extractor; for any other agent (or when the proactive loop is unwired) it unwraps stray markers (keeps inner text, removes tags) so the Captain never sees a raw block, and persists nothing.

**Boundaries.** No new store, no new consensus gate, no change to the proactive notebook path (its full telemetry stack is untouched — zero regression risk there). The base hook returns `""` so every non-Yeo agent is byte-unaffected (Open/Closed). The 1:1 "no structured formats" instruction is unchanged for everyone except Yeo, who now receives the explicit notebook affordance only when a records store is wired.

**Consequence.** +14 pytest (`tests/test_ad911_yeoman_notebook.py`, real `RecordsStore` + real `ProactiveCognitiveLoop` + real `DmReplyContext` per BF-287: base hook returns `""`, Yeo hook teaches/degrades on the store, gap-regex-safe, the lean writer persists + strips + dedups + skips empty blocks + no-ops without a store, and the pipeline step persists for the Yeoman / unwraps-without-persisting for a non-Yeoman / no-ops without a marker). `tests/test_ad726_dm_reply_pipeline.py`, `tests/test_ad845_yeo_chat_task_creation.py`, `tests/test_ad869_mesh_read_inline.py`, `tests/test_ad870_yeo_delegation_threshold.py`, `tests/test_ad550_notebook_dedup.py` all green. When the Captain asks Yeo to save a note in 1:1 chat, it is now durably written to Ship's Records and Yeo's "saved it" is true.

### AD-910: Yeoman personal standing orders + Bridge department assignment

**Context.** Yeo (the Captain's Yeoman, AD-766) had **no** personal standing orders. `config/standing_orders/` keys the Tier-5 agent file as `{agent_type}.md`, and no `yeoman.md` existed, so `compose_instructions` loaded only the federation + ship tiers for him. Separately, the legacy department fallback `_AGENT_DEPARTMENTS` in `standing_orders.py` had no `yeoman` entry — `get_department("yeoman")` returned `None`, and since the conversational compose call passes no explicit `department`, Yeo also missed the Bridge department tier (`bridge.md`) that the Counselor (`"counselor": "bridge"`) inherits. Yeo was running on federation + ship orders alone, with none of his AD-766 role expressed as standing orders.

**Decision.** Two additive config/data changes. (1) `config/standing_orders/yeoman.md` — Yeo's Personal Standing Orders, modeled on `counselor.md`'s section structure (Role / Standards / Boundaries / Cadence / Personality) and aligned with the AD-766 `_ROLE_RULES`: act in the Captain's name for admin; triage proactive scans into one digest; delegate specialist work via @-mention; maintain the Captain's standing orders and surface conflicts; never make tactical/financial/destructive decisions without approval. It adds an explicit **Honesty About Your Capabilities** section ("Never claim to have done something you did not do… only report a note as saved when it is genuinely written to durable storage") — the standing-order seam against the confabulation the Captain observed (AD-911 is the code seam). (2) `_AGENT_DEPARTMENTS["yeoman"] = "bridge"` so Yeo inherits the Bridge department protocols like the Counselor. The `YeomanAgent.department = "Bridge"` class attribute (and the `test_yeoman_agent` assertion on it) is unchanged.

**Boundaries.** Content/config only — no code path beyond the one-line department map entry. The Tier-5 agent file loads unconditionally (independent of department), and the existing Bridge mapping (`counselor`) is untouched (Open/Closed).

**Consequence.** +6 pytest (`tests/test_ad910_yeoman_standing_orders.py`: the file exists with the right header, `compose_instructions(agent_type="yeoman", …)` includes the personal orders + a role phrase + the honesty clause, `get_department("yeoman") == "bridge"`, the Counselor mapping is unchanged, and an unmapped agent still returns `None`). Yeo now composes with all the standing-order tiers his Bridge-officer role warrants.

### BF-611: Web search must parse DuckDuckGo result blocks before truncating — raw HTML spent the budget on page chrome

**Context.** `WebSearchAgent` (AD-248, `agents/utility/web_agents.py`) fetches `https://html.duckduckgo.com/html/?q=<query>` through the mesh, then hands the body to its LLM to extract the top results. `perceive()` set `obs["fetched_content"] = body[:8000]` — the **raw, unparsed** DuckDuckGo HTML truncated at 8000 chars. The DDG page emits a large `<head>`/inline-CSS block, a header banner, and the search `<form>` *before* the first `<div class="result results_links results_links_deep web-result">`, so the 8000-char budget was consumed by page chrome and the result `<div>`s were cut off. The LLM never saw any result entries. This is exactly what Yeoman reported on a live SpaceX-IPO search: *"The HTML was cut off before the actual result entries rendered… I won't invent search results."* — an honest-degrade landing where the agent should have had data. The sibling `PageReaderAgent` already does the right thing (strips tags via `re.sub(r"<[^>]+>", " ", body)` **before** truncating); `WebSearchAgent` lacked the equivalent parse-before-truncate step. `http_fetch`'s 1 MiB `MAX_BODY_BYTES` cap was *not* involved — the DDG page is well under 1 MiB; the 8000-char slice in `perceive` was the truncation.

**Decision.** Parse the DuckDuckGo result blocks into structured `{title, url, snippet}` entries *before* applying any char budget, mirroring `PageReaderAgent`'s clean-before-truncate pattern. Three module-level helpers in `web_agents.py`: `_strip_tags(fragment)` (tag strip + `html.unescape`), `_decode_ddg_url(href)` (extracts the real target from DDG's `//duckduckgo.com/l/?uddg=<url-encoded>&rut=…` redirect, falls back to scheme-completing a `//` href or passing through), and `_parse_ddg_results(body, max_results=10)` (regex over `result__a` title anchors and `result__snippet` blocks, paired by index — DDG emits one snippet per result in parallel order; a result missing a snippet gets `""` rather than being dropped). `_format_ddg_results` renders the parsed entries as a compact `Result N: / Title / URL / Snippet` block. `WebSearchAgent.perceive` now sets `fetched_content` to the formatted parsed results (capped at 8000 chars of *result text*, not page chrome); when nothing parses it honest-degrades to the tag-stripped body (readable context, never raw markup) so the LLM can explain what came back without fabricating (enforced by the existing instructions).

**Boundaries.** Touches only `agents/utility/web_agents.py` (three helpers + the one `perceive` body). No change to `_mesh_fetch`, `http_fetch.py`, `PageReaderAgent`, or the mesh/consensus path. The instructions string is unchanged (it already says parse the provided results and never fabricate). No new store, no config, no events.

**Consequence.** +7 pytest (`TestBF611DdgParsing` in `tests/test_utility_agents.py`, real HTML fixture with 9000 chars of `<head>` chrome ahead of two result `<div>`s — asserts the parser extracts title/url/snippet, decodes the `uddg` redirect, respects `max_results`, returns `[]` on no results, and that `perceive` hands the LLM the parsed entries with the page chrome stripped). `tests/test_utility_agents.py` (70) and `tests/test_bf599_yeo_web_research_delegation.py` green. Yeo's web searches now receive actual result entries (title + URL + snippet) instead of truncated boilerplate, so a genuine search returns results rather than an honest-but-empty degrade.

### AD-902: Per-agent developmental skill management surface

**Context.** BF-610 fixed the Service Record's `Skills 0` contradiction by relabelling the two skill systems, and in doing so surfaced a real gap: there is no per-agent skill *management* surface. **Cognitive** (T2, `SKILL.md` instruction-knowledge) skills are rank/department-gated and not individually editable — they follow the agent's billet — but **developmental** (T3, proficiency-tracked) skills are acquired through `AgentSkillService` (AD-428) per agent, and the Captain had no way to acquire, re-level, or retire one from the personnel console. The service already had `acquire_skill`, `assess_skill`, and `exercise_skill`, plus a `suspended` record field and an `assessment_history` field, but no soft-suspend verb, no Captain-driven proficiency override, and no HTTP/UI surface.

**Decision.** Two thin additive layers over the existing AD-428 service — no new store, no parallel state, no consensus gate. **Backend (`skill_framework.py`):** `AgentSkillService.suspend_skill(agent_id, skill_id, suspended=True)` flips the existing `suspended` flag on a held record (returns `None` when not held; preserves proficiency and `assessment_history` across the SQLite round-trip, so a reinstate restores the full record rather than re-acquiring at FOLLOW), and `update_proficiency(agent_id, skill_id, new_level, source="assessment", notes="")` sets a new `ProficiencyLevel`, refreshes `last_exercised`, appends an audit entry to `assessment_history`, and emits `SKILL_REGRESSION` on a downward move else `SKILL_EXERCISED`. **HTTP (`routers/crew.py`, prefix `/api/crew`):** `GET /{id}/skills` (lists all records incl. suspended, joined with their definition `name`/`category`, honest-degrading to empty), `POST /{id}/skills` (acquire, surfacing the unmet-prerequisite `ValueError` as a 400), `PATCH /{id}/skills/{skill_id}` (re-level via `update_proficiency` and/or suspend/reinstate via `suspend_skill`), `DELETE /{id}/skills/{skill_id}` (soft-suspend). **UI (`SkillManagement.tsx`, mounted in the AD-897 Service Record skills section):** a `DEVELOPMENTAL SKILLS` panel with acquire, single-click level-up, two-step confirm level-down, two-step confirm suspend, and one-click reinstate.

**Governance.** No new consensus gate (Minimal Authority). Every verb is a Captain-authority proficiency edit, and every one is reversible: a level change appends to `assessment_history` rather than overwriting history, and the destructive verb is a **soft-suspend** (`DELETE` flips the `suspended` flag, it does not drop the record) so a suspended skill can be reinstated with its full proficiency and history intact (Reversibility Preference). The two destructive affordances in the UI (level-down, suspend) are gated behind a two-step confirm, and the server's rejection `detail` (a 503/400/404 or the unmet-prerequisite message) is surfaced to the Captain rather than swallowed.

**Boundaries.** Touches only `AgentSkillService` (two new verbs over the existing record shape — `suspended` and `assessment_history` already existed), the already-registered `crew` router (four new routes, no `api.py` change), and the AD-897 Service Record (one mounted child). No new store, no `EventType`/events migration, no consensus/trust core change. Cognitive (T2) skills are explicitly NOT made editable here (they are billet-gated, not per-agent), the skill-definition catalog CRUD is AD-898's surface (not duplicated), and the BF-610 count display is unchanged. `acquire_skill`/`assess_skill`/`exercise_skill` and the AD-887 unified-profile path are untouched.

**Consequence.** +8 pytest (`tests/test_ad902_skill_management.py`, BF-287 real `AgentSkillService` + `SkillRegistry` on `tmp_path` through an in-process FastAPI `AsyncClient`) + 7 Vitest (`ui/src/components/personnel/SkillManagement.test.tsx`). Full backend gate green (15669 passed, 22 skipped); the AD-897 (5) and full personnel Vitest suite (36) still green; `tsc --noEmit` clean. The Captain can now manage a crew member's developmental skills — acquire, re-level, suspend, reinstate — from the personnel console, closing the gap BF-610 named.
...
**Decision.** A new `ui/src/components/personnel/ToolCertifications.tsx` that browses the tool catalog (`GET /api/tools`, honest-degrading to empty), carries a self-contained crew selector (`GET /api/crew/roster`) so the view is decoupled from the roster master pane, and on selection lists the agent's active certifications (`GET /api/crew/{id}/tools`). It exposes two verbs: **grant** (a `tool-grant-form` — a catalog tool select, an `observe`/`read`/`write`/`full` permission select, an optional reason — client-validating that a crew member and a tool are chosen before `POST /api/crew/{id}/tools`, surfacing the server `detail` inline) and **revoke** (a two-step `Revoke` → `Confirm revoke` red affordance, `DELETE /api/crew/{id}/tools/{grant_id}`). Restrictions (`is_restriction`) render with a red `RESTRICTION` flag alongside elevations. The view mounts in the console behind a third header view-switcher tab (`personnel-tab-tools`, the `view` type extended to `roster`|`skills`|`tools`, defaulting to `roster`) in its own `personnel-tools-pane`.

**Governance.** No new consensus gate (Minimal Authority). AD-899 is a thin UI over the AD-894 endpoints; granting and revoking a tool are audited Captain-authorized `ToolAccessGrant` edits whose authority model and audit trail live entirely in the backend `ToolPermissionStore` and are not bypassed. A grant is reversible and a revoke is a soft, audit-retained operation, so neither warrants a consensus gate; the destructive revoke is nonetheless gated behind a two-step confirm, and the server's rejection `detail` is shown to the Captain rather than swallowed.

**Boundaries.** No backend code is touched — AD-899 consumes the AD-894 crew-tool endpoints and the `GET /api/tools` catalog only. The AD-896 console shell gains a third view-switcher tab and a `personnel-tools-pane`, but the master-detail body and the AD-898 skills pane are preserved and `roster` remains the default view, so every AD-896/AD-897 testid and behavior renders unchanged. The crew selector is self-contained (it does not couple to the console's `selectedId`), and no skill action is added; the per-agent skill library is AD-898.

**Consequence.** +6 Vitest (`ui/src/components/personnel/ToolCertifications.test.tsx`). The AD-896 (8) suite remains green; `tsc --noEmit` clean. The Captain can now certify and revoke a crew member's tool qualifications from the personnel console, with restrictions flagged and the server's protection errors surfaced inline. **This completes the Crew Personnel Management epic (AD-891 → AD-901, eleven ADs).**
...
**Decision.** A new `ui/src/components/personnel/StandingOrders.tsx` that extends the Service Record's Standing Orders section into a two-part view: (1) the read-only four-tier composed orders (AD-893) — the same `sr-order-tier-{tier}` rows, lifted out of `ServiceRecord` and passed down as a `tiers` prop so the AD-897 testids and behavior are preserved — and (2) a Directives panel bound to AD-900. The panel fetches `GET /api/crew/{id}/directives` (honest-degrading to an empty list), lists active + pending-approval directives, and exposes three governed actions: **issue** a Captain's order (`content` + `priority`, `POST /api/crew/{id}/directives`, surfacing the authorization `detail` inline on a 400), **approve** a pending directive (`POST /api/crew/directives/{id}/approve`), and **revoke** with a two-step confirm (first click reveals "Confirm revoke", second issues `DELETE /api/crew/directives/{id}`). Pending directives are amber/awaiting with an Approve affordance; active directives are teal. `ServiceRecord` renders `<StandingOrders agentId tiers={tiers} />` inside its `sr-section-orders`.

**Governance.** No new consensus gate (Minimal Authority). AD-901 is a thin UI over the AD-900 endpoints; the authorization model (`authorize_directive`) and the PENDING_APPROVAL → ACTIVE approval state machine live entirely in the backend and are not bypassed. The view surfaces the existing approval gate (a Captain explicitly approving a pending lower-authority directive) rather than introducing a new authority. Revoke is a governing/destructive action and is gated behind a two-step confirm.

**Boundaries.** No backend code is touched — AD-901 consumes the AD-900 directive endpoints and the AD-893 tiers (passed as props) only. The AD-897 Service Record's other five sections, the AD-896 console shell, and the store slice are untouched except for the one tier-rendering block in `sr-section-orders` being replaced by `<StandingOrders>`. The AD-897 `sr-order-tier-{tier}` testids and the 600-char tier truncation are preserved. The `PATCH` (amend/FRAGO) endpoint is left unconsumed in this view (issue/approve/revoke cover the epic's stated surface); no skill or tool actions are added.

**Consequence.** +5 Vitest (`ui/src/components/personnel/StandingOrders.test.tsx`). The AD-897 (5) and AD-896 (8) suites remain green; `tsc --noEmit` clean. The Captain can now issue, approve, and revoke an agent's directives directly from the service record; the immutable four-tier `.md` orders remain read-only as designed.

### AD-897: Service Record detail view — the ESR (Crew Personnel Management epic, part 8 — #861)

**Context.** AD-896 shipped the Crew Personnel Console window with a master roster and a deliberately empty detail pane. AD-897 fills that pane with the per-agent personnel profile — the Workday-style Electronic Service Record. The three backend reads it needs already exist: the ACM consolidated profile (`GET /api/crew/{id}/record`, AD-892, which the endpoint augments with a `billet` block and `active_assignments`), the four standing-order tiers (`GET /api/crew/{id}/standing-orders`, AD-893), and the certified tool grants (`GET /api/crew/{id}/tools`, AD-894). The design constraint is that the personnel record must surface **both** qualification homes — the AD-894 tool certifications and the AD-595d billet qualification standing — which are distinct subsystems that the console renders side by side.

**Decision.** A new read-only `ui/src/components/personnel/ServiceRecord.tsx`, mounted into the AD-896 detail pane (replacing the placeholder) and keyed on the selected `agent_id`. A single `useEffect` fetches all three endpoints in parallel via a `safeJson` helper that returns `null` on a non-ok or thrown response, so each facet degrades independently. The record body is accepted only when it carries an `agent_id` facet — a roster-shaped or null body is rejected and the view falls back to the roster `summary` props passed down from the console, so the Identity header always renders even when the record fetch fails. Six progressive-disclosure sections: **Identity & Role** (callsign, post/billet, department, rank, lifecycle, Big-Five personality bars), **Skills & Proficiency** (count, avg-proficiency bar, cognitive-skill list), **Qualifications** (certified tools with restrictions flagged red, AND billet qualification standing — qualified vs. missing tests), **Duties & Active Assignments** (configured watch-bill duties + in-flight work), **Standing Orders** (the four tiers, present/absent distinct), **Experience** (trust, earned agency, episode count). Every empty facet renders a calm empty state.

**Governance.** No consensus gate (Minimal Authority). AD-897 is a read-only display surface — it consumes the three existing read endpoints and renders them; it issues no mutations. The grant/revoke and skill-edit write actions are explicitly deferred to AD-898/899.

**Boundaries.** No backend code is touched — AD-897 consumes the existing `/record`, `/standing-orders`, and `/tools` read endpoints only. The AD-896 console shell, store slice, and roster master pane are untouched except for the one-line detail-pane mount of `<ServiceRecord>`. The AD-896 test's obsolete placeholder-text assertion (`"Service record detail loads here."`) was updated to assert the `service-record` view mounts — a faithful consequence of AD-897 replacing the scaffold the AD-896 test asserted. The grant/revoke (AD-899) and skill-edit (AD-898) actions are out of scope.

**Consequence.** +5 Vitest (`ui/src/components/personnel/ServiceRecord.test.tsx`). UI typecheck (`tsc --noEmit`) clean. The console now renders a complete per-agent service record; AD-898/899 layer the management (write) actions on top.

### AD-896: Crew Personnel Console shell — the Ship's Office (Crew Personnel Management epic, part 7 — #860)

**Context.** The Crew Personnel Management epic needs a *home* in the HXI — a dedicated window through which a Captain reviews and manages the crew's service records, skills, qualifications, standing orders, and assignments. The design constraint (from the epic spec) is that this is a *separate experience* — the Ship's Office — **not** another tab on the existing agent profile panel. The AD-837 Ward Room (`ui/src/components/wardroom/WardRoomPanel.tsx`) already established the three-mode window pattern (`docked` ↔ `floating` ↔ `maximized`) with drag, resize, and `localStorage`-persisted geometry; AD-896 mirrors that pattern for a second independent window. The backend roster surface (`GET /api/crew/roster`, AD-892) already exists.

**Decision.** A new `CrewPersonnelConsole` window component plus an independent store slice, wired into the App shell.

1. **Store slice (`useStore.ts`).** Reuses the WardRoom `WardRoomDisplayMode`/`WardRoomWindowRect` types (no new geometry vocabulary). Four fields — `personnelConsoleOpen`, `personnelDisplayMode`, `personnelWindowRect` — seeded from a new exported `loadPersonnelLayout()` (defaults `mode='floating'`, rect `{x:120,y:80,w:880,h:660}`; log-and-degrade `try/catch` that falls back to the default rect on malformed JSON). `persistPersonnelMode`/`persistPersonnelRect` writers persist under the `probos.personnel.mode`/`probos.personnel.rect` keys (independent of the WardRoom keys, so the two windows remember their own geometry). Four actions — `openPersonnelConsole`, `closePersonnelConsole`, `setPersonnelDisplayMode`, `setPersonnelWindowRect` — are shaped exactly like the WardRoom precedent (persist-then-`set`).
2. **Component (`components/personnel/CrewPersonnelConsole.tsx`).** Master-detail layout. The master (left) pane is the crew roster bound to `GET /api/crew/roster`, fetched in an `open`-gated `useEffect` into component-local state, department-grouped with rank badges reusing the CrewRosterPanel `DEPT_COLORS`/`RANK_LABELS` conventions. Selecting a row sets a component-local `selectedAgentId`; the detail (right) pane is a deliberate **placeholder** (`data-testid="personnel-record-pane"`) that AD-897 (Service Record) fills in. Header chrome carries Dock/Undock, Maximize/Restore (hidden when docked), and Close stroke-SVG glyphs (`Glyphs.tsx`, amber active `#f0b060`). Floating mode adds a header drag handle and a bottom-right resize handle, both with the WardRoom add-on-mousedown/remove-on-mouseup window-listener discipline and min-size clamps (480×360). A failed fetch honest-degrades to an empty roster ("No crew aboard.") rather than crashing the shell.
3. **App wiring (`App.tsx`).** A `PERSONNEL` `NavButton` (testId `personnel-toggle`) in the TopNav People group, and `<CrewPersonnelConsole />` mounted in the App return alongside `<WardRoomPanel />`/`<CrewRosterPanel />`.

**Governance.** No consensus gate (Minimal Authority). AD-896 is a read-only window shell — it fetches the roster and renders it; it issues no mutations. The detail pane is a stub. The console is purely a viewing surface at this stage, so there is no governed action to gate.

**Boundaries.** No backend code is touched — AD-896 consumes the existing `GET /api/crew/roster` only. The roster list and `selectedAgentId` are component-local state (not promoted to the global store): only the window's open/mode/rect — which the launch NavButton and persistence need — live in the store, keeping the store change minimal. The WardRoom store slice, component, and `probos.wardroom.*` keys are untouched. The right detail pane is a placeholder by design; the AD-897 Service Record fills it.

**Consequence.** +8 Vitest (`ui/src/components/personnel/CrewPersonnelConsole.test.tsx`). UI typecheck (`tsc --noEmit`) clean. The Ship's Office is launchable from the TopNav and renders the live roster; AD-897 builds the service-record detail into the placeholder pane.
...
**Decision.** Two registry verbs, one service helper, and four HTTP endpoints.

1. **`SkillRegistry.update_skill_definition(defn, *, create=False)`** — a validated wrapper over `register_skill`. `create=True` rejects a duplicate `skill_id`; every prerequisite must reference an already-registered skill (a dangling prerequisite, or a self-referential one, raises `ValueError`) so the prerequisite DAG never points at a missing node; then it persists and returns the definition. The full `SkillDefinition` is passed (consistent with `register_skill(defn)`) rather than a loose `**fields` kwargs bag — the router owns HTTP-shape→`SkillDefinition` construction, the registry validates and persists (clean DI boundary).
2. **`SkillRegistry.delete_skill_definition(skill_id, *, skill_service=None)`** — two guards: a **built-in PCC** (seeded from `BUILTIN_PCCS`) is never deletable, and a skill **in active use** (`skill_service.count_agents_with_skill(skill_id) > 0`) is refused so a delete cannot orphan acquired records. On success it pops the cache and `DELETE`s from the store.
3. **`AgentSkillService.count_agents_with_skill(skill_id)`** — `SELECT COUNT(*) FROM agent_skills WHERE skill_id = ?`, returns 0 when there is no backing store. Supplies the in-use signal for the delete guard.
4. **HTTP (`routers/skills.py`, prefix `/api/skills`):** `GET /api/skills/definitions` (optional `category`/`domain`; honest-degrades to `{"definitions": [], "count": 0}` when the registry is absent; 400 invalid category), `POST` (build `SkillDefinition` from body; 503 no registry, 400 missing `skill_id`/`name`/`category` or invalid category, 400 duplicate/dangling `from None`; `create=True`), `PUT /{skill_id}` (404 when `get_skill` is None; `create=False`), `DELETE /{skill_id}` (404 unknown, 400 `from None` on the built-in/in-use guards). Reuses the `/registry` dict serialization shape.

**Governance — no consensus gate (Minimal-Authority).** Skill-library authoring is a **Captain-authority** edit, gated by the **Minimal-Authority** axiom, not a consensus quorum. Rationale: (a) the actor is the Captain, the ship's sole human authority — there is no peer quorum above the Captain to vote on a library edit; (b) every write is **reversible** — a created skill can be deleted, an updated one re-updated, satisfying the Reversibility-Preference axiom; (c) the action's blast radius is bounded by validation, not ceremony — the **delete guards** (built-in PCC protected, in-use skill refused) and the **create/update guards** (no duplicate, no dangling prerequisite) are the safety mechanism that keeps the prerequisite DAG sound and prevents orphaned agent records. A consensus gate here would add ceremony without adding safety: the authority is already the top of the trust hierarchy, the writes are validated, and the destructive verb (delete) is itself guarded against the two ways it could harm the ship.

**Boundaries.** The per-agent skill-record endpoints (assess/exercise/commission), the AD-887 unified-profile path, the `SkillDefinition` field shape, and `register_skill`/`start`/`register_builtins` are all untouched. No seeding from the ontology skills YAML. No paging/search on the list endpoint (the library is small; a later AD if it grows).

**Consequence.** The Skill Library UI (AD-898) has a stable browse surface (`GET /api/skills/definitions`) and a governed create/edit/retire path (`POST`/`PUT`/`DELETE`) with inline validation errors surfaced as 400s. +18 pytest (`tests/test_ad895_skill_library_crud.py`, BF-287 real fixtures: real `SkillRegistry`/`AgentSkillService` on `tmp_path`, TestClient driven through a FastAPI lifespan so the aiosqlite connections share the request event loop per the AD-894 cross-loop lesson). Fifth AD of the Crew Personnel Management epic (#855-865).

### AD-894: Tool asset catalog + per-agent certification HTTP surface (Crew Personnel Management epic, part 4 — #858)

**Context.** The personnel console needs to show the ship's tool inventory and let the Captain certify a crew member on a tool from the manning view. The infrastructure already existed: `ToolRegistry` (AD-422) holds the tool catalog with `to_dict()` projections, and `ToolPermissionStore` (AD-423b) holds the additive CRUD+O grants (`ToolAccessGrant`), both wired onto the runtime (`runtime.tool_registry`, `runtime.tool_permission_store`). Neither had an HTTP read/write surface, so the catalog and per-agent certifications were invisible to the UI and there was no certify/decertify path outside the CLI.

**Decision.** A ship-wide catalog endpoint plus three per-agent certification endpoints, split across two routers.

1. **Catalog — `GET /api/tools`.** Lives in a **new** `src/probos/routers/tools.py` (prefix `/api/tools`, registered in `api.py` alongside `crew_router`). Honest-degrades to `{"tools": [], "count": 0}` when `runtime.tool_registry` is absent; else returns `{tools: [reg.to_dict() for reg in registry.list_tools()], count}`.
2. **Certifications — `GET/POST/DELETE /api/crew/{agent_id}/tools[/{grant_id}]`.** Live in `routers/crew.py` as personnel-record facets. `GET` lists the agent's active grants (`get_active_grants_sync`) joined with catalog metadata (`registry.get(grant.tool_id).to_dict()`, `tool: null` when no longer registered). `POST` certifies (503 no store, 400 missing `tool_id`/`permission`, 404 unknown tool, 400 invalid `ToolPermission` via `raise … from None`, else `await issue_grant(agent_id, tool_id, perm, reason=…, issued_by="captain")`). `DELETE` revokes (`await revoke_grant`, 404 when unknown).

**Router split (why two routers).** A FastAPI prefix applies to every route in a router. The catalog is ship-scoped, not crew-scoped, so it cannot live under `/api/crew`. Rather than relativize the catalog under a crew agent (semantically wrong — the catalog is not a property of any one agent), the catalog gets its own `/api/tools` router; the certify/decertify endpoints stay under `/api/crew/{agent_id}` because a certification *is* a facet of that agent's personnel record. The split is documented in the `tools.py` module docstring.

**Governance — no consensus gate.** The grant/revoke pair is a **Captain-authority privilege edit**, gated by the **Minimal-Authority** axiom, not a consensus quorum. Rationale: (a) the actor is the Captain (`issued_by="captain"`), the ship's sole human authority — there is no peer quorum above the Captain to vote; (b) every grant is recorded as a `ToolAccessGrant` (audited) and is **reversible** via `revoke_grant`, satisfying the Reversibility-Preference axiom; (c) the grant only *widens or narrows* an agent's tool access within the existing permission lattice — it cannot create a tool or escalate beyond `FULL`, so the Safety-Budget risk is bounded by the catalog the runtime already trusts. A consensus gate here would add ceremony without adding safety, since the authority being exercised is already the top of the trust hierarchy and the action is logged and undoable.

**Boundaries.** Neither `ToolRegistry` nor `ToolPermissionStore` is modified — the endpoints only read (`list_tools`, `get`, `get_active_grants_sync`) and wrap the existing async mutators (`issue_grant`, `revoke_grant`). No change to `crew_tasks`, the ACM, or the AD-892/893 crew endpoints. The catalog does not page or filter (the full catalog is small); filtering is a later AD if the catalog grows.

**Consequence.** The Tool Certifications UI (AD-899) has a stable read surface (`GET /api/tools`, `GET /api/crew/{id}/tools`) and a governed grant/revoke path (`POST`/`DELETE`). +11 pytest (`tests/test_ad894_tool_cert_api.py`, BF-287 real fixtures: real `ToolRegistry` with `_StubTool` Tool-protocol stubs + real tmp-file `ToolPermissionStore` whose lifecycle runs in the `TestClient` loop via a lifespan handler). Fourth AD of the Crew Personnel Management epic (#855-865).
...
**Decision.** A new router `src/probos/routers/crew.py` (prefix `/api/crew`, `tags=["crew"]`) — kept distinct from the existing `crew_tasks` router (`/api/crew-tasks`) — wired into `api.py` via the import tuple (`crew as crew_router`) and the include loop. Two async read-only endpoints.

1. **`GET /api/crew/roster`.** Walks `runtime.registry.all()`, filters to crew via `crew_utils.is_crew_agent(agent, ontology)`, and joins each against `ontology.get_crew_manifest(trust_network=..., callsign_registry=...)` keyed by `agent_type`. Manifest-present → `callsign/post/department/rank` + `assigned: true`, `billet_state: "billeted"`. Crew-recognized but manifest-absent → `post/department/rank: null`, `assigned: false`, `billet_state: "unbilleted"` (callsign falls back to `agent.callsign`) — a visible manning gap rather than a silent omission. Each entry augmented with `lifecycle_state` (`(await acm.get_lifecycle_state(id)).value`), `skill_count` (`len((await skill_service.get_profile(id)).all_skills)`), `tool_count` (non-restriction active grants via `tool_permission_store.get_active_grants_sync`), each in its own Tier-2 debug-degrade guard. Sorted by `(department, agent_type)`. None registry → `{"crew": [], "count": 0}`.
2. **`GET /api/crew/{agent_id}/record`.** 503 when `acm` absent; 404 when `registry.get(agent_id)` is None; else `await acm.get_consolidated_profile(agent_id, runtime)` augmented with `active_assignments` (from `await work_item_store.list_work_items(assigned_to=id, limit=100)` filtered to `open`/`in_progress`) and a `billet` block (resolve `assignment.post_id` → `billet_registry.resolve` for title/department → `await billet_registry.check_qualifications` for `qualified` + `missing_qualifications`), the billet block omitted via honest-degrade when the agent has no assignment.

**Governance — no consensus gate.** Both endpoints are **read-only projections** of state the runtime already holds (registry, ontology manifest, ACM profile, work-item store, billet registry). They grant no authority, mutate no state, and issue no directive. Minimal-Authority: a read lens needs no consensus gate. The mutating crew endpoints (tool certification AD-894, directives AD-900) carry their own governance decisions in their ADs.

**Boundaries.** No change to `crew_tasks` router, `AgentRegistry`, `VesselOntologyService`, ACM, or any store. The roster does **not** reconcile the registry/manifest divergence — the unbilleted state is reported, not resolved (reconciliation, if ever, is a separate AD). The module docstring records the authority model: roster is registry-authoritative for who-is-aboard + ontology-authoritative for the org chart; record is ACM-authoritative for the HR view; the two drift and are not merged.

**Consequence.** The crew roster and service record now have a stable HTTP read surface for the Personnel Console (AD-896) and Service Record (AD-897) UIs. +9 pytest (`tests/test_ad892_crew_record_api.py`, real `AgentRegistry` substrate + explicit hand-written ontology/ACM fakes per BF-287). Second AD of the Crew Personnel Management epic (#855-865).
...
**Decision.** Add `AgentCapitalService.commission(self, agent_id: str, agent_type: str, runtime: Any) -> dict[str, Any]` (`src/probos/acm.py`) as the single commissioning front door. It is **self-stateless** — it reads only `runtime` and the module logger, never `self._db`, so it requires no `acm.start()` and can be unit-tested against a bare `AgentCapitalService(tmp_dir)`. The walk:

1. **Legacy back-compat.** If `runtime.skill_service` is present, `await skill_service.commission_agent(agent_id, agent_type)` first (wrapped, debug-on-fail) so the pre-existing AD-428 commission path still runs.
2. **Role → Skills.** Read `runtime.ontology.get_role_template_for_agent(agent_type)`; for each `SkillRequirement` map `min_proficiency` → `ProficiencyLevel` (an out-of-range value falls back to `ProficiencyLevel.FOLLOW`, never raises), then `await skill_service.acquire_skill(agent_id, req.skill_id, source="commission", proficiency=level)`. A `ValueError` (unmet prerequisite) is **debug-logged, not raised** — partial commissioning is acceptable; a missing prereq is a soft skip.
3. **Fetch the profile once.** `profile = await skill_service.get_profile(agent_id)` — a single read, reused for the tool walk; `summary["skills_acquired"]` is `[r.skill_id for r in profile.all_skills]`.
4. **Skills → Tools (idempotent).** Only when `profile is not None` **and** all three of `runtime.tool_permission_store` / `runtime.tool_registry` / `runtime.skill_registry` are present: for each acquired skill, look up its `SkillDefinition` (`skill_registry.get_skill`, skip if absent), `resolve_tools_for_skill(skill_def, agent_id=, tool_registry=)`, and for each resolved tool check `tool_permission_store.get_active_grants_sync(agent_id, tool_id)` — if any **non-restriction** active grant already exists, skip (no double-grant); otherwise `await tool_permission_store.issue_grant(agent_id, tool_id, ToolPermission.READ, is_restriction=False, reason="commission", issued_by="captain")` and append to `summary["tools_granted"]`.

Returns `{agent_id, agent_type, skills_acquired: [...], tools_granted: [...]}`.

**Wiring — Option A (birth path at `create_pool`), chosen over Option B (`agent_onboarding.wire_agent`).** The commission hook lives in `ProbOSRuntime.create_pool` (`src/probos/runtime.py`) inside the existing post-creation `for agent in self.registry.get_by_pool(name)` loop, immediately after `await self.onboarding.wire_agent(agent)`, guarded `if self.acm and is_crew_agent(agent, self.ontology)` and wrapped in a Tier-2 `try/except` → `logger.debug(..., exc_info=True)` so a commission failure never aborts pool creation. **Why Option A and not Option B:** the AD-515 `AgentOnboardingService.wire_agent` deliberately carries a dependency-light surface — it has no handle to `runtime`, `skill_registry`, or `tool_permission_store`. Threading those four services into the onboarding service to host the commission walk would invert AD-515's design. The ACM front door instead pulls every dependency off the live `runtime` it already receives, so onboarding stays lean and the commission logic lives with the ACM (the unification owner). `is_crew_agent(agent, self.ontology)` scopes commissioning to crew agents (infrastructure/utility agents are not commissioned).

**Endpoint re-point — deliberate contract change.** `/skills/agents/{agent_id}/commission` (`src/probos/routers/skills.py`) now calls `runtime.acm.commission(agent_id, req.agent_type, runtime)` and **returns the commission summary dict** (`{agent_id, agent_type, skills_acquired, tools_granted}`) instead of the previous `profile.to_dict()`. This is an intentional, documented consequence of collapsing the two commission paths behind one lens — the endpoint now reports exactly what was commissioned (which skills acquired, which tools granted) rather than a full profile dump. A `503` is raised when `runtime.acm` is absent.

**NO duties step.** Same verify-first NO-SEAM as AD-885: there is no public per-agent duty accessor (`DutyScheduleTracker.get_due_duties` is private on `runtime.proactive_loop._duty_tracker`, a Law-of-Demeter violation to reach). Duties join the commissioning walk in a future AD that first ships a public duty-tracker surface.

**Honest-degrade matrix.** Missing `skill_service` → empty summary, no crash. Missing `ontology` or no template for the type → legacy-only (step 1 ran, no template skills). Missing any of the three tool services → skills acquired but `tools_granted == []`. Every branch is a soft path; `commission` never raises for an absent collaborator.

**Tests.** +6 in `tests/test_ad889_commission_chain.py` (BF-287 real fixtures — real `AgentCapitalService`, real `SkillRegistry` + `AgentSkillService` on a tmp `skills.db`, real `ToolRegistry`, real `ToolPermissionStore`; a `_StubOntology`/`_StubRuntime` attribute holder only for the runtime-attribute boundary, no MagicMock at the substrate boundary; the `(service, registry)` tuple helper avoids any private-attr reach into the service): template skills acquired + tools granted; legacy-only fallback when no template; idempotent re-commission (second pass grants nothing new); a skill with no preferred tool grants nothing cleanly; degrade without a permission store; empty summary without a skill_service. Full gate green (15589 passed, 22 skipped; the only 2 reds — `test_auto_commit_after_debounce`, `test_bf304_single_flight_drops_concurrent_describe` — are xdist-parallel timing flakes that pass serially, unrelated to commissioning). One commit.

**Consequence.** The Skills & Tools Unification epic's capstone: a crew agent born into a pool is now commissioned automatically — its role template's required skills are acquired and the tools those skills resolve to are granted — through one ACM front door, and the same front door backs the `/commission` endpoint. AD-890 (docs) closes the epic.
...
**Decision.** Make a single call report both skill kinds, **merging at the bridge** (not the service, not the catalog).

1. **Additive field, foundation-layer-safe.** Add `cognitive_skills: list[dict[str, Any]] = field(default_factory=list)` to the `SkillProfile` dataclass (`src/probos/skill_framework.py`). It is typed as plain kind-tagged dicts — **not** `list[CognitiveSkillEntry]` — so this foundation-layer module (no LLM calls, no I/O in data classes) stays decoupled from the cognitive-layer `CognitiveSkillEntry` type. `Any`/`field` are already imported.
2. **Bridge merge method.** Add `async def get_unified_profile(self, agent_id, *, department=None, agent_rank=None) -> SkillProfile` to `SkillBridge`. It calls `await self._service.get_profile(agent_id)` for the developmental profile, then `self._catalog.list_entries(department=department, min_rank=agent_rank)` for the cognitive entries, appending each as `{name, description, skill_id, department, min_proficiency, min_rank, origin, kind: "cognitive"}` to `profile.cognitive_skills`. Developmental records stay in their existing `pccs`/`role_skills`/`acquired_skills` buckets (implicitly developmental).
3. **T3 stays pure.** `AgentSkillService.get_profile` is unchanged — it never consults `_catalog`, so a directly-fetched profile has `cognitive_skills == []`. The merge is bridge-only.
4. **No schema change.** The cognitive kind is derived at read time, not stored — no `skills.db` migration.
5. **Optional dept/rank filters.** The keyword-only `department`/`agent_rank` mirror the AD-885 lens pattern and pass straight through to `list_entries`; defaulting both to `None` returns all visible cognitive entries (wildcard `department == "*"` always visible). The `SkillProfile`/`get_profile` API carries no dept/rank today, so the caller supplies them — chosen over inferring crew identity inside the bridge to keep the merge a pure read with no extra dependency.
6. **Honest-degrade.** A catalog query failure is caught and logged Tier-2 (`logger.warning(..., exc_info=True)`) and the method returns the developmental-only profile rather than raising — a cognitive-store outage must not break the unified query.

**Verify-first facts.** The bridge ctor is `SkillBridge(catalog, skill_registry, skill_service)` (the prompt's L22 `SkillBridge(catalog, skill_service)` reference predates the registry arg); `get_profile` is async; `list_entries(department=None, min_rank=None)` returns all entries when both are `None` and filters department by wildcard-or-exact and rank by `_RANK_ORDER`. `from __future__ import annotations` is present in both edited files, so the `-> SkillProfile` return annotation in the bridge (where `SkillProfile` is only `TYPE_CHECKING`-imported) is a lazy string needing no runtime import.

**Untouched.** The `SKILL.md` on-disk format, progressive-disclosure loading (`get_instructions`/`find_by_intent` stay the runtime context-injection path), and the developmental proficiency math.

**Tests.** `tests/test_ad887_skill_library_unification.py` (+7, BF-287 real fixtures — real `SkillRegistry`+`AgentSkillService` on a tmp DB + real `CognitiveSkillCatalog` + real `SkillBridge`): an agent with both kinds reports both; cognitive entries carry `kind="cognitive"` and the required keys; cognitive-only and developmental-only agents resolve; a department filter excludes department-specific entries while keeping wildcards; `get_profile` stays T3-only; a forced `list_entries` exception degrades to developmental-only. AD-428 + AD-596 blast-radius suites green (94) serially. One commit.
...
**Decision.** Append two **additive, read-only** blocks to `get_consolidated_profile`, each guarded with the same `hasattr(runtime, ...) and runtime....` + per-block `try`/`logger.debug` pattern as blocks 5-6, so a missing subsystem omits its keys without crashing and no existing block changes. **(7) Cognitive skills:** when `runtime.cognitive_skill_catalog` is present, `profile["cognitive_skills"] = [{"name": n, "description": d, "skill_id": s} for (n, d, s) in catalog.get_descriptions(department=profile.get("department"), agent_rank=profile.get("rank"))]` plus `profile["cognitive_skill_count"]`. `get_descriptions` (`src/probos/cognitive/skill_catalog.py:353`) returns `list[tuple[name, description, skill_id]]` already filtered by department + rank, so the lens shows exactly what THIS agent is permitted to see (progressive disclosure). **(8) Tool grants:** when `runtime.tool_registry` is present, iterate `tool_registry.list_tools()` and keep each `reg.tool_id` for which `tool_registry.check_permission(agent_id, reg.tool_id, ToolPermission.READ, agent_department=<dept>, agent_rank=<rank or "ensign">)` is true → `profile["tools"]` + `profile["tool_count"]`. The agent's **real** `rank`/`department` come from block 2's crew identity (`profile.get("rank")`/`profile.get("department")`); falling back to the `"ensign"` default would understate a senior agent's grants. `ToolPermission` is imported lazily inside the block (matching block 4's inline `from probos.earned_agency import ...` pattern) to avoid any `tools`-package import-cycle risk at module load.

**Verify-first NO-SEAM: duties omitted.** The epic originally called for three blocks (the third = duties). Verify-first established there is **no public per-agent duty accessor** at HEAD: `runtime.duty_schedule` is a `DutySchedule` scan-policy gate (`should_scan`/`next_scan_window`), not a per-agent lookup; the real `DutyScheduleTracker.get_due_duties(agent_type)` is **private** on `runtime.proactive_loop._duty_tracker` (`src/probos/proactive.py:384`) and reaching it would be a Law-of-Demeter violation; `runtime.duty_schedule_tracker` is aspirational/unshipped. **No duty block was added and `_duty_tracker` is not reached.** Duties join the lens in a future AD that first ships a public `runtime.duty_schedule_tracker` property.

**Why additive-only.** Smallest blast radius first (the epic ships the read-only lens before any subsystem-mutating AD). The method signature, the six existing blocks, and the `acm.db` schema are untouched — AD-885 only reads. Block 3's existing `hasattr`-only (no truthiness) guard means a stub runtime must OMIT `trust_network`/`registry`/`skill_service`/`episodic_memory` attributes rather than set them `None`; the test runtime stub does exactly that.

**Tests.** +8 pytest (`tests/test_ad885_acm_unified_lens.py`, BF-287 real fixtures — real `AgentCapitalService(data_dir=tmp_path)`, real `ToolRegistry()`, real started `CognitiveSkillCatalog(skills_dir=tmp_path)` with an on-disk `SKILL.md`, and a plain `_LensRuntime` stub that holds only the real subsystems under test so the other blocks stay dormant via `hasattr`): cognitive skills present / absent-no-catalog / empty-catalog; tool grants ship-wide / **rank-gated** (a `commander`-only tool is excluded for an ensign and included for the agent's real higher rank — proving the real rank is passed, not the default) / absent-no-registry / empty-registry; and the existing blocks (e.g. `lifecycle_state`) remain present alongside the new keys. Blast radius `tests/test_acm.py` (37) green. One commit.

### BF-609: Per-intent TTL for Yeo's inline mesh reads — network reads need a larger ceiling than local-IO reads (#848)

**Context.** Yeo's AD-869 Tier-2 "do-and-report" path resolves a single read-only intent synchronously in the chat turn via `DmReplyPipeline.step_4h_mesh_read_parse` and renders the result inline. A Captain web lookup returned `"Good catch on the name — let me look that up now. (The web_search lookup came back empty (Agent did not respond in time.))"` — the dispatch fired but the agent never replied inside the TTL.

**Root cause.** AD-869 applied one flat `_MESH_READ_TTL_SECONDS = 5.0` ceiling to every allowlisted intent (`_MESH_READ_INTENT_POOLS`). That is correct for the local-IO reads (`list_directory`, `read_file`, `stat_file`, `search_files` — milliseconds) but wrong for `web_search` / `read_page`: `WebSearchAgent`/`PageReaderAgent` mesh-fetch through the AD-270 rate-limited `http_fetch` path (≥2s per-domain wait + internet round-trip) and then, because both `IntentDescriptor`s carry `requires_reflect=True`, run a synthesis LLM call — realistically 8-20s. The 5s `asyncio.wait_for` in `IntentBus.send` timed out every time, producing the literal `"Agent did not respond in time."` This was a latent defect in AD-869 from day one: the inline-web path could never have succeeded under a 5s ceiling.

**Decision.** Keep the tight default for local reads and add a per-intent override map for the network + LLM-bound reads, rather than raising the global ceiling (which would let a slow local read block the reply for 25s) or making the dispatch async (which breaks the "answer this turn" Tier-2 contract). New constants in `src/probos/cognitive/dm/reply_pipeline.py`: `_MESH_READ_NETWORK_TTL_SECONDS = 25.0` and `_MESH_READ_TTL_BY_INTENT = {"web_search": ..., "read_page": ...}`; the dispatch uses `ttl_seconds=_MESH_READ_TTL_BY_INTENT.get(intent_name, _MESH_READ_TTL_SECONDS)`. The longer wait does **not** weaken the Tier-2 safety boundary — read-only safety is enforced by the allowlist plus the structural fact that consensus (and therefore any mutation) cannot resolve in one turn, not by the latency ceiling. `http_fetch` and writes stay excluded. +4 pytest in `tests/test_ad869_mesh_read_inline.py` (real `IntentBus`/`AgentRegistry` per BF-287): network intents dispatch at 25s, local reads keep 5s, the map covers exactly `{web_search, read_page}`.

### AD-882: Federation node-scope guard — default-safe no-op with a forward seam (Quartermaster-hardening epic, part 8/final — #847)

**Context.** Liveness is checked against the **local** `AgentRegistry` (`WorkItemReconciler.resolve_live_agent`). In a multi-node federated mesh, a work item whose assignee is a live agent on **another node** resolves to `None` locally and would be wrongly reclaimed by this node's periodic sweep — a node stealing another node's in-flight work.

**Verify-first finding.** There is **no per-agent or per-work-item node marker at HEAD.** Node identity exists only at config/peer granularity (`FederationConfig.node_id`, `FederationConfig.enabled` in `src/probos/config.py`; both default to single-node: `node_id="node-1"`, `enabled=False`). `WorkItem.assigned_to` is a bare agent id with no node qualifier, and the registry tracks no owning node. So AD-882 **cannot determine** remote ownership from existing data — a true federation-aware reclaim needs a node marker that does not yet exist. Building that marker (peer gossip of work ownership, a `WorkItem.owner_node` field, registry node tagging) is a federation-layer change well beyond this hardening epic's scope.

**Decision.** Ship the **guard seam** now, default-safe, so the moment a node marker exists the protection is already in place. The convention: an optional `metadata['owner_node']` key future federation code can populate. In the shared `_process_item` (`src/probos/agents/quartermaster.py`), **before** acting on a `clear_and_reroute` decision and **before** the AD-877 attempt counter increments, when `self._federation_enabled` and `metadata.get("owner_node")` is set and `!= self._local_node_id`, the sweep skips with `reason="remote_owner"`, increments `counts["remote_owner_skipped"]`, and returns without unassigning, re-dispatching, or accruing a local attempt. Federation disabled (default), marker absent, or marker == local node → unchanged behavior. **On a single-node deployment the branch is never taken — a pure no-op.** Local node id + federation flag are injected in `startup/finalize.py` `_wire_board_reconciler` from `config.federation.{node_id,enabled}`; constructor kwargs `local_node_id="node-1"` / `federation_enabled=False` mirror them for direct construction. The always-present counts dict gains `remote_owner_skipped` (AD-875 empty-board exact-dict updated).

**Why a seam, not full federation reclaim.** Reversibility + Minimal Authority: the safe default is to **not** touch an item this node cannot prove it owns. Skipping a possibly-remote item is reversible (a later sweep, or the owning node, handles it); reclaiming a remote item is a cross-node mutation with no rollback. The guard chooses the reversible default and leaves real ownership resolution to the federation layer.

**Tests.** +5 pytest (`tests/test_ad882_federation_node_guard.py`, real `WorkItemStore` + real `AgentRegistry` per BF-287): federation-off ignores the marker (item reclaimed), enabled+remote-owner skipped and left untouched (not unassigned, not dispatched), enabled+local-owner reclaimed normally, enabled+no-marker treated as local, remote_owner skip does not bump `reconcile_attempts`. One commit. **Epic #847 complete (AD-877 → AD-884).**

### AD-881: Detect live-but-stalled assignees — coarse, config-gated, default-off (Quartermaster-hardening epic, part 7 — #847)

**Decision:** Add a config-gated, default-off (`0`) staleness signal so an `in_progress` item whose assignee is **live** but has made **no board progress** within `stall_timeout_seconds` is rerouted (`reason="stalled"`) instead of skipped. The sweep owns the clock and threshold and passes a precomputed `is_stalled` boolean into `WorkItemReconciler.classify(wi, *, is_dispatchable, is_stalled=False)` (`src/probos/cognitive/work_reconciler.py`) — keeping the reconciler pure (no clock, no config); when `is_stalled` and the assignee resolves live and `status == "in_progress"`, classify returns `clear_and_reroute reason="stalled"` instead of the usual `in_progress_live_owner` skip. The stall computation lives in the shared `_process_item` (`src/probos/agents/quartermaster.py`): `is_stalled = _stall_timeout_seconds > 0 and status == "in_progress" and float(updated_at) < time.time() - _stall_timeout_seconds` (**strict `<`** older-than), and `counts["stalled"]` (init `0` in `_new_counts`) increments when a `clear_and_reroute` carries `reason="stalled"`. Because the logic is in `_process_item`, it applies to both periodic `reconcile()` and reactive `reconcile_for_agent()`. A stall-reroute flows through the **AD-877 attempt guard** like any other clear_and_reroute (increments `reconcile_attempts`, honours backoff, quarantines at the threshold). New bounded config `stall_timeout_seconds: int = Field(default=0, ge=0, le=86400)` on `WorkBoardReconcilerConfig` (`src/probos/config.py`, transitional default-off — conv #14), injected as `agent._stall_timeout_seconds` in `startup/finalize.py` `_wire_board_reconciler` with a matching `stall_timeout_seconds=0` constructor kwarg.

**Rationale (documented limitation drives the scope):** Verify-first established that `updated_at` is **last board-mutation, not a heartbeat** — `WorkItemStore.update_work_item` (`src/probos/workforce.py:1130`) sets `updated_at = time.time()` on every update, so it advances on claim / status transition / assignment changes routed through the board, but **does not** advance from agent-internal "I'm still working" progress that never touches the board. Therefore `now - updated_at` is a *coarse* "claimed long ago, never progressed" signal, **not** a true liveness heartbeat. That is exactly why this ships **default-off (0)** and why no heartbeat/progress plumbing or new timestamp was added (explicitly out of scope, pending a real progress signal in a later AD). Stall applies only to `in_progress` claimed work — `open`/non-`in_progress` items are never reclaimed as stalled. Passing `is_stalled` into a pure `classify` (rather than giving the reconciler a clock) preserves AD-874's side-effect-free decision service. Tests: `tests/test_ad881_stall_detection.py` (+6, real `WorkItemStore` + real `AgentRegistry` per BF-287 so `resolve_live_agent` resolves a registered live agent; `updated_at` seeded via `create_work_item`): disabled (0) never reclaims a live-old item, enabled reroutes an old live in_progress item with `counts['stalled']++` and a persisted `reconcile_attempts`, a fresh item is not reclaimed, an absent assignee takes the normal `assignee_not_live` path (never counted as stalled), a stall-reroute quarantines at the AD-877 threshold, and the strict-`<` boundary (`updated_at == now - stall_timeout_seconds`) is not stalled. The AD-875 empty-board exact-dict gained the always-present `stalled` key (obsolete-contract guard). One commit.

### AD-880: Reactive reclaim on `AGENT_REMOVED` — additive, default-off, shared per-item path (Quartermaster-hardening epic, part 6 — #847)

**Decision:** Add an event-driven reclaim path that complements (does not replace) the periodic sweep. `AgentRegistry.unregister` (`src/probos/substrate/registry.py`) — the single agent-removal chokepoint reached by `pool.remove_agent → registry.unregister` — emits a new `EventType.AGENT_REMOVED` (`src/probos/events.py`, agent-lifecycle cluster) carrying `{agent_id, agent_type}`. The emit happens **outside** the registry lock (BF-598 re-entrancy safety), only when a present agent was actually popped, and only when a `_emit_event_fn` is bound (late-bound by finalize; an unbound fn is a silent no-op). The Quartermaster gains `reconcile_for_agent(agent_id)` (`src/probos/agents/quartermaster.py`): an honest-degrade scoped sweep that filters the `open` + `in_progress` board to the dead agent's items and reuses a new shared `_process_item` per-item routine. `_process_item` is the inline per-item body of `reconcile()` extracted verbatim, and the 11-key counts dict was extracted into `_new_counts` (single source of truth) — so the AD-877 thrash/quarantine/backoff guards, AD-878 boot-grace skip, and AD-879 oldest-first ordering apply identically to both reclaim paths and `reconcile()` behavior is byte-for-byte preserved. The reactive sweep emits `WORK_ITEM_RECONCILED` with `{**counts, "trigger": "reactive", "agent_id": ...}`, stores an episode, and records the AD-883 `_last_sweep` surface with `trigger="reactive"`. New default-off config `reactive_reclaim: bool = False` on `WorkBoardReconcilerConfig` (`src/probos/config.py`, transitional flag — conv #14); when enabled and the reconciler is otherwise wired, `startup/finalize.py` `_wire_board_reconciler` subscribes a Tier-2-guarded async handler to `AGENT_REMOVED` via `runtime.add_event_listener`, holding the reference on `runtime.board_reactive_reclaim_handler`. When disabled (default) no subscription is created.

**Rationale:** Periodic-only reclaim leaves a crashed agent's in-flight work stranded for up to `interval_seconds` (default 300s); a reactive trigger closes that latency without changing the safety-net cadence. The registry `unregister` is the correct emission point because it is the single chokepoint for agent removal — instrumenting `pool.remove_agent_by_id` (pool-only bookkeeping that never hits the registry) was explicitly avoided. Emitting outside the lock follows the BF-598 idempotency/re-entrancy lesson: event listeners can run arbitrary code, and holding the registry lock across a fan-out invites deadlock/re-entrancy. Extracting `_process_item` + `_new_counts` is DRY (standing order) and the coordination point for the rest of the epic — AD-881's stall detection and AD-882's federation node guard both land inside `_process_item`, so they apply to reactive and periodic reclaim uniformly without duplication. Default-off keeps the reactive path opt-in (conv #14); the periodic sweep remains the always-on guarantee. Tests: `tests/test_ad880_reactive_reclaim.py` (+8, real `AgentRegistry` + real `WorkItemStore` per BF-287, `_Fake` only at the router/runtime-bus boundary): present-agent unregister emits once, absent-id emits nothing, unbound emit-fn doesn't crash, payload carries id+type, `reconcile_for_agent` reclaims only the named agent's items and honours the AD-877 attempt guard, records a `trigger="reactive"` last-sweep, `reactive_reclaim=False` creates no subscription, and `reactive_reclaim=True` wires a handler that reclaims a dead agent's orphan item. One commit.

### AD-883: Reconciliation observability via `QuartermasterAgent.info()` — no new command or endpoint (Quartermaster-hardening epic, part 5 — #847)

**Decision:** Surface the Quartermaster's last reconcile sweep through the **existing introspection path** rather than adding any new operator surface. `QuartermasterAgent.reconcile()` now records `self._last_sweep = {"counts": dict(counts), "at": time.time(), "trigger": "periodic"}` at the end of every pass (initialized to `None` in `__init__` for the never-run state), and a new `QuartermasterAgent.info()` override (`src/probos/agents/quartermaster.py`) calls `super().info()` and adds a `reconciliation` block: `{"last_sweep": None}` when never run, otherwise `{"last_sweep": {...counts...}, "age_seconds": round(time.time() - at, 1), "trigger": ...}`. The block copies the whole counts dict and hard-indexes no individual key, so later-AD counters (`quarantined`, `too_fresh`, `truncated`, `stalled`, `remote_owner_skipped`) never KeyError an older renderer. The summary reaches the Captain automatically via the `agent_info quartermaster` intent (AD-320 introspection delegation → `IntrospectionAgent._agent_info` (agents/introspect.py:271) → `agent.info()`). **No `/board` slash command, no HTTP endpoint, and no `panels.py`/`shell.py` change** were added.

**Rationale:** A verify-first pass found no `/board` command and no quartermaster reference in `experience/`, and the epic's "Do not add a new HTTP endpoint unless required; prefer the existing introspection path" guard pointed directly at `BaseAgent.info()` (substrate/agent.py:159) being overridable and already merged into the introspection render. Overriding `info()` is the minimum-surface change: it reuses the AD-320 self-knowledge path the Captain already uses to inspect agents, keeps the reconciliation state where the agent owns it, and avoids a new command/endpoint that would have to be documented, tested, and maintained. Copying the whole counts dict (instead of formatting a fixed human-readable line) is deliberate forward-compat: every subsequent AD in this epic adds a counter key, and a `.get`-free dict copy renders all of them without a per-AD renderer edit. Tests: `tests/test_ad883_reconcile_observability.py` (+5, real `WorkItemStore` + real `QuartermasterAgent` per BF-287): `reconcile()` records the summary, `info()` includes the `reconciliation.last_sweep` block, never-run reports `last_sweep: None` without crashing, `age_seconds` reflects elapsed time, and `info()` renders cleanly when counts carry later-AD keys. One commit.

### AD-884: Authority scoped to reconcile-only — consensus-free by Reversibility + Minimal Authority (Quartermaster-hardening epic, part 4 — #847)

**Decision:** Recorded and regression-locked the `QuartermasterAgent`'s authority scope. The Quartermaster's authority is **scoped to reconcile-only operations** — `unassign_work_item`, re-`dispatch_work_item`, and the AD-877 metadata `quarantined` flag — all reversible board housekeeping. Per the Reversibility Preference and Minimal Authority axioms, **no consensus gate is required**: the agent performs no destructive, irreversible, or externally-visible side-effecting intents. It declares a single intent (`reconcile_board`) and must never declare a `requires_consensus=True` destructive intent; if a future change needs one, that is a new AD with a consensus design, not a Quartermaster capability. This AD adds **no** consensus gate, quorum vote, or red-team hook — the decision is that none is needed — and records the reasoning plus a regression guard instead. The only code change is one class constant on `QuartermasterAgent` (`src/probos/agents/quartermaster.py`): `RECONCILE_ONLY_INTENTS: frozenset[str] = frozenset({"reconcile_board"})`, the allow-list the guard test asserts the declared descriptor names are a subset of.

**Rationale:** The scoping was previously **implicit** — the agent simply happened not to declare a consensus-gated intent. The Minimal Authority axiom wants scoped capabilities to be explicit and earned, and the Reversibility Preference axiom is the actual justification for the consensus-free design: every Quartermaster mutation is undoable board housekeeping (an item can be re-assigned, re-dispatched, or un-quarantined), so gating it behind a quorum vote would add governance cost with no safety benefit while slowing the very reconciliation sweep that exists to keep the board healthy. Making the scope explicit (the constant + the guard test) converts an accidental property into a deliberate, enforced contract: a future contributor who adds a destructive intent to the agent now trips the guard test rather than silently widening the Quartermaster's authority. Keeping the record in DECISIONS.md rather than a standalone governance doc keeps the architectural-decision trail in one append-only place. Tests: `tests/test_ad884_authority_scope.py` (+3, real `QuartermasterAgent` per BF-287, no LLM): declared `intent_descriptors` names are reconcile-only (`⊆ RECONCILE_ONLY_INTENTS` and `== {"reconcile_board"}`), no declared descriptor sets `requires_consensus=True`, and the agent stays `tier="utility"` / `agent_type="quartermaster"`. One commit.

### AD-878: Boot-race grace period — skip items younger than `min_item_age_seconds` (Quartermaster-hardening epic, part 3 — #847)

**Decision:** Added a per-item boot-race grace period to `QuartermasterAgent.reconcile()` (`src/probos/agents/quartermaster.py`) so a work item created moments before a warm-boot sweep is not reclaimed mid-first-dispatch. A new bounded config field `min_item_age_seconds: int = Field(default=30, ge=0, le=600)` on `WorkBoardReconcilerConfig` (`src/probos/config.py`) is injected onto the live agent in `startup/finalize.py` `_wire_board_reconciler` (`agent._min_item_age_seconds = cfg.min_item_age_seconds`) alongside the AD-876/877 injections, with a matching constructor kwarg (`min_item_age_seconds=30`). The check runs **inside** the per-item Tier-2 `try`, after `md_current = wi.get("metadata") or {}` and **before** the AD-877 already-quarantined / backoff guards and any classification: if `self._min_item_age_seconds > 0` and `wi["created_at"] > time.time() - self._min_item_age_seconds`, the item is counted `too_fresh` and skipped (`continue`). The summary dict always carries the new `too_fresh` counter (default `0`, stable shape for the HXI and the AD-876 ticker logging). Setting the grace to `0` disables the skip entirely. No `WorkItemStatus` change; no change to `list_work_items`.

**Rationale:** Without the grace period there is a race: the dispatch path creates an `open` work item and is about to dispatch it, but a concurrent warm-boot reconcile sweep sees the item as "unassigned + dispatchable", classifies it `clear_and_reroute`, bumps `reconcile_attempts` (AD-877), and competes with the legitimate dispatch — wasting an attempt (moving the item closer to AD-877 quarantine) for an item that was never actually stranded. The grace period is an **always-on safety bound, not a transitional flag** (hence default `30` rather than the convention-#14 default-`False`): a sub-grace item is, by definition, too young to be confidently classified as stranded, so the conservative action is to leave it for the next sweep. The check is placed before the AD-877 attempt/backoff guards specifically so a fresh item neither accrues an attempt nor consumes a backoff window — it is treated as if the sweep never saw it. The bound is `[0, 600]`: `0` disables (for tests and operators who want eager reclaim), `600` (10 min) caps how long a genuinely-stranded item can hide behind the grace. Because the default is always-on, every pre-AD-878 quartermaster test (AD-875/876/877) that drives the sweep with fresh items now sees them skipped; those tests are pre-AD-878-contract and opt out of the grace (`min_item_age_seconds=0` in their `_qm` helper / `cfg.work_board_reconciler.min_item_age_seconds = 0` in the AD-876 warm-boot integration test), and the AD-875 empty-board exact-dict gained the always-present `too_fresh` key — an obsolete-contract fixture update, not a behavior regression. Tests: `tests/test_ad878_boot_grace_period.py` (+6, real `WorkItemStore` per BF-287, `_FakeRouter` only for the dispatch boundary, every item seeds an explicit `created_at`): a fresh item is skipped and never dispatched; an old item (`created_at = now - 120`) is processed normally; the boundary at exactly `now - min_item_age_seconds` is processed (strict `>` comparison); `min_item_age_seconds=0` disables the grace; the `too_fresh` key is present (`0`) when the grace is zero; and a `clear_and_reroute` of an old item does not let a fresh sibling accrue an attempt. One commit.

### AD-879: Deterministic oldest-first scan ordering + starvation guard (Quartermaster-hardening epic, part 2 — #847)

**Decision:** Fixed item starvation in `QuartermasterAgent.reconcile()` (`src/probos/agents/quartermaster.py`). The sweep lists `open` + `in_progress` items via `WorkItemStore.list_work_items(status=…, limit=self._scan_limit)`, whose SQL is `ORDER BY priority ASC, created_at DESC` — newest-first within a priority band. Merged into the id-keyed dict and iterated in insertion order, the **oldest** stranded items sorted last and could fall outside the `scan_limit` window forever. The verify-first pass confirmed the epic's "ordering is already oldest-first; confirm only" branch is FALSE, so AD-879 adds an explicit re-sort of the merged set right after the merge: `ordered = sorted(merged.values(), key=lambda i: (i.priority, i.created_at))` (priority ascending = critical first, then `created_at` ascending = oldest first within a band), and the per-item loop iterates `ordered`. Added a starvation-visibility flag to the summary: `counts["truncated"]` defaults `False` and, when `len(merged) >= self._scan_limit`, is set `True` with one Tier-2 warning (`"Board reconcile truncated: merged=%d >= scan_limit=%d; oldest items prioritized but backlog growing"`). The key is always present in the counts dict. No SQL change to `list_work_items` and no change to the `scan_limit` default (200).

**Rationale:** Re-routing the *oldest* stranded item first is the correct policy — an item that has been stuck longest is the one most in need of reconciliation, and starving it indefinitely behind a stream of newer items defeats the purpose of the sweep. The re-sort lives **in the sweep only**, not in `list_work_items`' SQL, because the create-dispatch path and the REST board view depend on the existing newest-first ordering; changing the shared query to satisfy the reconciler would silently reorder unrelated callers (Open/Closed — extend the consumer, don't mutate the shared primitive). Full backlog drainage beyond `scan_limit` is deliberately out of scope: raising the cap or paginating the whole board is a separate capacity decision, and the honest signal for "the backlog is outgrowing the per-sweep window" is the `truncated` flag plus a warning, not an unbounded scan that could make a single sweep arbitrarily expensive. The per-status DB `limit` still caps the fetch; the re-sort only guarantees that *within whatever was fetched*, the oldest are processed first — so even a truncated sweep makes maximal progress on the items most at risk. `truncated` is always present (not conditionally added) so the counts dict has a stable shape for the HXI and the AD-876 ticker's logging. Tests: `tests/test_ad879_scan_ordering.py` (+5, real `WorkItemStore` per BF-287; a `_FakeRouter` records `dispatch_work_item` call order via the `unassigned_dispatchable → live_redispatch` path, which dispatches in processing order without mutating metadata): processing order is `(priority asc, created_at asc)`, same-priority older-first, lower-priority-number before higher regardless of age, `truncated` True when merged ≥ `scan_limit` (seeded small via the injected `_scan_limit`), and `truncated` False (key present) for a small backlog. The AD-875 empty-board exact-dict test gained the always-present `truncated` key (obsolete-contract guard, not a behavior regression). One commit.

### AD-877: Reconcile-attempt tracking + dead-letter quarantine thrash guard (Quartermaster-hardening epic, part 1 — #847)

**Decision:** Hardened `QuartermasterAgent.reconcile()` (`src/probos/agents/quartermaster.py`) so a permanently-stranded work item can no longer be unassigned and re-dispatched on every sweep indefinitely. Each `clear_and_reroute` action now performs read-modify-write on the item's `metadata` (`update_work_item` REPLACES the metadata column, so the agent reads the current item, mutates its dict, and writes it back): it increments `reconcile_attempts` and stamps `last_reconcile_at = time.time()` after the unassign + before the re-dispatch. When `reconcile_attempts + 1 >= self._max_reconcile_attempts` (config default 3) the item is **dead-lettered** instead: its metadata is stamped `quarantined=True`, `quarantine_reason="max_reconcile_attempts"`, `quarantined_at`, and the attempt counter is persisted; a single `EventType.WORK_ITEM_QUARANTINED` event is emitted (`{"work_item_id", "reason", "attempts"}`); and the item is **left assigned** (NOT unassigned) so it stops thrashing the board. Two early-skip guards run inside the per-item Tier-2 try before classification: an already-`quarantined` item (metadata flag truthy) is skipped and counted as `quarantined_skipped`; an item whose `last_reconcile_at` is within `self._reconcile_backoff_seconds` (config default 600s; `0` disables the cooldown) is skipped and counted as `backoff_skipped`. The summary dict always carries the three new counters (`quarantined`, `quarantined_skipped`, `backoff_skipped`, all `0` on an empty board); `degraded` keeps its AD-875 bool semantics. New bounded config on `WorkBoardReconcilerConfig` (`src/probos/config.py`): `max_reconcile_attempts: int = Field(default=3, ge=1, le=20)` and `reconcile_backoff_seconds: int = Field(default=600, ge=0, le=86400)`, injected onto the live agent in `startup/finalize.py` `_wire_board_reconciler` (`agent._max_reconcile_attempts`, `agent._reconcile_backoff_seconds`) alongside the AD-876 `_scan_limit` injection. New constructor kwargs with the same defaults. `import time` added at module top.

**Rationale:** AD-875's sweep re-dispatches any stranded `clear_and_reroute` item every cadence tick; for an item that is genuinely un-routable (no matching capability, a permanently dead department) that is an unbounded re-dispatch loop — the same work churning the bus and the dispatcher forever. A dead-letter quarantine is the standard bound: after N attempts, stop trying and surface the item for human/Captain attention via a distinct `WORK_ITEM_QUARANTINED` event, rather than silently dropping it or thrashing. Quarantine is recorded as a **metadata flag**, not a new `WorkItemStatus` enum value, deliberately: the work-type transition graph (AD-498) and the store's `_IMMUTABLE_FIELDS` stay untouched, no migration is needed, and the item keeps its real lifecycle status (an `open` quarantined item is still `open` — just flagged "don't reconcile me"). The item is left **assigned** on quarantine (not unassigned) because unassigning it would re-open it to the AD-581a create-dispatch path and defeat the guard; leaving it parked is what actually stops the thrash. Read-modify-write is mandatory because `update_work_item` replaces the metadata column wholesale (the BF-607/BF-608 lesson) — a blind write would erase every other metadata key. The `reconcile_backoff_seconds` cooldown is a second, softer guard: even below the attempt ceiling, an item shouldn't be hammered on back-to-back ticks, so a per-item cooldown gives a transient capability gap time to resolve before the next attempt. Both knobs are bounded Pydantic fields validated at parse time, and both default to safe production values (3 attempts, 10-minute cooldown) so the guard is active the moment the reconciler is enabled — this is a safety bound, not a transitional capability, so it does NOT default off. Tests: `tests/test_ad877_reconcile_thrash_guard.py` (+10, real `WorkItemStore`/`AgentRegistry`/`WorkItemReconciler` per BF-287; `_FakeRouter` only at the dispatch boundary): attempt-counter increment + `last_reconcile_at` stamp on `clear_and_reroute`, reaching the ceiling quarantines-not-redispatches and leaves the assignee untouched, the `WORK_ITEM_QUARANTINED` payload, `max_reconcile_attempts=1` quarantines on first attempt, already-quarantined → `quarantined_skipped` (untouched), recent `last_reconcile_at` → `backoff_skipped`, `backoff=0` disables the cooldown, the empty-board summary always carries the three new keys, and a per-item exception still marks `degraded` while the sibling item is cleared. The AD-875 empty-board exact-dict test was updated to include the three new always-present counters (obsolete-contract guard, not a behavior regression). One commit.

### AD-876: Wire the Quartermaster into the runtime with a cadence ticker (Quartermaster epic, part 3 — closes #846)

**Decision:** Wired the AD-875 `QuartermasterAgent` into boot, shutdown, and a periodic cadence, closing the work-board reconciliation epic. New transitional config `WorkBoardReconcilerConfig` (`src/probos/config.py`): `enabled: bool = False` (conv #14 — transitional flag defaults off), `interval_seconds: int = Field(default=300, ge=30, le=3600)`, `warm_boot: bool = True`, `scan_limit: int = Field(default=200, ge=1, le=2000)`; docstring notes it requires `hybrid_dispatch`. Added to `SystemConfig` as `work_board_reconciler: WorkBoardReconcilerConfig = Field(default_factory=…)` beside `hybrid_dispatch`. The `quartermaster` template is registered in `runtime.py` (`spawner.register_template("quartermaster", QuartermasterAgent)`, mirrors `introspect`), and a single-agent `quartermaster` pool is created in `startup/agent_fleet.py` **only when** `config.work_board_reconciler.enabled`. New `src/probos/mesh/board_reconciler_ticker.py` — `BoardReconcilerTicker(*, agent, interval_seconds, warm_boot, startup_delay=10.0)` owns the cadence (the agent itself has no loop): an idempotent `start()` parks the `asyncio.create_task(self._loop(), name="ad876-board-reconciler")` reference on `self._task`; `_loop()` runs an optional warm-boot reconcile after `startup_delay` then `await self._agent.reconcile()` every `interval_seconds`, each sweep wrapped in `_safe_reconcile()` (Tier-2 log-and-degrade, `CancelledError` re-raised); an idempotent `async stop()` cancels + awaits the task, swallows `CancelledError`, log-warns any other teardown exception, and nulls `self._task`. `startup/finalize.py` gains `_wire_board_reconciler(*, runtime, config) -> bool` (placed between `_wire_hybrid_dispatch` and `_wire_capability_gap_driver`, called inside the finalize try-block right after `_wire_hybrid_dispatch`): returns `False` (INFO no-op) when disabled or when `runtime.work_item_router` (only present with hybrid_dispatch), `work_item_store`, or `registry` is absent; otherwise it builds a `WorkItemReconciler`, resolves the live quartermaster via `registry.get_by_pool("quartermaster")`, injects the collaborators by their exact private attrs (`agent._reconciler/_store/_router/_emit/_episodic/_scan_limit`), constructs and `start()`s the ticker, and parks it on `runtime.board_reconciler_ticker`. `startup/shutdown.py` `await`s `runtime.board_reconciler_ticker.stop()` (after the AD-733-1 attachment_reaper stop, honest-degrade, nulls the attr).

**Rationale:** AD-874 (reusable dispatch + classifier) and AD-875 (the deterministic sweep agent) were deliberately built wiring-free so the cadence and lifecycle could land as a separate, independently-reviewable unit. The ticker owns the cadence rather than the agent so the agent stays a pure `perceive → decide → act → report` sweep with no scheduling concern (Single Responsibility) — the same split as every other ProbOS background loop. `enabled` defaults `False` because a board-reconciliation sweep is a transitional capability that should ship dark and be flipped on deliberately (convention #14 — default-True on a transitional flag is breaking-change-on-first-commit); and because it is gated on `work_item_router`, it is inert unless `hybrid_dispatch` is also enabled, so the default config creates no pool, wires no ticker, and changes no behavior. `_wire_board_reconciler` injects by the exact private attrs the AD-875 constructor reads (not the public kwargs) because the agent is spawned by the pool factory before finalization, so collaborators are injected post-construction — the same late-bind pattern as the other finalize wirers. Every dependency check is INFO-level honest-degrade (a missing router/store/registry/agent is an expected configuration, not an error), and `stop()` is idempotent + non-raising so a duplicate shutdown (sleep/wake SIGTERM, BF-598) tears the ticker down cleanly without becoming an unretrieved task exception. The ticker holds its task reference (asserted by a source scan in the tests, per the fire-and-forget review flag), uses `asyncio.create_task` (never `ensure_future`), and re-raises `CancelledError` so cancellation propagates. Tests: `tests/test_ad876_reconciler_wiring.py` (+12, real `WorkItemStore`/`AgentRegistry`/`SystemConfig` per BF-287; `_FakeRouter`/`_FakeAgent` only at the dispatch/sweep boundary): config defaults + bounds rejection (`interval_seconds`/`scan_limit`), `_wire_board_reconciler` disabled/missing-router/missing-agent no-ops, the enabled injection path (asserts each injected attr incl. `_scan_limit==77` and `runtime.board_reconciler_ticker` set), the ticker task-reference source assertion, warm-boot reconcile-once (`startup_delay=0.0`), idempotent `stop()`, a real-store warm-boot redispatch integration (stranded dispatchable item → `router.dispatched` non-empty), and the default-config gate predicate (no pool, `_wire` returns `False`). Blast-radius gate (runtime/workforce/dispatch/finalize/config/shutdown) 181 green. One commit.

### AD-875: QuartermasterAgent — deterministic work-board reconciliation sweep (Quartermaster epic, part 2)

**Decision:** Added `QuartermasterAgent` (`src/probos/agents/quartermaster.py`), a plain deterministic `BaseAgent` (`tier="utility"`, `agent_type="quartermaster"`, no LLM — mirrors `IntrospectionAgent`) that consumes the AD-874 `WorkItemReconciler` decisions and performs the board mutations. `reconcile()` lists only the live `open` + `in_progress` work items from the `WorkItemStore` (`list_work_items(status=…, limit=scan_limit)`, merged by id; terminal statuses are never scanned), and for each item classifies via `WorkItemRouter.is_dispatchable(wi)` + `WorkItemReconciler.classify(wi, is_dispatchable=…)`, then executes the action: `live_redispatch → router.dispatch_work_item(wi)`; `clear_and_reroute → store.unassign_work_item(id, reason=…)` then re-dispatch the freshly-read cleared item; `skip → count only`. Returns a `{scanned, redispatched, cleared, skipped, degraded}` counts dict. Per-item failures are tier-2 log-and-degrade (`degraded=True`, sweep continues to the next item); a missing collaborator (`store`/`router`/`reconciler` is `None`) returns a degraded zero-summary without raising. On completion it emits a single `EventType.WORK_ITEM_RECONCILED` (new enum value, added after `WORK_ITEM_STATUS_CHANGED`) carrying the counts, and stores an optional episodic `Episode` summary (lazy import, tier-2). The agent follows the `perceive → decide → act → report` lifecycle; `handle_intent` services only `reconcile_board` (any other intent → `None`).

**Rationale:** AD-874 split the pure classifier (`WorkItemReconciler.classify`) from any sweep loop precisely so AD-875 could add the *mechanical* half — listing the board, calling the classifier, and mutating — as a separate, independently-testable unit. The Quartermaster is the agent that owns those board mutations, kept deliberately LLM-free: re-dispatching a stranded item or clearing a dead assignee is deterministic policy, not cognition, so it is a utility-tier `BaseAgent` rather than a `CognitiveAgent` (no instructions, no reasoning, no consensus). Scanning only `open` + `in_progress` keeps the sweep cheap and naturally excludes `done`/`failed`/`cancelled` (the classifier's `terminal` branch is a defensive belt that the status filter already exercises). The single `WORK_ITEM_RECONCILED` summary event (not per-item events) keeps the bus quiet — one event per sweep, with aggregate counts for the HXI — and the counts dict is the agent's machine-readable report for the future ticker (AD-876) to log/trend. Honest-degrade everywhere (per-item and missing-collaborator) means a misbehaving store/router can never crash the sweep or the eventual background ticker; a degraded sweep reports `degraded=True` rather than silently completing. No ticker, startup wiring, pool creation, or config yet — AD-876 wires it. Tests: `tests/test_ad875_quartermaster.py` (+12, real `WorkItemStore` + real `AgentRegistry` + real `WorkItemReconciler` per BF-287; a `_FakeRouter` records dispatch calls and reads `metadata["dispatchable"]` at the dispatch boundary): live-assignee/dead-assignee/in-progress-owner/non-dispatchable/empty-board/per-item-error/missing-collaborator sweeps, the `reconcile_board` lifecycle round-trip, non-matching intent → `None`, utility tier, and a decomposer gap-regex hygiene check on the descriptor strings. One commit.

### AD-874: Reusable work-item dispatch path + deterministic stranded-item classifier (Quartermaster epic, part 1)

**Decision:** Two changes, no behavior change to live paths. (1) Refactored the AD-581a `WorkItemRouter` (`src/probos/mesh/work_item_router.py`) for Open/Closed reuse: extracted `async dispatch_work_item(wi: dict) -> None` containing everything from the `is_dispatchable` early-return through `DepartmentDispatcher.route` (direct/broadcast `RoutingDecision`), `TaskEvent` construction (`task_event_for_agent` / `task_event_broadcast`), the `HYBRID_DISPATCH_DIRECT` / `HYBRID_DISPATCH_BROADCAST` emits, and `dispatcher.dispatch`. `on_work_item_created(event)` now only unwraps the `{"data": {"work_item": …}}` envelope and delegates to `dispatch_work_item`, keeping its tier-2 log-and-degrade wrapper. (2) Added `WorkItemReconciler` (`src/probos/cognitive/work_reconciler.py`), a pure deterministic classifier: `resolve_live_agent(assigned_to)` returns the live agent slot for an assignee by checking `AgentRegistry.get` first, then walking the AD-441 sovereign-DID seam (assignee slot → `AgentIdentityRegistry.get_by_slot` birth-cert `agent_uuid` → the live registry agent whose own cert carries the same `agent_uuid`); `classify(wi, *, is_dispatchable)` returns a frozen `ReconcileDecision(work_item_id, action, assignee, resolved_agent_id, reason)` with `action ∈ {skip, live_redispatch, clear_and_reroute}` and reasons `terminal`, `not_dispatchable`, `unassigned_dispatchable`, `in_progress_live_owner`, `assignee_live`, `assignee_not_live`. All collaborator access is tier-2 log-and-degrade.

**Rationale:** The Quartermaster reconciler (AD-875) must re-dispatch work items stranded by agent re-spawns (slot churn from warm boot / AD-441 sovereign identity), but the only dispatch logic lived inside the `WORK_ITEM_CREATED` event listener, which a sweep cannot reuse without faking an event envelope. Extracting `dispatch_work_item` gives both the create-listener and the future reconciler one governed dispatch path (same `DepartmentDispatcher`, same `TaskEvent` shape, same emits) — Open for the new caller, Closed against duplicating routing. The classifier is deliberately split from any sweep loop or wiring: it is a side-effect-free function of `(work_item_dict, is_dispatchable)` so it can be unit-tested against a real registry with no runtime, and so the policy (which states get re-dispatched vs cleared vs left alone) is auditable in isolation before AD-875 adds the loop and AD-876 wires the ticker. `resolve_live_agent` reuses the existing AD-441 birth-certificate seam rather than introducing a new agent-identity map: an assignee recorded against a now-dead slot is still the *same sovereign agent* if a live slot shares its `agent_uuid`, so the work should follow the agent, not be orphaned. No sweep, no scheduling, no LLM (this is mechanical reconciliation, not cognition). Tests: `tests/test_ad874_work_reconciler.py` (+13, real `AgentRegistry` + real `AgentIdentityRegistry` per BF-287) and `tests/test_ad839_work_item_dispatch.py` (+4 — direct and broadcast routing identical through `dispatch_work_item`, non-dispatchable early-return, `on_work_item_created` still routes via the shared path). One commit.

### BF-608: Enforce AD-498 `requires_assignment` at the store boundary (a task cannot enter `in_progress` while `Unassigned`)

**Decision:** Made `WorkItemStore.transition_work_item` (`src/probos/workforce.py`) honor the `requires_assignment` flag that AD-498 attached to state-machine transitions but never read. New `WorkTypeRegistry.transition_requires_assignment(type_id, from_status, to_status) -> bool` exposes the per-edge flag (unknown type / unknown edge → `False`, matching `validate_transition`'s backward-compat stance). Immediately after the AD-498 `validate_transition` check — where `item.assigned_to` is available — `transition_work_item` now refuses the move when `item.assigned_to is None` and the edge is flagged `requires_assignment` (e.g. `task` `open → in_progress`, `work_order`/`incident` `scheduled/open → in_progress`): it logs a `BF-608` warning and returns `None`, leaving the item in its prior (dispatchable) status. Two legitimate consumers that promote an unassigned item to `in_progress` were updated to claim an owner first: the AD-867 crew **parent** promotion (`crew_orchestrator._promote_parent`) now assigns the coordination-container parent to `"crew_orchestrator"` before transitioning (its children already carry the real per-agent assignments via `_assign_child`); the BF-607 dispatch handler and `crew_executor` (children) already assign before transitioning, so they were unaffected.

**Rationale:** BF-606 and BF-607 each addressed one facet of Captain-reported work item `1e0ffcdb7b57` ("Bayesian trust calibration best practices", `yeo-delegated`) showing `in_progress` / `Unassigned`, but the item *stayed* unassigned after BF-607 shipped. Ground-truth inspection of the live runtime API (`/api/work-items`) confirmed the item was created at 11:03 and reached `in_progress` at 11:08 — **before** the BF-607 server restart at 15:01 — so it is a stale orphan from pre-fix code that the `WorkItemRouter` (which dispatches only on `WORK_ITEM_CREATED`) never re-dispatches. More importantly, BF-607 only claims on the *direct*-dispatch path (`intent.target_agent_id == self.id`); this item reached `in_progress` via a broadcast dispatch / the AD-855 capability-gap `blocked → in_progress` resume, neither of which runs the claim. The true root cause is upstream of all of these: AD-498 defined `WorkTypeTransition("open", "in_progress", requires_assignment=True)` to encode "you cannot start work without an owner", but `validate_transition` only checked that the edge *exists*, never the flag — so **any** path could drive an unassigned item into `in_progress` and the board faithfully rendered the contradictory state. Enforcing the flag at the single store-boundary chokepoint (where `assigned_to` is known) makes the contradictory state unreachable from every path — direct dispatch, broadcast, AD-855 resume, the REST transition endpoint — without the store needing to know about the agent registry (layer discipline preserved: the store rejects rather than auto-assigning a synthetic owner). A refused transition leaves the item `open` and dispatchable, which is the honest state (nobody has claimed it) rather than a lie (in-progress with no owner). The crew parent is the one genuine "no single worker" case — it is a coordination container, so it is claimed for the orchestrating subsystem (`"crew_orchestrator"`), which is both correct (the orchestrator owns the parent) and more honest on the board than `Unassigned`. The stale orphan itself predates the fix and is cleaned up out-of-band via the live API; BF-608 prevents recurrence. Tests: `tests/test_workforce.py` (+5 — refusal of an unassigned `open → in_progress` with no DB write / no `STATUS_CHANGED` emit, the `BF-608` warning, an assigned item transitions, the AD-855 `blocked → in_progress` resume keeps an existing owner, and the `transition_requires_assignment` registry helper across task/work_order/unknown edges) plus 9 existing transition tests updated to assign an owner before promoting; `tests/test_ad867_crew_orchestrator.py` (parent-promotion now asserts `assigned_to == "crew_orchestrator"`). **Reconciliation:** this commit also lands the **BF-606** same-status no-op guard (`if new_status == item.status: return item`, before `validate_transition`) and its two tests — BF-606 was *documented* in `a33f67bd` but its `workforce.py`/`test_workforce.py` code was never committed (only the banners landed), so HEAD's runtime never actually had the BF-606 fix. The BF-606 and BF-608 changes are entangled in the same hunks (the BF-606 same-status tests' setup transition into `in_progress` now requires BF-608 assignment), so a clean per-BF split is not safely separable; they ship together and HEAD's code now matches its BF-606 docs. One commit.

### BF-607: Dispatched work item is claimed by the handling agent (stop the `in_progress` / `Unassigned` board state)

**Decision:** When a `CognitiveAgent` handles a directly-dispatched work item, `_handle_work_item_dispatch` (`src/probos/cognitive/cognitive_agent.py`) now claims the item for itself — it reads the current item and, only if `assigned_to` is empty, calls `store.update_work_item(work_item_id, assigned_to=self.id)` immediately before the existing `transition_work_item(id, "in_progress")`. An item that the Captain or crew orchestrator already assigned is left untouched (no reassignment). The claim is tier-2 log-and-degrade: a failure logs a warning and the item still transitions to `in_progress`.

**Rationale:** A Captain-reported board screenshot showed a `yeo-delegated` task as `in_progress` + `dispatched` with `Assigned to: Unassigned`. The AD-581a `WorkItemRouter` resolves a direct agent (`decision.agent_id`) and emits a `work_item_dispatched` task event to it, but it never persists the assignment to the store — and the AD-839 dispatch handler only transitioned status, so the board faithfully reported work running with nobody assigned. The handling agent is the correct place to record the assignment: it is the one actually doing the work, so claiming `assigned_to = self.id` makes the displayed assignee reflect reality. The `if not current.assigned_to` guard preserves the Captain/crew-orchestrator push-assignment path (those set `assigned_to` at create/delegate time, e.g. `crew_orchestrator._assign_child`), so this only fills in the gap left by router dispatch. `assigned_to` is a mutable field (`update_work_item` writes it; it is not in `_IMMUTABLE_FIELDS = {"id", "created_at", "created_by"}`). Single-point fix co-located with the in_progress transition; the dispatch re-run / acknowledgment behavior is untouched. The concurrently-reported `tier=standard` empty-content LLM warnings were diagnosed as a transient (the config pins `llm_model_standard: claude-sonnet-4.6` and the live proxy log confirmed Sonnet 4.6 serving healthy responses; AD-632c caught the one-off empty completion and AD-643a fell back and recovered) — no code change for that. Tests: `tests/test_ad839_work_item_dispatch.py` (+3 — claims unassigned, does not reassign already-assigned, log-and-degrades a failed claim while still transitioning); the existing 5 AD-839 tests stay green. One commit.

### BF-606: Same-status work-item transition is an idempotent no-op (stop the `in_progress → in_progress` warning loop)

**Decision:** Made `WorkItemStore.transition_work_item` (`src/probos/workforce.py`) treat a transition to the status the item is already in as an idempotent no-op success. A new early guard — `if new_status == item.status: return item` — runs immediately after the item is fetched and *before* the AD-498 `validate_transition` state-machine check. For a same-status request it returns the item unchanged: no DB write, no `WORK_ITEM_STATUS_CHANGED` event, and no `logger.warning("Invalid transition …")`. The pre-existing `test_transition_from_terminal_status_rejected` was relying on the old behavior by accident (it created a `task`, called `transition_work_item(id, "done")` which is **not** a legal `open → done` transition for `task`, so the item never left `open`, then asserted `open → open` returned `None`); its setup was corrected to reach `done` via the valid `open → in_progress → done` path so it genuinely exercises terminal-status rejection.

**Rationale:** A crash forensic of the live runtime (process hard-killed 2026-06-05 11:28:08) surfaced a death-spiral pathology: work item `1e0ffcdb7b57` logged `Invalid transition for 1e0ffcdb7b57: Work type 'task' does not allow transition 'in_progress' → 'in_progress'` dozens of times. Root cause: `work_item_dispatched` is delivered **at-least-once** — it broadcasts to every crew agent (each recipient's `_handle_work_item_dispatch` calls `transition_work_item(id, "in_progress")` at the end), the AD-855 capability-gap `_resume` path sets an item to `in_progress` and then re-dispatches it, and the bus can redeliver — so an already-`in_progress` item is repeatedly re-driven. The AD-498 state machine has no `in_progress → in_progress` edge, so each redundant delivery logged a warning and returned `None` (which `_resume` and other callers read as a *failure*). The idempotent-consumer pattern is the correct fix for at-least-once delivery: transitioning to the current state is success-by-definition, not a state-machine violation. The guard sits before `validate_transition` so it cannot interact with terminal-status logic (a terminal item asked to move to a *different* status still hits the existing rejection; asked to "move" to its own terminal status is a harmless no-op). This is a single-point fix at the store boundary — it does not touch the dispatch handler's re-run behavior, so the AD-855 resume re-dispatch keeps working (it now gets the item back instead of a spurious `None`). The broadcast/redelivery amplification of the *agentic re-run* is a separate, broader concern (IntentBus fan-out) left for a follow-up. Tests: `tests/test_workforce.py` (+2 — same-status returns the item with zero emits; same-status emits no `Invalid transition` warning) plus the corrected terminal-status test. One commit.



**Decision:** Updated the count assertion in `ui/src/components/icons/Glyphs.test.tsx` from `expect(count).toBe(25)` to `29` to match the 29 glyph components currently exported from `ui/src/components/icons/Glyphs.tsx`. Comment updated to note the +4 drift since BF-041.

**Rationale:** Four glyphs were added to `Glyphs.tsx` (last touched 2026-05-31) without updating the hardcoded export-count guard in its test (last touched 2026-04-26), leaving the test red on `main` — unrelated to and discovered during BF-604. The count is derived (`Object.keys(Glyphs).filter(typeof === 'function').length`), so the assertion is purely a tripwire that a glyph was added/removed deliberately; bumping it to the real count restores the tripwire. Trivial, low-risk stale-constant fix; one commit.
 Board layout — clear the tab bar, resizable WORK columns, auto-hide the empty KANBAN tab

**Decision:** Fixed three HXI board complaints from the Captain in one cohesive UI change (no backend touched). (1) **Overlap:** the crew WORK board (`ui/src/components/work/WorkBoard.tsx`) and the Mission-Control KANBAN board (`ui/src/components/bridge/FullKanban.tsx`) rendered their headers under the fixed `ViewSwitcher` tab bar (`position: fixed; top: 12; left: 12`). Both root containers now reserve `paddingTop` clearance (`TAB_BAR_CLEARANCE = 40` on WORK, matching `40` on KANBAN) with `boxSizing: 'border-box'`, so the toolbar sits below the floating tabs. (2) **Resizable columns:** WORK columns were fixed equal-flex (`flex: 1; minWidth: 160`). Added `colWidths` state (`Record<ColKey, number | null>`, all `null` = auto) and a `startResize` pointer handler; each non-terminal column renders a `role="separator"` drag handle on its right edge (`cursor: col-resize`). A resized column switches to explicit `width: Xpx; flex: '0 0 auto'`; un-resized columns keep `flex: 1; minWidth: MIN_COL_WIDTH (140)`. (3) **Two-board confusion:** the KANBAN tab is now hidden when there are no active builds — `ViewSwitcher` (`ui/src/components/ViewSwitcher.tsx`) reads `missionControlTasks` and only includes the KANBAN tab when it is non-empty, with a `useEffect` that redirects `mainViewer` from `'kanban'` → `'work'` if the tab is hidden out from under the viewer. Tests: `ui/src/__tests__/WorkBoard.bf604.test.tsx` (+3 — top clearance, one resize handle per non-terminal column, col-resize cursor), `ui/src/__tests__/ViewSwitcher.bf604.test.tsx` (+4 — KANBAN hidden when no builds, shown when builds active, redirect-when-hidden, no-redirect-while-active); the existing `ComponentRendering.test.tsx` ViewSwitcher case was updated to seed a build so the full tab set renders. One commit.

**Rationale:** The Captain reported the board overlapping the menu, asked why WORK and KANBAN both exist, and noted KANBAN was always empty. WORK (WorkItemStore, AD-496/497) and KANBAN ("Mission Control" build/design pipeline, BuildQueueManager, AD-322/325) are **different systems that share a kanban-style UI** — different actors (crew agents vs. Architect/Builder), different stores, different semantics — so merging them would conflate crew task tracking with the self-modification build queue. The right fix is progressive disclosure (HXI Design Principle #5): KANBAN only surfaces when it has content, so a Captain with no active builds sees one board, not two, and the empty-board confusion disappears without removing the pipeline view. The overlap fix is a pure layout offset (the tab bar is `position: fixed`, so it does not participate in the board's flow and the board must reserve space for it). Resizable columns honor the Captain's direct request and use a pointer-drag handle rather than a library, keeping the dependency surface flat. The `useEffect` redirect is load-bearing: without it, a Captain sitting on the KANBAN view when the last build completes would be stranded on a now-tabless viewer.

### BF-603: Intent self-tag must tolerate quoted values and strip from any position (no leak to the Captain)

**Decision:** Hardened the AD-722a intent self-tag parse/strip regexes in `src/probos/avatars/divergence_detector.py` so the `<intent emotion=NAME>` tag can never reach the Captain. Three regex-only changes (no wiring touched): (1) `_TAG_RE` (parse, searches anywhere) now accepts an optional quote around the value — `<intent\s+emotion\s*=\s*["']?([a-zA-Z_]+)["']?\s*/?\s*>`. (2) `_TAG_STRIP_RE` gains the same optional-quote tolerance AND drops the trailing `\s*$` end-of-line anchor plus the `re.MULTILINE` flag, so it matches the tag at any position (leading / inline / trailing) and consumes adjacent whitespace. (3) `strip_intent_self_tag` now does `_TAG_STRIP_RE.sub(" ", text).strip()` (was `.sub("", text).rstrip()`) — collapsing each match to a single space so inline removal of a mid-prose tag never glues the surrounding words together, then trimming the result.

**Rationale:** Yeo (YeomanAgent) leaked a literal `<intent emotion="warm">` at the head of a 1:1 DM reply. The AD-722a self-tag is operator opt-in (`avatar_telemetry.divergence_detection`, default OFF); when ON, the LLM is instructed to append `<intent emotion=NAME>` and `apply_divergence_check` (single call site, `routers/agents.py:agent_chat`) parses + **unconditionally strips** it before `mark_reply_emitted` — "the tag MUST NEVER reach the Captain." The instruction teaches the unquoted form, but the model emitted a **quoted** value, which the value class `[a-zA-Z_]+` could not match: the tag therefore failed BOTH to parse (divergence signal silently lost) and to strip (raw tag leaked). The strip regex was additionally anchored to end-of-line, so even an unquoted tag placed at the start (as Yeo did) would not have been removed. The fix attacks both gaps at the single source of truth — the two module-level regexes — rather than touching the call site, preserving the AD-722a single-call-site invariant. `sub(" ")` + `strip()` replaces `sub("")` + `rstrip()` so that an inline tag (`"Acknowledged <intent emotion='warm'> Captain."`) collapses to `"Acknowledged Captain."` instead of `"Acknowledged Captain."` losing the space; trailing/leading cases trim cleanly. Idempotency and the prose-safety guarantee (`"I am intent on warmth."` untouched) are preserved and pinned by tests. Tests: `tests/test_ad722a_divergence_detector.py` (+7 — double/single-quoted parse, quoted self-closing, the exact leading-quoted leak shape, leading-unquoted strip, inline-no-word-merge, quoted idempotency). The full 91-case AD-722a/AD-737 divergence + taxonomy suite stays green. One commit.

### BF-602: Ward Room routing must quiesce on shutdown (no fire-and-forget post into a closed DB)

**Decision:** Gave `WardRoomRouter` (`src/probos/ward_room_router.py`) an explicit shutdown teardown and made its coalesced-routing path crash-safe during shutdown. Four changes: (1) two new instance attributes — `_stopping: bool = False` and `_coalesce_fire_tasks: set[asyncio.Task[None]]` — plus a new sync, idempotent `stop()` method that sets `_stopping = True`, cancels every pending `_coalesce_timers` `TimerHandle` (and clears the dict), and cancels every in-flight `_fire` task (and clears the set). (2) An early-return guard `if self._stopping: return` at the top of `route_event_coalesced`. (3) The same guard at the top of `route_event` (after the existing `if not self._ward_room` check) so the direct routing path is also suppressed once shutdown begins. (4) `route_event_coalesced` no longer schedules `_fire` as a bare `asyncio.create_task`; a new `_spawn_fire` closure holds the task in `_coalesce_fire_tasks` (drained via `task.add_done_callback(self._coalesce_fire_tasks.discard)`), guards on `_stopping`, and `_fire` itself re-checks `_stopping`, then wraps `route_event` in a try/except that re-raises `asyncio.CancelledError` and log-and-degrades any other `Exception`. Wiring: `startup/shutdown.py` calls `runtime.ward_room_router.stop()` immediately after the BF-296 Phase A intent-dispatch close — before the explicit consolidation `dream_cycle` — inside a Tier-2 honest-degrade try/except (absent router/attr on transitional procs logs a warning and proceeds).

**Rationale:** On shutdown the explicit dream_cycle drives agents to post to the Ward Room (a real shutdown log showed `yeoman` running `proactive_think` during consolidation). Each `ward_room_post_created` event schedules a ~200ms AD-616 coalesce timer whose `_fire()` callback calls `route_event → get_thread`. The timer was a fire-and-forget `asyncio.create_task` with no reference held, so when it fired *after* the ward_room aiosqlite connection was torn down it raised `ValueError('no active connection')` that nobody awaited — surfacing as the noisy `asyncio: Task exception was never retrieved` traceback at the very end of shutdown. The fix attacks all three latent faults the bug exposed: the **ordering** fault (routing kept running after the DB closed) is closed by quiescing the router at BF-296 Phase A, before any shutdown post is created; the **fire-and-forget** fault (exceptions silently lost / unretrieved) is closed by holding the task reference and draining it on done; and the **defense-in-depth** fault (a late or in-flight fire could still race) is closed by the `_stopping` guards on both routing entry points plus the log-and-degrade wrapper in `_fire`. `stop()` is synchronous because it only cancels handles/tasks (no awaitable teardown) and idempotent because shutdown can re-enter (BF-598). The guard reads a plain `bool` attribute always set in `__init__`; no defensive `getattr` was added to production code (the one partial-construction unit test in `test_ad616_router_hot_path.py` got `_stopping = False` added to its `__new__` fixture, mirroring the BF-598 precedent where MagicMock/partial doubles were given the new guard attribute). Tests: `tests/test_bf602_ward_room_shutdown_quiesce.py` (11 — `stop()` exists/sync/idempotent, sets the flag, cancels a pending coalesce timer + clears it, cancels an in-flight `_fire` task; `route_event`/`route_event_coalesced` no-op when stopping while the normal path still schedules; the closed-DB regression where `_fire`'s `route_event` raises `ValueError('no active connection')` is caught and degraded with a BF-602 warning rather than left unretrieved; the in-flight `_fire` task reference is held while routing and drained on completion; and a `startup/shutdown.py` source assertion that it calls `ward_room_router.stop()`). Real-object fixtures (`WardRoomRouter.__new__` + minimal attributes, `AsyncMock` only for the `route_event` collaborator being simulated) per the no-MagicMock-at-substrate-boundary rule.

### AD-873: Ebbinghaus episode decay + composite retrieval reranking

**Decision:** Closed the Memory Epistemics wave (the third and final unit after AD-871 provenance + AD-872 forge observability) with a forgetting curve over episodic memory and a config-gated composite recall reranker. Five changes: (1) Two new last-position fields on the frozen `Episode` dataclass (`src/probos/types.py`): `strength: float = 1.0` and `stability: float = EBBINGHAUS_DEFAULT_STABILITY_SECONDS`, plus the module constant `EBBINGHAUS_DEFAULT_STABILITY_SECONDS: float = 1_728_000.0` (20 days). (2) Two pure, fully-typed decay helpers in `src/probos/cognitive/episodic.py`: `decayed_strength(strength, stability, delta_seconds) -> float` (`strength * math.exp(-delta_seconds / stability)`, guarding `stability <= 0` and `delta_seconds <= 0` by returning `strength` unchanged) and `reinforced_stability(stability, activation) -> float` (`stability * (1.0 + _STABILITY_REINFORCEMENT_GAIN * activation)` with `_STABILITY_REINFORCEMENT_GAIN = 0.25`, guarding a non-positive activation). (3) `EpisodicMemory.sweep_episode_decay(activation_tracker=None, *, limit=500) -> dict[str, int]` — reads one page of episode metadata via `asyncio.to_thread(self._collection.get, ...)`, batch-reads activations (honest-degrade → `{}`), and per episode recomputes `stability` **idempotently from the default baseline** (`reinforced_stability(EBBINGHAUS_DEFAULT_STABILITY_SECONDS, activation)`), decays `strength`, and persists via the AD-608 `update_episode_metadata` read/merge/update primitive; returns `{"swept": N, "reinforced": M}`. (4) A `DreamingEngine` Step 7f2 decay sweep (`src/probos/cognitive/dreaming.py`, between Step 7f and 7g) that calls `sweep_episode_decay(self._activation_tracker)` during the offline dream cycle. (5) Config-gated composite recall reranking: `EpisodicMemory.recall` over-fetches (`k * 10` instead of `k * 3`) and reranks via a static geometric `_composite_recall_score` (similarity baseline multiplied by `_clamp01(field) ** weight` for strength / recency=`exp(-age_hours/168.0)` / importance / confidence, each `if w:` skipped when its weight is 0) when `self._recall_rerank_enabled`, preserving the AD-607a anomaly gate and the relevance threshold; gated by two new `__init__` kwargs (`recall_rerank_enabled: bool = False`, `recall_rerank_weights: dict[str, float] | None = None`) backed by two new `MemoryConfig` fields (`config.py`) and wired at the primary `__main__.py` boot site (the migration CLI site left at defaults → rerank disabled). The new fields round-trip through ChromaDB via `_episode_to_metadata` (`float(ep.strength)` / `float(ep.stability)`) and `_metadata_to_episode` (float reads with defaults `1.0` / `EBBINGHAUS_DEFAULT_STABILITY_SECONDS`).

**Rationale:** Provenance (AD-871) gave a memory graded belief and forge observability (AD-872) made the designer measurable; the missing third piece was that memory had no notion of *forgetting* — every episode was equally retrievable forever, so stale low-importance traces competed with fresh load-bearing ones at recall time. The Ebbinghaus forgetting curve (`strength = e^(-Δt / stability)`) is the canonical model: a reinforced (recently-activated) memory grows its `stability` (the time constant), so it decays slower; an un-reinforced one fades. Recomputing `stability` **idempotently from the default baseline** rather than compounding the stored value is load-bearing — a decay sweep that ran twice (two dream cycles before a new activation) must not double-reinforce, so the sweep is safe to re-run. Decay runs only in `DreamingEngine` (the offline consolidation path), never on the recall hot path, so retrieval latency is untouched. The composite reranker is **disabled by default** (`recall_rerank_enabled=False`) so zero-config boot behavior is byte-identical to pre-AD-873 semantic-only recall; the geometric form (multiply, not add) means any single near-zero factor suppresses a result (an AND of soft constraints), and an all-zero weight map collapses `_composite_recall_score` back to the raw similarity so a misconfigured weight set degrades to semantic-only rather than to noise. The over-fetch (`k * 10`) gives the reranker a candidate pool to reorder while the relevance threshold and AD-607a anomaly gate still bound what is eligible. Every new seam honest-degrades: a missing/throwing activation tracker yields an empty batch (decay still proceeds with `activation=0`), a per-episode persist failure logs and continues the sweep, the dream-cycle Step 7f2 wraps the whole call in Tier-2 log-and-degrade so a decay failure never aborts consolidation, and the ChromaDB read seam defaults malformed `strength`/`stability` values so pre-AD-873 rows recall cleanly with neutral defaults — zero migration. Tests: `tests/test_ad873_episode_decay.py` (25 — decay-helper guard matrix, geometric composite-score incl. all-zero-weights→similarity, strength/stability ChromaDB round-trip on a real `EpisodicMemory`/`tmp_path` fixture, idempotent decay sweep against a real memory + fake/broken activation trackers, rerank ordering, and a real `DreamingEngine`/`MockEpisodicMemory` Step-7f2 integration that seeds an episode before constructing the engine — the dream cycle early-returns on empty memory), real fixtures per BF-287; `tests/test_episodic.py` + `tests/test_dreaming.py` + `tests/test_ad871_provenance_envelope.py` (70) green. Closes [#845](https://github.com/seangalliher/ProbOS/issues/845). Final unit of the Memory Epistemics wave (AD-871 → AD-872 → AD-873). Verify all changes comply with the Engineering Principles in `.github/copilot-instructions.md`.

### AD-872: Forge observability for the self-modification pipeline

**Decision:** Added a new pure, fully-typed, side-effect-free module `src/probos/cognitive/forge_observability.py` with three units and read-only honest-degrade wiring into `SelfModificationPipeline`. (1) `validate_forge_shape(intent_name: str, intent_description: str, parameters: dict[str, str] | None) -> list[str]` — a cheap pre-design gate returning error strings (empty list = proceed, mirroring `CodeValidator.validate`'s contract shape) for an empty/whitespace name, a degenerate (<2 char) name, a non-alphanumeric name, an empty description, `None` parameters, or an empty parameter key; an empty `{}` param map is explicitly **valid** (a no-parameter intent). (2) `classify_forge_rejection(record, validator_errors: list[str] | None = None) -> str` — deterministic bucketing into `syntax_error | forbidden_import | schema_nonconformance | judge_correctness | dependency_declined | dependency_failed | user_rejected | max_limit | design_failed | failed_sandbox | failed_registration | shape_rejected | other`, duck-typed on `.status`/`.error`; `failed_validation` combines `validator_errors` + `record.error` and substring-matches (`syntax error`, `forbidden import`, `judge`/`correctness`, schema signals) defaulting to `schema_nonconformance`; a direct status→bucket map covers the rest; an explicit `else → "other"` fallback means a future/unseen status (`failed_pool`, unknown) never crashes the aggregator. (3) `ForgeStatsAggregator(records: list)` — materializes a local copy of the records and reports `total_attempts`, `total_unique_intents`, `attempt_approval_rate`, `unique_intent_approval_rate`, `rejection_histogram: dict[str, int]`, and a flat `summary()` dict; `active`/`removed` count as approvals. Wiring in `src/probos/cognitive/self_mod.py`: a top-level import of `ForgeStatsAggregator`/`validate_forge_shape`; `forge_stats(self) -> ForgeStatsAggregator` returning `ForgeStatsAggregator(list(self._records))`; and a Tier-2 honest-degrade shape gate at the very top of `handle_unhandled_intent` that, on a non-empty error list, appends a `DesignedAgentRecord(status="shape_rejected", error="; ".join(shape_errors))` to `_records` and returns `None` before the design call. The optional `EventType.FORGE_SHAPE_REJECTED` was **not** added.

**Rationale:** The forge was a black box — only a `status` string and scattered logs, so designer-quality drift (rising rejection rate, retry storms on a single intent) was invisible. The three units make throughput measurable without touching the design/validate/sandbox path: `validate_forge_shape` short-circuits obviously-malformed requests *before* the expensive LLM design call (a pure gate, no side effects), `classify_forge_rejection` turns the flat status string into actionable rejection buckets, and `ForgeStatsAggregator` separates `unique_intent_approval_rate` from `attempt_approval_rate` — the load-bearing distinction, because a 3-retry recovery on one intent is one success, not three. The aggregator materializes its own copy of the records so post-construction source mutation can't silently drift the stats. Every wiring seam honest-degrades: a shape-gate exception logs a warning and falls through to normal design (a gate failure must never block a legitimate request); `classify_forge_rejection`'s `other` fallback keeps an unforeseen status from crashing the histogram. The circular-import risk (forge_observability references `DesignedAgentRecord`, which lives in self_mod) is resolved by importing it only under `TYPE_CHECKING` and duck-typing at runtime, so self_mod can import forge_observability at module top-level with no cycle. The `EventType.FORGE_SHAPE_REJECTED` emission was skipped because self_mod's `EventLog` uses string event names (`log(category, event, detail)`), not the `EventType` enum — it did not slot cleanly, and the prompt marked it optional ("only if it slots cleanly"). No new LLM calls, no persistence (in-memory aggregation over `_records` only), no memory-layer change. Tests: `tests/test_ad872_forge_observability.py` (28 — shape-gate accept/reject matrix incl. empty-`{}`-is-valid, every status→bucket mapping + validator-error substrings + `other` fallback, attempt-vs-unique-rate separation on a retried+distinct fixture, histogram correctness, local-copy materialization, `summary()` shape, `shape_rejected` short-circuits `handle_unhandled_intent` with the designer's `design_agent` asserted not-called, honest-degrade when the gate raises via monkeypatched `validate_forge_shape`, and `forge_stats()` returning a populated aggregator after a real design), real `DesignedAgentRecord` + real `SelfModificationPipeline` fixtures per BF-287 (no MagicMock at the substrate boundary). `tests/test_self_mod.py` + `tests/test_self_mod_deps.py` green; full gate 15411 passed, 22 skipped. Closes [#844](https://github.com/seangalliher/ProbOS/issues/844). Second unit of the Memory Epistemics wave; AD-873 (Ebbinghaus decay + composite rerank) deferred to a later commit.

### AD-871: Provenance-aware memory envelope (source_type/confidence/verification_count/contradicted_by)

**Decision:** Added four last-position fields to the frozen `Episode` dataclass (`src/probos/types.py`): `source_type: str = ""`, `confidence: float = 1.0`, `verification_count: int = 0`, `contradicted_by: list[str] = field(default_factory=list)`. Added a provenance kernel to `types.py`: `SOURCE_TYPE_CONFIDENCE` (per-source default belief — `user_statement`/`tool_result`=1.0, `observation`/`external`=0.8, `agent_inference`/`reflection`=0.5), `SOURCE_TO_SOURCE_TYPE` (maps the legacy `MemorySource` string to a `source_type`: `direct`→`observation`, `secondhand`→`agent_inference`, `ship_records`/`briefing`→`external`, `reflection`→`reflection`), `resolve_provenance(source, source_type, confidence) -> tuple[str, float]`, and `mark_contradicted(episode, contradicting_id) -> Episode`. Wired three seams in `src/probos/cognitive/episodic.py`: (1) `store()`'s AD-598 importance rescore reconstructs the frozen Episode via `dataclasses.replace(episode, importance=...)` so the new fields survive; (2) `_episode_to_metadata` stamps the envelope onto the ChromaDB row (`contradicted_by` JSON-encoded to `contradicted_by_json` per the metadata str/int/float/bool-only rule, source/type resolved through `resolve_provenance` at the write boundary); (3) `_metadata_to_episode` decodes it back with Tier-2 honest-degrade. `compute_episode_hash` and `_hash_v` untouched.

**Rationale:** Memory previously recorded *what* happened but not *how it was learned* or *how trustworthy it is*. The provenance envelope lets a memory carry graded belief (`confidence`), its acquisition source (`source_type`), corroboration (`verification_count`), and contradiction links (`contradicted_by`) — the substrate for future belief-revision and contradiction handling (AD-872/873). `resolve_provenance` derives confidence from the source map **only** when the field is set AND confidence is still the 1.0 default, so an explicit caller-supplied graded belief is never overwritten; an empty `source_type` is back-filled from the legacy `source` string so pre-existing call sites get a sensible type for free. The `store()` reconstruction fix was load-bearing: a failure outcome at default importance=5 rescores to 7, and the prior field-by-field copy silently dropped the new fields — `dataclasses.replace` preserves every carried field by construction. The read seam honest-degrades every malformed value (bad `confidence`→1.0, bad `verification_count`→0, bad/non-list `contradicted_by_json`→`[]`) so pre-AD-871 rows recall cleanly with neutral defaults — zero migration. Tests: `tests/test_ad871_provenance_envelope.py` (14 — defaults, store/recall round-trip of all four fields, legacy-source back-fill, `resolve_provenance` derive-vs-preserve matrix, map self-consistency, graded round-trip, `mark_contradicted` idempotency, read-seam honest-degrade on malformed/non-list metadata, pre-AD-871 default recall, and the AD-598 reconstruction regression), real `EpisodicMemory`/`tmp_path` fixtures per BF-287. `tests/test_episodic.py` (16) green; full gate 15382 passed. Closes [#843](https://github.com/seangalliher/ProbOS/issues/843). First unit of the Memory Epistemics wave; AD-872 (forge observability) and AD-873 (decay + rerank) deferred to later commits.

### AD-847: Desktop notification on Yeo task completion (renderer → preload → main → native notify)

**Decision:** Closed the wave's final surface: a native OS notification when a Yeo-delegated task finishes. Added a `notifyTaskDone({title, body, route})` method to the `window.probos` context-bridge API (`desktop/src/preload/index.ts`) that `ipcRenderer.invoke("probos:notifyTaskDone", payload)`s the main process. The main process (`desktop/src/main/index.ts`) registers `ipcMain.handle("probos:notifyTaskDone", ...)` which coerces the untrusted renderer payload via a new pure helper `coerceTaskDonePayload(payload): NotifyArgs | null` (`desktop/src/main/notifications.ts`) and, on success, calls the existing AD-759 `notify(args, { showAndRoute })` primitive. When `route` is present (e.g. Yeo's 1:1 chat deep-link), clicking the notification activates the window and routes there via the existing `showAndRoute`. Reuses the existing `probos:*` channel naming and the AD-759 notification wrapper — no new WebSocket, no new window plumbing.

**Rationale:** AD-846 posts the completion DM into Yeo's chat channel, but a desktop user with the window minimized would still miss it. AD-847 adds the OS-level surface so a finished task can pull the Captain back to the exact place to read the result. The renderer payload is untrusted across the context-isolation boundary, so `coerceTaskDonePayload` validates `title` (non-empty string) / `body` (string) and drops a blank `route` to `undefined` (so a click never routes to an empty path) — making validation the testable unit independent of Electron. The notification dispatch degrades silently when the OS reports no notification support (AD-759 behavior, unchanged). Tests: `desktop/src/main/notifications.test.ts` (9 — 5 coercion: route/blank-route/empty-title/non-string-body/non-object; 4 dispatch: shows/click-routes/no-route-no-handler/unsupported-suppressed; electron mocked via `vi.mock`). Full desktop gate 65 green; `electron-vite build` clean. Closes [#842](https://github.com/seangalliher/ProbOS/issues/842). Completes the AD-869→870→846→847 lightweight-delegation wave.
...
**Date:** 2026-06-04
**Decision:** Extended `YeomanAgent._conversational_task_protocol` (the AD-845 hook) to teach the full four-tier delegation threshold in plain language instead of only the Tier-3 `[CREATE_TASK]` guidance: Tier 1 Answer (reply directly, no tag) / Tier 2 Do-and-report (a quick read-only lookup Yeo can finish *this turn* → emit the AD-869 `[MESH <intent> key=value ...]` tag) / Tier 3 Write-it-down (substantial work or anything that changes state → `[CREATE_TASK ...]`) / Tier 4 Get-help (`specialist=@Callsign`). A new helper `_available_mesh_read_intents()` reads the live registry and returns `{intent: param-hint}` only for read pools that currently have an agent (`directory`→`list_directory path=<dir>`, `filesystem`→`read_file path=<file>`, `search`→`search_files path=<dir> pattern=<glob>`, `web_search`→`web_search query=<terms>`, `page_reader`→`read_page url=<url>`) — so the `[MESH]` guidance lists only forms the ship can actually serve. The closing "rule of thumb" encodes the two-axis threshold: "if you can get the answer in the time it takes to reply and it changes nothing, just do it; otherwise write it down as a task."
**Rationale:** AD-869 built the Tier-2 *seam* but Yeo had no instruction telling it *when* to prefer an inline `[MESH]` read over opening a `[CREATE_TASK]` — without the threshold rule Yeo would either over-escalate (open a task for a one-line directory listing) or under-deliver. The threshold is grounded in graduated-autonomy doctrine (command-by-negation / Mission Command: act within intent, escalate by exception) and a decision-cost model (Vroom–Yetton: match decision mode to the stakes): the two axes Yeo weighs are *latency* (can I finish this turn?) and *reversibility/cost* (does it change anything?). Teaching this in the conversational prompt (not `decide()`/`act()`) keeps Yeo instructions-first (Design Principle #6). Honest-degrade is preserved and tightened: `[MESH]` guidance is gated per-pool on a live agent and `[CREATE_TASK]` guidance on a wired work-item store, so Yeo is never told to use a seam the substrate cannot back; a registry that raises during the read-intent build degrades to the `[CREATE_TASK]`-only block and never crashes the reply. All text remains free of the `_CAPABILITY_GAP_RE` tokens (asserted).
**Status:** Implemented. +8 pytest (`tests/test_ad870_yeo_delegation_threshold.py`, real `AgentRegistry` fixtures per the no-MagicMock-at-substrate-boundary rule — all-tiers-when-fully-wired, MESH-lists-only-live-pools, no-seams→empty, MESH-only-when-store-absent, CREATE_TASK-only-when-no-read-pools, gap-regex-safe full block, registry-raises→degrade-to-CREATE_TASK, base-hook no-op); `tests/test_ad845_yeo_chat_task_creation.py` + `tests/test_yeoman_agent.py` (34) green. Closes [#840](https://github.com/seangalliher/ProbOS/issues/840). Verify all changes comply with the Engineering Principles in `.github/copilot-instructions.md`.

### AD-869: Yeo's Tier-2 "do-and-report" — synchronous inline mesh reads from a 1:1 chat reply

**Date:** 2026-06-04
**Decision:** Added Yeo's lightweight delegation *default* — the missing Tier-2 of the four-tier delegation model (Tier 1 Answer / **Tier 2 Do-and-report** / Tier 3 Write-it-down = AD-845 `[CREATE_TASK]` / Tier 4 Get-help = AD-845 `specialist=@`). When the Captain asks for a quick read-only lookup Yeo can answer *this turn*, Yeo emits a reply tag `[MESH <intent> key=value key=value]`. Two new `DmSanityGate` methods parse it (`extract_mesh_read(text) -> tuple[str, dict[str,str]] | None` / `strip_mesh_read(text) -> str`, with `_MESH_READ_RE` / `_MESH_READ_STRIP_RE` / `_MESH_PARAM_KEY_RE`; the key regex anchors on a token boundary so a `=` inside a value such as a URL query string is preserved, and multi-word values like `query=Nvidia SPARK RTX` parse intact). A new `DmReplyPipeline.step_4h_mesh_read_parse` (registered before `step_4f_extract_artifacts`, so a large read result is still eligible for artifact extraction + episodic storage) maps the intent to its registry pool via the module-level allowlist `_MESH_READ_INTENT_POOLS` (`list_directory`→`directory`, `read_file`/`stat_file`→`filesystem`, `search_files`→`search`, `web_search`→`web_search`, `read_page`→`page_reader`), resolves the first live agent in that pool via `runtime.registry.get_by_pool(pool)` (reusing the BF-599 resolution pattern), runs ONE targeted `IntentBus.send(IntentMessage(intent, params, target_agent_id, ttl_seconds=5))` (single dispatch — NOT a broadcast, which would fan out to every pool subscriber), renders the result inline (truncated to 1500 chars), and strips the tag. A non-allowlisted intent (e.g. `http_fetch`) is stripped and shipped, never executed.
**Rationale:** AD-845 built the *escalation* path (async tracked `[CREATE_TASK]` work) but live testing showed the right *default* is a lightweight synchronous read the Captain gets back this turn — "if I can get the answer in the time it takes to reply, and it doesn't change anything, I just do it; otherwise I write it down." The allowlist is the Tier-2 safety boundary and it is structural, not merely a policy list: a single-turn ~5s synchronous read is *incapable* of mutation because destructive ops require consensus and consensus cannot resolve in one turn, so the latency ceiling enforces the safety ceiling. `http_fetch` is consensus-gated and therefore excluded; writes are excluded; anything heavier belongs on the Tier-3 async path. The seam follows the established in-reply-tag precedent (`[MOVE]`, `[SELF_CHECK]`, `[GEN_IMAGE]`, `[ACTION]`, `[CREATE_TASK]`) rather than a decomposer intent (never fires on the conversational path). Web/file access stays mesh-delegated (Design Principle #10): no `httpx`/`requests` import is added (asserted by source scan). Every failure mode is Tier-2 honest-degrade (missing gate/bus/agent, send failure, timeout, unsuccessful result → strip tag, append a brief note whose wording avoids the `_CAPABILITY_GAP_RE` tokens, never raise). The Yeo-facing threshold instructions that teach *when* to emit `[MESH]` vs `[CREATE_TASK]` vs `specialist=@` ship next in AD-870; completion → Captain DM (AD-846) and desktop notification (AD-847) are gated follow-ups, NOT built here.
**Status:** Implemented. +16 pytest (`tests/test_ad869_mesh_read_inline.py`, real `IntentBus` + `SignalManager` + `AgentRegistry` + subscribed handler + real `DmSanityGate` per the no-MagicMock-at-substrate-boundary rule — extract/strip unit tests incl. multi-word value + embedded-`=` URL + parameterless-malformed, allowlisted read executes/renders/strips with single targeted dispatch, non-allowlisted `http_fetch` not executed, no-tag no-op, intent-bus-None degrade, no-capable-agent honest note, send-failure honest note, unsuccessful-result empty note, large-payload truncation, allowlist excludes writes/consensus-gated intents, no-direct-http-import source scan; all degrade notes asserted gap-regex-clean); DM pipeline + sanity-gate regression suites (53) green. Closes [#839](https://github.com/seangalliher/ProbOS/issues/839). Design grounded in graduated-autonomy doctrine (command-by-negation / Mission Command; workflows-vs-agents; Vroom–Yetton decision model). Verify all changes comply with the Engineering Principles in `.github/copilot-instructions.md`.

### AD-845: Yeo creates a dispatchable task from a 1:1 chat reply (D2 auto-delegation, Phase 1)

**Date:** 2026-06-04
**Decision:** Added a second overridable base hook `CognitiveAgent._conversational_task_protocol(self, observation: dict) -> str` (default `""`), appended next to the BF-599 `_conversational_capability_block` inside `_decide_via_llm`'s `is_conversation` branch. `YeomanAgent` overrides it to teach a reply tag `[CREATE_TASK title=... | instructions=... | specialist=@callsign]` (honest-degrade: returns `""` when `runtime.work_item_store` is absent, so Yeo is never told it can open tasks it cannot back; tag text is `_CAPABILITY_GAP_RE`-clean). A new `DmReplyPipeline.step_4g_create_task_parse` (sibling of the AD-572 `[MOVE]` / AD-728d `[SELF_CHECK]` steps, registered after `step_4f_extract_artifacts`, before `step_5_episodic_store`) parses the tag via two new `DmSanityGate` methods (`extract_create_task` / `strip_create_task` + `_CREATE_TASK_RE` / `_CREATE_TASK_STRIP_RE`), resolves the specialist callsign → live agent UUID through `callsign_registry.resolve` (fallback: Yeo's `resolve_delegate` department keyword map over the instructions text), creates a `dispatchable=True` work item tagged `yeo-delegated` via `WorkItemStore.create_work_item(work_type="task", created_by="captain", ...)`, strips the tag, and appends the new task id to the Captain-visible reply. The dispatchable item then runs automatically through the AD-834/AD-839 `WorkItemRouter` — no new execution code.
**Rationale:** BF-599/BF-601 closed D1 (grounding — telling Yeo *what* it can delegate) but live testing showed the deeper D2 gap: Yeo's conversational path is pure text generation and has no seam to actually *fire* a mesh intent, so even when correctly grounded it emitted fake `[MESH http_fetch ...]` text and then honestly refused to pretend it had executed (the confabulation guard working as designed). AD-845 gives the conversational reply a real action seam following the established in-reply-tag precedent (`[MOVE]`, `[CHALLENGE]`, `[SELF_CHECK]`, `[GEN_IMAGE]`, `[ACTION]`) rather than adding a decomposer intent (which never fires on the conversational path) or overriding Yeo's `decide()`/`act()`. The hook stays Open/Closed (base `""`, only Yeo opts in); the work item is the existing dispatch primitive (no new router/execution code); every failure mode is Tier-2 honest-degrade (missing gate/store/create-failure → strip tag, ship reply, never raise). Supersedes the deferred "BF-599b / AD-840" marker (AD-840–844 are reserved for the Desktop Console epic). AD-846 (completion → proactive Captain DM) and AD-847 (desktop notification) are gated follow-ups, NOT built here.
**Status:** Implemented. +9 pytest (`tests/test_ad845_yeo_chat_task_creation.py`, real `WorkItemStore` in-memory + real `AgentRegistry`/`CallsignRegistry` + real `DmSanityGate` per the no-MagicMock-at-substrate-boundary rule — dispatchable-item creation, tag strip + task-id surface, no-tag no-op, store-None honest-degrade, keyword-map specialist fallback, unresolved→unassigned-but-dispatchable, gap-regex-safe protocol text, store-absent hook degrade, base-hook no-op); `tests/test_bf599_yeo_web_research_delegation.py` + `tests/test_yeoman_agent.py` (43 total) and `tests/test_ad726_dm_reply_pipeline.py` + `tests/test_ad724_dm_sanity_gate.py` + `tests/test_ad724_dm_hardening.py` (38) green. Verify all changes comply with the Engineering Principles in `.github/copilot-instructions.md`.
...
**Date:** 2026-06-03
**Decision:** Added an overridable base hook `CognitiveAgent._conversational_capability_block(self, observation: dict) -> str` (default returns `""`) that is appended to the composed conversational system prompt for every reply path (1:1, ward room, proactive) inside `_decide_via_llm`'s `is_conversation` branch — `composed += self._conversational_capability_block(observation)` when non-empty. `YeomanAgent` overrides it to render a POSITIVE capability block from the **live** `runtime.registry`: for each of pools `web_search` / `page_reader` / `http` that has ≥1 registered agent, it lists the delegable mesh intent (`web_search` / `read_page` / `http_fetch`) and instructs Yeo to delegate web research / page-reading through the mesh (Science → @Number One) instead of declining. The block is honest-degrade — returns `""` when there is no runtime, no `registry` attribute, a registry read that raises (Tier-2 log-and-degrade), or none of the three pools are present — and is authored to contain none of the decomposer `_CAPABILITY_GAP_RE` tokens so it can never itself be mistaken for a capability-gap declaration. Scope is D1 (grounding) only; auto-delegation (D2) shipped in AD-845 (Yeo creates a dispatchable task from chat — supersedes the earlier "BF-599b / AD-840" marker, since AD-840 is now reserved for the Desktop Console HXI Skill Registry).
**Rationale:** Yeo (AD-766, the always-on conversational front-door) was telling the Captain "I can't browse the web" in DMs even though the ship runs live `web_search` / `page_reader` / `http` pools. The root cause is structural: the DM prompt is composed with `hardcoded_instructions=""`, so Yeo's static role rules describing its delegation duties never reach the conversational turn, and the LLM confabulates a plausible-but-false limitation into the gap. A static instruction string would drift from reality (pools come and go at runtime); rendering the block from the live registry on each turn keeps the grounding truthful and self-updating. Making it a base hook (default `""`) rather than Yeo-specific code preserves the Open/Closed principle — the conversational composition point gains one extensible seam, every non-overriding agent is byte-unaffected, and future agents can opt in. Web access stays mesh-delegated (Design Principle #10): the block tells Yeo to *delegate*, and no direct `httpx`/`requests` import is added (asserted by a source-scan test).
**Status:** Implemented (Wave 209). +6 pytest (`tests/test_bf599_yeo_web_research_delegation.py`, real `AgentRegistry` fixture per the no-MagicMock-at-substrate-boundary rule — live-pool listing, base-hook no-op, no-runtime degrade, no-pools degrade, gap-regex-clean, no-direct-http-import); `tests/test_yeoman_agent.py` (25) + `test_cognitive_agent.py` / `test_cognitive_agent_dm_game.py` / `test_bf010_conversational_prompt.py` (70) green. Verify all changes comply with the Engineering Principles in `.github/copilot-instructions.md`.

### BF-600: HNSW structural pre-check must not flag a fresh `cur_element_count=0` index as a torn write

**Date:** 2026-06-02
**Decision:** `validate_hnsw_files` (the AD-822b/BF-298 boot-time structural pre-check) now exempts `header.cur_element_count == 0` from the torn-write signature `length.bin entries != cur_element_count`. ChromaDB ≥ 1.5.x writes `length.bin` at the index allocation capacity (`== max_elements`, default 100) and leaves `header.bin` at its init value `cur_element_count=0` until a real sync/compaction flushes it — so a freshly-built or small store legitimately shows `cur=0` with `length.bin=100` while every added row is safe in the WAL (chroma rebuilds the in-memory index on next open). The mismatch comparison is now gated on `cur_element_count > 0`; a genuine partial flush always leaves a NON-ZERO `cur_element_count` disagreeing with `length.bin` (the BF-298 reference corruption: header=70779 vs length=70966), which is still flagged. All other structural checks (max-elements/size-per-element sanity, `data_level0.bin` internal-size consistency, file existence) are unchanged. The `_episodic_probe` subprocess still proceeds from a passing file pre-check to a real chroma open + `peek(1)` + `count()`, so a `cur=0` store now gets the deep integrity check instead of a blind exit-5 reject — strictly safer than the false-positive it replaces.
**Rationale:** The BF-298 invariant was validated only against large compacted stores. On chromadb 1.5.8 the structural pre-check hard-blocked (exit code 5, "rebuild-episodic" operator message) on every fresh/small data dir — reproduced live against `data/node-1` and the AD-822/AD-823 test fixtures, all sharing the identical `cur=0, length=100` signature. Because exit 5 short-circuits *before* chroma opens, the false positive removed the very safety net that would have confirmed health. Gating on `cur > 0` removes the false positive without weakening detection of real mid-flush corruption, and matches the validator's existing "absence is not corruption / first-boot returns ok" philosophy. Also resolved in the same suite-cleanup sweep: two pre-existing stale-test-double failures (8 tests) where AD-791a's explicit-`thread_id` validation tripped on bare `MagicMock` request/runtime objects whose `thread_id`/`chat_thread_store` attributes auto-materialize truthy — fixed test-side by setting `runtime.chat_thread_store = None` in the `test_ad732_vision_tier` and `test_agents_router_game_move` runtime helpers (production behavior unchanged; the Phantom-via-MagicMock trap).
**Status:** Implemented (Wave 208). +2 pytest (`tests/test_bf298_hnsw_validation_uses_actual_count.py`, now 6); AD-822/AD-823/ad732/game_move suites (38) green; AD-822b + BF-298 validator regressions (11) green. Verify all changes comply with the Engineering Principles in `.github/copilot-instructions.md`.

### BF-598: Idempotent shutdown + non-regressive AD-820 marker (recurring boot refusal)

**Date:** 2026-06-02
**Decision:** The shutdown path is now idempotent and the AD-820 clean-shutdown marker write is non-regressive. `ProbOSRuntime` carries a `_shutdown_started` guard flag so a second `shutdown()` (signal + atexit + explicit call racing) cannot re-run the teardown sequence, and `startup/shutdown.py` was hardened so the AD-820 marker is written without clobbering a newer marker — preventing the recurring boot-refusal where an interrupted/duplicated shutdown left the runtime unable to start. Changes are confined to `runtime.py` (+5) and `startup/shutdown.py` (+46).
**Rationale:** The operator hit repeated boot refusals traced to a torn/duplicated shutdown sequence interacting with the AD-820 marker. Making shutdown run-once and the marker write non-regressive closes the loop reproduced across multiple restarts; the BF-598 boot fix was confirmed working live by the operator.
**Status:** Implemented (Wave 208, commit b3e20866). +N pytest (`tests/test_bf598_shutdown_idempotency.py`, 232 lines); `tests/test_bf296_shutdown_phase_ordering.py` updated (+3). Full serial gate confirmed BF-598's own shutdown tests green. Verify all changes comply with the Engineering Principles in `.github/copilot-instructions.md`.

### AD-839: Dispatched work items reach the assigned agent's awareness

**Date:** 2026-06-01
**Decision:** When the AD-581a `WorkItemRouter` directly dispatches a dispatchable work item to a CognitiveAgent (the AD-834 "dispatch to agent now" path), the resulting `work_item_dispatched` IntentMessage must be consumed by the assignee instead of being silently dropped. A new branch in `CognitiveAgent.handle_intent` — placed BEFORE the self-deselect fast path because `work_item_dispatched` is not in any agent's `_handled_intents` — routes `intent.intent == "work_item_dispatched" and intent.target_agent_id == self.id` to a new `_handle_work_item_dispatch(intent)` helper. The helper (1) composes a Captain-style task message from the intent's `work_item_id`/`title`/`description` params, (2) logs it into the agent's DM thread via `runtime.chat_thread_store.get_or_create_default_for_agent` + `append_message` (author `captain`/role `captain`), (3) runs the assignment through the normal direct-message lifecycle (`self.handle_intent` with a synthesized `direct_message` IntentMessage) so the agent reasons about and acknowledges the task, (4) appends the agent's acknowledgment to the thread (author `self.id`/role `agent`, mirroring the AD-791a router append), and (5) transitions the work item to `in_progress` via `runtime.work_item_store.transition_work_item(work_item_id, "in_progress", source=self.id)`. Scope is direct assignment only (gated on `target_agent_id == self.id`); broadcast dispatch is untouched. Every side effect is tier-2 log-and-degrade — a thread-store outage, a transition validation failure, or a missing store can never raise into the cognitive queue; the work item simply stays in its prior status and/or the agent produces no acknowledgment.
**Rationale:** AD-834 shipped NL task creation with a "dispatch to agent now" toggle, but the dispatched intent reached `handle_intent`, matched neither `is_direct` nor `_handled_intents`, hit the self-deselect fast path, and returned None — so the assigned agent (Ezri in the operator's report) had no awareness of the task and the Kanban card never advanced from BACKLOG. Reusing the existing direct-message lifecycle (rather than inventing a parallel task-reasoning path) keeps the cognitive contract intact: the agent perceives → decides → acts on the assignment exactly as it would a Captain DM, and the thread surfaces both the assignment and the acknowledgment so the operator can see the work started.
**Status:** Implemented (Wave 202). +5 pytest (`tests/test_ad839_work_item_dispatch.py`). Verify all changes comply with the Engineering Principles in `.github/copilot-instructions.md`.

### BF-332: WorkBoard assignee fallback to the agents map + work-item detail modal

**Date:** 2026-06-01
**Decision:** Two frontend regressions surfaced by the AD-834 dispatch feature. (1) **Assignee "Unassigned":** the Kanban card resolved an assignee only against the bookable-resource registry (`bookableResources`); when that registry is empty (the BF-331 backend bug, or before crew commissioning completes) every dispatched card rendered "Unassigned" even though `assigned_to` held a valid agent UUID. A new `resolveAssignee(assignedTo)` useCallback in `WorkBoard.tsx` resolves the resource first, then falls back to `agents.get(assignedTo)` (the independently-populated agent map keyed by UUID), returning the agent's `callsign || agentType`. The dept filter and swim-lane grouping route through the same resolver. (2) **Cards could not be opened:** `WorkCard` gained `onClick`/`role="button"`/`tabIndex`/`onKeyDown` (Enter/Space) wiring to a new `detailItem` state and a click-backdrop detail modal showing title, priority dot, work_type/status/priority/dispatched badges, Assigned-to/Created-by/Due rows, description, step progress, and tags. The modal reads exclusively from the already-loaded `WorkItemView` (no fetch) and closes via the stroke-SVG `Close` glyph (HXI #3, no emoji) or backdrop click.
**Rationale:** Resolving the assignee through two independent registries (bookable resources, then the agent map) makes the board robust to either source being empty — defense in depth at the display boundary. The detail modal closes the "can't open the task" gap the operator reported; reusing the in-memory `WorkItemView` avoids an extra round-trip since the list payload already carries every field the modal renders.
**Status:** Implemented (Wave 202). +5 vitest (`ui/src/__tests__/WorkBoard.bf332.test.tsx`); `cd ui && npx vitest run` and `cd ui && npm run build` both green per the BF-279 dual gate; zero new deps. Verify all changes comply with the Engineering Principles in `.github/copilot-instructions.md`.

### BF-331: Workforce resource registration ordering (empty bookable-resource registry)

**Date:** 2026-06-01
**Decision:** `ProbOSRuntime._register_workforce_resources` ran during the communication startup phase (`startup/communication.py`) *before* `self.work_item_store` was assigned from `init_communication`'s return value, so its `if not self.work_item_store: return` guard fired and **zero** BookableResources were ever registered — the root cause of every dispatched task rendering "Unassigned" on the Kanban board (`GET /api/resources` returned count 0). The method now takes an optional `work_item_store: Any | None = None` parameter and resolves `store = work_item_store or self.work_item_store`; the communication phase passes the freshly-created store explicitly (`register_workforce_resources_fn(work_item_store)`), while existing zero-argument call sites keep working via the instance-attribute fallback. The `BookableResource`/`AgentCalendar` construction is unchanged; only the store source and call site changed.
**Rationale:** The registration logic was correct but ran one assignment too early in the startup ordering. Threading the store as a parameter (rather than reordering the whole startup phase, which would risk other init dependencies) is the minimal, local fix and preserves the zero-argument contract for any other caller. This is the OS-substrate analog of "register against the dependency you were handed, not the instance attribute that isn't wired yet."
**Status:** Implemented (Wave 202). +3 pytest (`tests/test_bf331_workforce_resource_registration.py`, using real ProbOSRuntime instances + fake stores per the no-MagicMock-at-substrate-boundary rule); existing `tests/test_workforce.py` (114) green. Verify all changes comply with the Engineering Principles in `.github/copilot-instructions.md`.
...
**Date:** 2026-05-31
**Decision:** Give the Ward Room panel a three-state display mode — `docked` (the current 420px left sidebar, unchanged default), `floating` (a draggable, corner-resizable in-app window), and `maximized` (the floating window scaled to near-full viewport, restoring to the prior floating rect) — toggled from one-click header controls (`docked → floating → maximize/restore → docked`). Frontend-only: no backend, no API, no server-data-model change. The implementation reuses the AD-721 AgentProfilePanel drag/resize/persist convention (DRY) rather than reinventing it — header `onMouseDown` captures a `dragOffset`; a `mousemove`/`mouseup` listener pair is added on drag-start and torn down on release (no always-on global listeners); the bottom-right corner handle resizes with the same clamp shape (min 360×320, max viewport−40). New store slice in `ui/src/store/useStore.ts`: exported `WardRoomDisplayMode`/`WardRoomWindowRect` types, `loadWardRoomLayout()` (validates persisted mode + rect, falls back to `{x:80,y:80,w:720,h:640}` on malformed JSON / unavailable storage), and `setWardRoomDisplayMode`/`setWardRoomWindowRect` actions writing through to `localStorage` (`probos.wardroom.mode` / `probos.wardroom.rect`) with Tier-2 log-and-degrade. Four new stroke-SVG glyphs (`Undock`/`Dock`/`Maximize`/`Restore`) per HXI #3 (no emoji); control glyphs `stopPropagation` on `onMouseDown` to stay out of the drag handle; Maximize/Restore hidden in docked mode.
**Rationale:** The Ward Room is the densest reading/triage surface in the HXI (channels, threads, DM log) and 420px is cramped for multi-column thread reading. HXI Principle #5 (progressive disclosure driven by engagement) and #9 (alert-driven layout) both argue the operator should be able to promote it from an ambient docked strip to a focused full-screen workspace, then dock it again. The docked branch is kept byte-equivalent to the pre-AD-837 sidebar (slide-in transform + `pointerEvents` gating) so the default is regression-safe; floating/maximized are opacity/visibility-gated windows above the docked z-index. A true OS-level detached window (`window.open` / Tauri portal) is deliberately deferred to the AD-837b seam because it requires React-portal-into-popup plumbing.
**Status:** Implemented (Wave 201). +8 vitest (`ui/src/components/wardroom/__tests__/WardRoomPanel.windowmode.test.tsx`); vitest 1023 → 1031; `npm run build` green per BF-279 gate; zero new deps. Forward markers: AD-837b (true OS-level detached window), AD-837c (left/right/top edge resize handles), AD-837d (snap-to-edge / tiling). Closes #811.
...
## AD-812 (2026-05-23) — Natural-language scheduled actions

Closes #736. New `src/probos/cognitive/schedule_parser.py` exposes `ScheduleSpec` (frozen dataclass mapping 1:1 onto `PersistentTaskStore.create_task(...)` kwargs) and `parse_nl_schedule(text, llm_client, now=None)`. Order of operations: (1) regex fast-path matches `in N (second|minute|hour|day)s?` and returns `kind=once` with no LLM call; (2) fast-tier LLM with a strict JSON-output system prompt that enumerates `kind=once|interval|cron`, `intent_text`, `execute_at_iso`, `interval_seconds`, `cron_expr`, `channel`, `max_runs`; (3) honest-degrade `ScheduleSpec(kind='error', reason=...)` when LLM is unavailable and regex misses, when LLM output is non-JSON, when `kind` is invalid, when `intent_text` is empty, when `execute_at_iso` is missing/malformed/in-the-past, when `interval_seconds` is non-positive, or when `cron_expr` fails `croniter.is_valid(...)`. Channel binding via `_channel_to_id(name)` maps lowercase `{telegram, slack, discord, matrix, teams, gmail}` to `<adapter>:default` and returns None for anything else (no invented channels). New `src/probos/experience/commands/commands_schedule.py` ships `cmd_remind` and `cmd_schedule` (list / cancel <id> / NL). New `src/probos/routers/schedule_nl.py` ships `POST /api/schedule/nl`. All three surfaces share a single `PersistentTaskStore.create_task` call site so the existing AD-418 substrate handles persistence, restart-safe replay, and SCHEDULED_TASK_* events. NOT changed: `persistent_tasks.py`, `workflow_cron.py`, `routers/scheduled_tasks.py`, the runtime startup sequence. Channel-id resolution beyond name-to-default is deferred. Zero new dependencies. +9 pytest in `tests/test_ad812_schedule_parser.py`.

## BF-301 (2026-05-23) — Migrate browser STT from whisper.cpp WASM to transformers.js

Closes #775. The AD-826 architecture (local-first STT, browser-side inference, browser SR as fallback) ships intact; only the local engine swaps. whisper.cpp's WASM example bundle has been abandoned upstream (https://huggingface.co/ggerganov/whisper.cpp/resolve/v1.5.4/... returns "Invalid rev id", the whisper.ggerganov.com GitHub Pages CDN 404s, 
pm i whisper.cpp ships only JS glue with no stable WASM mirror). The industry has converged on `@huggingface/transformers` v3 (Apache 2.0, ~3M weekly npm downloads) which ships the `Xenova/whisper-tiny.en` ONNX model directly from the HF CDN with browser Cache API persistence — zero operator setup, identical behavior across Chrome / Edge / Firefox / Safari / Chromium-Electron. New `ui/src/audio/transformersStt.ts` mirrors the AD-705a `whisperStt.ts` public surface (`armTransformersStt` / `disarm` / `onTransformersTranscript` / `onTransformersTranscribing`) and adds a new `onTransformersProgress(handler)` channel so the UI can render a first-load model download bar (`data-testid="bf301-progress"`, amber `#f0b060` per HXI #3). New `ui/src/audio/transformersWorker.ts` owns the `pipeline('automatic-speech-recognition', 'Xenova/whisper-tiny.en')` lifecycle in a dedicated Web Worker (`new Worker(new URL('./transformersWorker.ts', import.meta.url), { type: 'module' })`; postMessage init/transcribe/shutdown protocol with transferred Float32 ArrayBuffer to avoid a copy on every utterance). All three production call sites migrated via import-name swap so the BF-290/292/293/294/294b/300 mic UX gating logic remains untouched: `ProfileChatTab.tsx`, `IntentSurface.tsx`, `conversationController.ts`. Backend `cognitive.primary_stt` Literal extended to `"transformers" | "whisper" | "browser"` (default flipped from `whisper` to `transformers`); `whisper` retained as a deprecated alias that resolves to `engine: "transformers"` in `/api/voice/health` so saved operator configs do not break. New `cognitive.transformers_model` field (default `Xenova/whisper-tiny.en`; operators can pin `Xenova/whisper-base.en`). `/api/voice/health` adds a `model` field and no longer probes the GGML filesystem path (browser owns the fetch via HF CDN + Cache API); `backend_available` semantics become `resolved_engine == 'transformers' AND offline_stt_enabled`, `healthy` becomes `backend_available OR resolved_engine == 'browser'`. `ProfileChatTab.tsx` PTT branching updated to accept both `primary_stt='transformers'` (new default) AND `primary_stt='whisper'` (deprecated alias) as the local-first engine. `whisper_model_path`, `whisperStt.ts`, `whisperLoader.ts`, `scripts/whisper-tiny-en-fetch.ps1`: retained with deprecation banners for one release cycle; follow-up hygiene PR will delete them. `onnxruntime-web` removed from `optionalDependencies` (v3 bundles it internally). New Vite `stt-vendor` `manualChunks` branch keeps the ~1MB transformers.js + onnxruntime runtime out of the main bundle (lazy on PTT arm). Privacy invariant preserved: audio bytes never leave the browser; the only HF-CDN traffic is ONNX weight shards in / transcript text out. Honest-degrade: worker emits `{status: 'error'}` progress event on model-fetch failure; existing AD-826 empty-counter fallback to browser SR continues to work. Bundle delta: main `index` chunk unchanged at 48.8 kB (gzip 15.4 kB); new `stt-vendor` chunk at 13.43 kB (gzip 5.34 kB) for the main-thread shim + lazy `transformersWorker` chunk at 877 kB + `ort-wasm-simd-threaded` at 21.6 MB (gzip 5.1 MB) loaded only when STT arms. +9 backend pytest (`tests/test_bf301_voice_health.py`) + 3 AD-826 backend tests migrated (default flipped, `whisper`-alias path, `model` field). +9 worker-boundary vitest (`ui/src/__tests__/transformersStt.bf301.test.tsx`) + 9 integration vitest (`ui/src/__tests__/ProfileChatTab.bf301.test.tsx`) + 5 AD-826 frontend tests migrated + 7 BF-290/292/293/294/300 + conversationController + conversationWiring tests re-pointed at the new module-mock target. Vitest 924 → 957 (+33). Backend AD-820..AD-826 + BF-291/295/296/297/298/300 regression suite green (112/116 pass; 4 pre-existing AD-822/AD-823 chromadb HNSW fixture failures on HEAD are unrelated to BF-301). UI `npm run build` green per BF-279 gate.
