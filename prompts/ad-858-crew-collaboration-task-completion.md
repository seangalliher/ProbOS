...
- A `class SubtaskVerifier` in `src/probos/cognitive/crew_verifier.py` that, for each `SubtaskResult`, runs an **independent** agent (a *different* crew member than the producer — independence is the point) to **refute** the result: an LLM-judge prompt that tries to find a flaw, missing requirement, or unsupported claim. When the subtask carries a declared `expected_output` contract (AD-858), the judge prompt is anchored to it — "does this result satisfy *this declared acceptance criterion*?" — which is a concrete, checkable target rather than open-ended criticism; when `expected_output` is `None`, fall back to the free-text critique prompt (honest-degrade). Output: `@dataclass VerificationVerdict{ accepted: bool, confidence: float, critique: str, verifier_agent_id: str }`.
...
**Acceptance.** `tests/test_ad860_crew_verifier.py` (≥8, fake LLM judge): accepted verdict folds through; refuted verdict triggers re-dispatch with critique; verdict anchored to `expected_output` when present; `expected_output=None`→free-text-critique fallback path; convergence stops at `max_convergence_rounds` then escalates (not silent-accept); verifier ≠ producer enforced; no-independent-agent→unverified degrade; verdicts recorded against trust ledger; verdicts map to `Vote` shape. Verify Engineering-Principles compliance.
